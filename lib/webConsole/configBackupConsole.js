...
function restoreMaskedBackupSecrets(incoming, current) {
  if (incoming === undefined || incoming === null) {
    return incoming;
  }
  if (Array.isArray(incoming)) {
    const currentArray = Array.isArray(current) ? current : [];
    return incoming.map((item, index) => restoreMaskedBackupSecrets(item, currentArray[index]));
  }
  if (incoming && typeof incoming === 'object') {
    ...
    return Object.fromEntries(Object.entries(incoming).map(([key, item]) => [
      ...
      isSensitiveConfigKeySegment(key)
        ? (isMaskedSecretValue(item) && currentObject[key] !== undefined ? currentObject[key] : item)
        : restoreMaskedBackupSecrets(item, currentObject[key]),
    ]));
  }
  return incoming;
}
...
export function createConfigBackupConsole(options = {}) {
  ...
  const listConfigHistoryEntries = typeof options.listConfigHistoryEntries === 'function'
    ? options.listConfigHistoryEntries
    : async () => [];
  const rollbackConfigHistoryEntry = typeof options.rollbackConfigHistoryEntry === 'function'
    ? options.rollbackConfigHistoryEntry
    : async () => {
        throw new Error('当前环境不支持配置历史回滚');
      };
  ...
  function buildBackupPayload() {
    ...
    return {
      ...
      configRoot,
      files: maskConfigPayloadSecrets(allConfigs),
    };
  }
  ...
  async function restoreBackupPayload(payload = {}) {
    ...
    const normalizedFiles = normalizeConfigBackupFiles(files, createHttpError);
    const currentFiles = getAllConfigs() || {};
    const restoredFiles = Object.fromEntries(Object.entries(normalizedFiles).map(([key, value]) => [
      key,
      restoreMaskedBackupSecrets(value, currentFiles[key]),
    ]));
    ...
    const filesToRestore = selectedFiles.length > 0
      ? Object.fromEntries(Object.entries(restoredFiles).filter(([key]) => selectedFileSet.has(key)))
      : restoredFiles;
  ...
  function maskConfigHistoryChanges(changes = []) {
    return (Array.isArray(changes) ? changes : []).map(item => ({
      ...item,
      before: maskConfigRestorePreviewValue(item.key, item.before),
      after: maskConfigRestorePreviewValue(item.key, item.after),
      sensitive: isSensitiveConfigPreviewKey(item.key),
    }));
  }

  function summarizeConfigHistoryEntry(entry = {}) {
    return {
      id: String(entry.id || ''),
      createdAt: String(entry.createdAt || ''),
      action: String(entry.action || ''),
      source: String(entry.source || ''),
      fileKeys: Array.isArray(entry.fileKeys) ? entry.fileKeys.map(item => String(item || '')).filter(Boolean) : [],
      changedCount: Number(entry.changedCount || 0),
      truncatedChanges: entry.truncatedChanges === true,
      changes: maskConfigHistoryChanges(entry.changes).slice(0, 40),
    };
  }

  async function buildHistoryPayload(options = {}) {
    const limit = Math.max(1, Math.min(100, Number(options.limit || 20) || 20));
    const entries = await listConfigHistoryEntries({ limit });
    return {
      success: true,
      items: (Array.isArray(entries) ? entries : []).map(item => summarizeConfigHistoryEntry(item)),
      limit,
    };
  }

  async function rollbackHistoryPayload(payload = {}) {
    const id = String(payload?.id || '').trim();
    if (!id) {
      throw createHttpError(400, '缺少配置历史记录 ID', 'CONFIG_HISTORY_ID_REQUIRED');
    }
    const result = await rollbackConfigHistoryEntry(id);
    return {
      success: true,
      ...result,
    };
