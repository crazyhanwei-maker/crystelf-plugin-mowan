...
const PLUGIN_CATALOG_TASK_EVENT_LIMIT = 60;
const PLUGIN_CATALOG_TASK_OUTPUT_EVENT_LIMIT = 100;
...
function pushTaskEvent(task = {}, event = {}) {
  if (!task || typeof task !== 'object') return;
  const events = Array.isArray(task.events) ? task.events : [];
  events.push({
    time: new Date().toISOString(),
    level: String(event.level || 'info').trim() || 'info',
    stage: String(event.stage || task.stage || '').trim(),
    message: redactTaskText(String(event.message || '').trim(), task),
  });
  task.events = events.slice(-PLUGIN_CATALOG_TASK_EVENT_LIMIT);
}

function pushTaskOutputEvent(task = {}, stream = 'stdout', chunk = '') {
  if (!task || typeof task !== 'object') return;
  const text = redactTaskText(Buffer.isBuffer(chunk) ? chunk.toString('utf8') : String(chunk || ''), task).trim();
  if (!text) return;
  ...
  events.push({
    ...
    process: String(task.currentProcessLabel || '').trim(),
    text: text.length > 1200 ? `${text.slice(0, 1200)}...` : text,
  });
  task.outputEvents = events.slice(-PLUGIN_CATALOG_TASK_OUTPUT_EVENT_LIMIT);
...
function getPluginInstallProgress(task = {}) {
  const status = String(task.status || '').trim();
  if (status === 'success') return { percent: 100, label: '安装完成' };
  if (status === 'error') return { percent: 100, label: task.error ? '安装失败' : '已结束' };
  if (status === 'pending') return { percent: 5, label: '等待执行' };
  const steps = Array.isArray(task.steps) ? task.steps : [];
  const clone = steps.find(step => step.key === 'clone') || {};
  const dependencies = steps.find(step => step.key === 'dependencies') || {};
  if (clone.status === 'running') return { percent: 30, label: '正在克隆仓库' };
  if (clone.status === 'success' && dependencies.status === 'running') return { percent: 75, label: '正在安装依赖' };
  if (clone.status === 'success' && dependencies.status === 'skipped') return { percent: 90, label: '依赖安装已跳过' };
  if (clone.status === 'success') return { percent: 70, label: '仓库已克隆' };
  return { percent: 15, label: task.stage || '准备安装' };
}

function serializeTask(task = {}) {
  const progress = getPluginInstallProgress(task);
  return {
    id: task.id || '',
    ...
    events: Array.isArray(task.events) ? task.events.slice(-PLUGIN_CATALOG_TASK_EVENT_LIMIT).map(event => ({
      time: event.time || '',
      level: String(event.level || 'info').trim() || 'info',
      stage: String(event.stage || '').trim(),
      message: redactTaskText(event.message || '', task),
    })) : [],
    outputEvents: Array.isArray(task.outputEvents) ? task.outputEvents.slice(-PLUGIN_CATALOG_TASK_OUTPUT_EVENT_LIMIT).map(event => ({
      time: event.time || '',
      stream: String(event.stream || 'stdout').trim() === 'stderr' ? 'stderr' : 'stdout',
      stage: String(event.stage || '').trim(),
      process: String(event.process || '').trim(),
      text: redactTaskText(event.text || '', task),
    })) : [],
...
export function createPluginCatalogInstallTasks(options = {}) {
  ...
  async function executeTask(task = {}) {
    ...
    pushTaskEvent(task, { stage: task.stage, message: '插件安装任务开始执行，准备克隆仓库。' });
    try {
      ...
      pushTaskEvent(task, { stage: task.stage, message: `执行命令：${task.command.join(' ')}` });
      ...
      if (cloneResult.canceled || task.canceled === true) {
        ...
        pushTaskEvent(task, { level: 'error', stage: task.stage, message: task.error });
      ...
      pushTaskEvent(task, { stage: task.stage, message: '插件仓库克隆成功。' });
      ...
      if (task.installDependencies && fs.existsSync(manifestPath)) {
        ...
        pushTaskEvent(task, {
          stage: task.stage,
          message: `使用 ${packageManager.manager}${packageManager.version ? ` ${packageManager.version}` : ''} 安装插件依赖。`,
        });
        ...
        pushTaskEvent(task, { stage: task.stage, message: `插件依赖安装成功，包管理器：${packageManager.manager}。` });
        ...
        pushTaskEvent(task, { stage: 'install-dependencies', message: '目标插件没有 package.json，已跳过依赖安装。' });
      }

      task.stage = 'done';
      task.status = 'success';
      task.error = '';
      pushTaskEvent(task, { stage: task.stage, message: '插件安装任务完成。' });
      ...
      pushTaskEvent(task, { level: 'error', stage: task.stage || failedStepKey, message: task.error });
  ...
  function cancelInstallTask(taskId = '') {
    ...
    pushTaskEvent(task, { stage: task.stage || 'cancelled', message: '已请求取消插件安装任务。' });
