...
export function createSettingsRoutes(options = {}) {
  ...
  async function handle(req, res, url, context = {}) {
    ...
    if (url.pathname === '/api/persona-presets') {
      sendJson(res, { success: true, data: listPersonaPresets() });
      return true;
    }
    if (url.pathname === '/api/persona-presets/get') {
      try {
        const name = url.searchParams.get('name') || '';
        sendJson(res, { success: true, data: getPersonaPreset({ name }) });
      } catch (error) {
        sendJson(res, { success: false, error: error.message, code: error.code || '' }, getHttpErrorStatus(error, 404));
      }
      return true;
    }
    if (url.pathname === '/api/persona-presets/save' && req.method === 'POST') {
      if (rejectReadOnly(res, bootstrapMode)) return true;
      try {
        const body = await parseRequestBody(req);
        sendJson(res, { success: true, data: savePersonaPreset(body) });
      } catch (error) {
        sendJson(res, { success: false, error: error.message, code: error.code || '' }, getHttpErrorStatus(error, 400));
      }
      return true;
    }
    if (url.pathname === '/api/persona-presets/delete' && req.method === 'POST') {
      if (rejectReadOnly(res, bootstrapMode)) return true;
      try {
        const body = await parseRequestBody(req);
        sendJson(res, { success: true, data: deletePersonaPreset(body) });
      } catch (error) {
        sendJson(res, { success: false, error: error.message, code: error.code || '' }, getHttpErrorStatus(error, 404));
      }
      return true;
