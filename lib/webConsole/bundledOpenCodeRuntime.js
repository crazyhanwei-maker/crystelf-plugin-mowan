...
// 注册固定档位的同时生成 numeric 通道：用户在 UI 填纯数字（thinking 预算）时
// 合并进 variants，保证请求侧认识这个键（ anthropic → budgetTokens，openai → reasoningEffort 原值）。
...
// 自定义思考值 → providerOptions：anthropic 收纯数字（thinking 预算），其余协议原样透传。
// 新模型思考参数不等表/适配层更新——UI 填模型认识的值即可
function buildCustomVariantMap(isAnthropic) {
  return isAnthropic
    ? { numeric: { thinking: { type: 'enabled', budgetTokens: 0 } } }
    : { numeric: { reasoningEffort: 0 } };
}

// 把 numeric 占位替换成用户填的实际值（0 = 占位符，替换后必然非 0；非法值丢弃）
export function applyCustomVariantValue(variants, value) {
  const numeric = String(value ?? '').trim();
  if (!/^\d{1,9}$/.test(numeric) || Number(numeric) <= 0) return variants;
  const resolved = { ...variants };
  for (const [key, options] of Object.entries(resolved)) {
    if (key !== 'numeric' || !options || typeof options !== 'object') continue;
    if (options.thinking && typeof options.thinking === 'object') {
      resolved[key] = { thinking: { type: 'enabled', budgetTokens: Number(numeric) } };
    } else if ('reasoningEffort' in options) {
      resolved[key] = { reasoningEffort: numeric };
    }
  }
  return resolved;
}
...
export function buildOpenCodeConfig(aiConfig = {}, requestedModel = '', options = {}) {
  ...
  // 自定义思考值（纯数字 = anthropic thinking 预算；openai 原样 reasoningEffort）：
  // 只在值合法时挂 numeric 通道；无值/非法值不注册 numeric，避免空壳档位出现在 UI
  const customVariantMap = buildCustomVariantMap(normalizeAiProtocol(apiSource.apiProtocol || aiConfig.apiProtocol) === 'anthropic');
  const customValue = /^\d{1,9}$/.test(String(options.customVariantValue ?? '').trim()) && Number(options.customVariantValue) > 0
    ? applyCustomVariantValue(customVariantMap, options.customVariantValue).numeric
    : null;
  ...
  modelConfig.variants = {
    ...(isAnthropic ? ANTHROPIC_REASONING_VARIANTS : REASONING_VARIANTS),
    ...(customValue ? { numeric: customValue } : {}),
  };
    ...
    provider: {
      ...
      ...modelProviders.reduce((providers, item) => {
        ...
        providers[providerId] = {
          ...
          models: item.models.reduce((models, modelEntry) => {
            ...
            const customContext = Math.max(0, Math.round(Number(typeof modelEntry === 'object' ? modelEntry?.context : 0) || 0));
            const customVision = typeof modelEntry === 'object' ? modelEntry?.vision : undefined;
            models[key] = {
              name: key,
              // 视觉：用户显式声明优先（false 也尊重），否则默认放开（网关上模型家族判断不可靠）
              attachment: customVision === undefined || customVision === null ? true : customVision === true,
              modalities: { input: customVision === false ? ['text'] : ['text', 'image'], output: ['text'] },
              ...(customContext > 0 ? { limit: { context: customContext, output: 0 } } : {}),
              variants: {
                ...(providerIsAnthropic ? ANTHROPIC_REASONING_VARIANTS : REASONING_VARIANTS),
                ...buildCustomVariantMap(providerIsAnthropic),
              },
            };
            return models;
          }, {}),
...
export function buildBundledOpenCodeEnvironment(options = {}) {
  ...
  const config = buildOpenCodeConfig(options.aiConfig, options.model, {
    ...
    customVariantValue: options.customVariantValue,
