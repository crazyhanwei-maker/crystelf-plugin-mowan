...
export function createHelpDiyRoutes(options = {}) {
  const {
    ...
    deleteHelpDiyImage,
    getHelpDiyTemplates,
    generateHelpDiyImage,
    generateHelpDiyHtmlImage,
    getHttpErrorStatus,
    parseRequestBody,
    saveBase64Image,
    saveHelpDiyPayload,
  ...
  async function handle(req, res, url) {
    ...
    if (url.pathname === '/api/help-diy/generate-image' && req.method === 'POST') {
      if (rejectReadOnly(res)) return true;
      try {
        const body = await parseRequestBody(req);
        const slot = String(body?.slot || 'home');
        const text = String(body?.text || '');
        if (!text.trim()) {
          sendJson(res, { success: false, error: '帮助内容为空，无法生成图片' }, 400);
          return true;
        }
        const imageUrl = await generateHelpDiyImage(slot, text);
        sendJson(res, { success: true, url: imageUrl });
      } catch (error) {
        sendJson(res, { success: false, error: error.message, code: error.code || '' }, getHttpErrorStatus(error, 500));
      }
      return true;
    }
    if (url.pathname === '/api/help-diy/generate-html-image' && req.method === 'POST') {
      if (rejectReadOnly(res)) return true;
      try {
        const body = await parseRequestBody(req);
        const slot = String(body?.slot || 'home');
        ...
        if (!html.trim()) {
          sendJson(res, { success: false, error: 'HTML 内容为空，无法生成图片' }, 400);
          return true;
        }
        const imageUrl = await generateHelpDiyHtmlImage(slot, html);
        sendJson(res, { success: true, url: imageUrl });
      } catch (error) {
        sendJson(res, { success: false, error: error.message, code: error.code || '' }, getHttpErrorStatus(error, 500));
      }
