...
export function normalizeWebConsoleHost(value = '', fallback = '127.0.0.1') {
...
export function buildWebConsoleConfig(config = {}, options = {}) {
  ...
  return {
    ...
    host: normalizeWebConsoleHost(config.webConsoleHost || '127.0.0.1'),
...
export function getWebConsoleDisplayUrl(config = {}) {
  const host = formatWebConsoleHostForUrl(config.host || '127.0.0.1');
