export function createWebConsoleAuditLog(options = {}) {
  ...
  const sensitivePathPattern = /(?:login|logout|save|write|delete|clear|restore|install|upload|cleanup|reset|rollback|approve|reject|fix|toggle|mkdir|rename|create|send|bridge|agent-workbench)(?:$|[/?-])/i;
  ...
  function sanitizeAuditValue(value, depth = 0) {
    ...
    if (typeof value === 'object') {
      ...
      for (const [key, item] of Object.entries(value).slice(0, 60)) {
        if (/(api[_-]?key|secret|token|password|passwd|pwd|cookie|authorization)/i.test(key)) {
  ...
  function getAuditAction(pathname = '') {
    ...
    if (pathValue.includes('/file-browser/upload')) return 'file_upload';
    ...
    if (pathValue.includes('/command-center/bridge')) return 'command_bridge_change';
    if (pathValue.includes('/agent-workbench/settings')) return 'agent_workbench_settings';
    if (pathValue.includes('/agent-workbench/tasks') && pathValue.includes('/cancel')) return 'agent_workbench_cancel';
    if (pathValue.includes('/agent-workbench/tasks')) return 'agent_workbench_run';
  ...
  function recordWebConsoleOperation(entry = {}) {
    const startedAt = Date.now();
    const action = String(entry.action || 'operation').trim() || 'operation';
    appendAuditEntry({
      time: entry.time || new Date(startedAt).toISOString(),
      action,
      method: String(entry.method || 'TASK').toUpperCase(),
      path: String(entry.path || `operation:${action}`).trim(),
      queryKeys: [],
      clientIp: sanitizeHeader(entry.clientIp || 'internal', 80),
      userAgent: sanitizeHeader(entry.userAgent || 'webConsole-task', 240),
      referer: sanitizeHeader(entry.referer || '', 240),
      statusCode: Number(entry.statusCode || (entry.result === 'error' ? 500 : 200)),
      result: entry.result === 'error' ? 'error' : 'success',
      aborted: false,
  ...
  function buildWebConsoleAuditLogEntries(filters = {}) {
    ...
    const items = readAuditEntries()
      ...
      .filter(item => {
        ...
        return [item.action, item.method, item.path, item.clientIp, item.userAgent, item.statusCode, item.result, JSON.stringify(item.details || {})]
