...
export function createMorningReport(options = {}) {
  ...
  function buildReportText() {
    const now = new Date();
    const lines = [`🌅 灵晶晨报 ${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}-${String(now.getDate()).padStart(2, '0')}`];
    lines.push('━━━━━━━━━━━━━━━━━━');
    ...
    if (events.length === 0) {
      ...
      for (const event of events.slice(-5)) {
        ...
        lines.push(`  ${formatLocalTime(event.at)} ${label}（${event.detail || '-'}）`);
      }
    }
    ...
    if (backup) {
      if (backup.latest) {
        lines.push(`💾 备份：${backup.latest.name}（${backup.count} 份存量，最近 ${formatLocalTime(backup.latest.time)}）`);
      } else {
  ...
  async function runReport({ trigger = 'scheduled' } = {}) {
    ...
    const now = new Date();
    state.lastSentDay = `${now.getFullYear()}-${now.getMonth() + 1}-${now.getDate()}`;
