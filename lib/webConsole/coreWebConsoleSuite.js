...
export function createCoreWebConsoleSuite(options = {}) {
  ...
  let yunzaiCfgPromise = null;
  async function getYunzaiCfg() {
    if (yunzaiCfgPromise) return yunzaiCfgPromise;
    yunzaiCfgPromise = (async () => {
      try {
        const mod = await import('../../../../lib/config/config.js');
        return mod?.default || mod?.cfg || null;
      } catch {
        return null;
      }
    })();
    return yunzaiCfgPromise;
  }

  async function notifyLoginSuccess({ req, ip = '', method = 'token' } = {}) {
    try {
      const bot = typeof globalThis !== 'undefined' ? globalThis.Bot : null;
      if (!bot?.pickUser) return;
      const yunzaiCfg = await getYunzaiCfg();
      const masterIds = Array.isArray(yunzaiCfg?.masterQQ)
        ? yunzaiCfg.masterQQ.map(item => String(item ?? '').trim()).filter(Boolean)
        : [];
      ...
      for (const id of masterIds) {
        try {
          ...
          logger.warn(`[webConsole] 登录通知发送失败 ${id}: ${error.message}`);
        }
      }
    } catch (error) {
      logger.warn(`[webConsole] 登录通知处理失败: ${error.message}`);
    }
  }
