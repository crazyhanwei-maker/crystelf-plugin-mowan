...
export function createQqSimulatorConsole(options = {}) {
  ...
  function normalizeQqSimulatorDispatchMode(value = '') {
    return String(value || '').trim().toLowerCase() === 'replay' ? 'replay' : 'safe';
  }
  ...
  function buildQqSimulatorAiPrompt(payload = {}) {
    ...
    if (payload.voice) {
      if (payload.voice.transcript) {
        parts.push(`用户发送了一条语音消息，模拟器提供的语音转写如下：${payload.voice.transcript}`);
      } else {
        parts.push(`用户发送了一条语音消息，但模拟器没有语音转写。语音文件：${payload.voice.name || 'voice-message'}，类型：${payload.voice.mimeType || 'audio/*'}。`);
      }
    }
  ...
  function normalizeQqSimulatorPayload(payload = {}) {
    ...
    if (!['message', 'join_request', 'group_increase', 'poke'].includes(eventType)) {
      throw createHttpError(400, '事件类型不正确', 'QQ_SIMULATOR_EVENT_INVALID');
    }
    const groupId = normalizeGroupIdValue(source.groupId);
    ...
    const groupHistory = normalizeQqSimulatorGroupHistory(source.groupHistory || source.group_history);
    if (eventType === 'message' && !messageText && images.length === 0 && !voice) {
  ...
  function normalizeQqSimulatorScenarioPayload(payload = {}) {
    ...
    const normalizedEventType = ['message', 'join_request', 'group_increase', 'poke'].includes(eventType)
      ? eventType
      : 'message';
    return {
      eventType: normalizedEventType,
      groupId: normalizeGroupIdValue(source.groupId) || String(source.groupId || '').trim().slice(0, 20),
      ...
      conversationMode: readBoolean(source.conversationMode, true),
      history: [],
      groupHistory: normalizeQqSimulatorGroupHistory(source.groupHistory || source.group_history),
  ...
  function createQqSimulatorContext(payload = {}) {
    ...
    return {
      ...
      debug: {
        safeMode: true,
        note: 'QQ simulator does not send real group messages or execute destructive QQ operations.',
  ...
  async function resolveQqSimulatorMemeSegment(ctx = {}, segment = {}, options = {}) {
    ...
    try {
      ...
      const actionMessage = options.actionMessage || (displayUrl ? 'AI 回复会发送表情包图片' : 'AI 回复会尝试发送表情包，但模拟器无法预览图片');
  ...
  async function appendQqSimulatorImagePathReply(ctx = {}, imagePath = '', label = 'Bot 表情包', detail = {}) {
    ...
    pushQqSimulatorAction(ctx, displayUrl ? 'sandbox_emoji_image_reply' : 'sandbox_emoji_image_unpreviewable', displayUrl ? '聊天引擎生成了表情包图片' : '聊天引擎生成了表情包，但模拟器无法预览图片', {
  ...
  function buildQqSimulatorOnebotEvent(payload = {}, ctx = {}, adapterFormat = 'onebot') {
    ...
    const base = {
      ...
      group: buildQqSimulatorGroupMock(payload, adapterFormat),
  ...
  function buildQqSimulatorIcqqEvent(payload = {}, ctx = {}) {
    ...
    const base = {
      ...
      group: buildQqSimulatorGroupMock(payload, 'icqq'),
      group_history: groupHistory,
  ...
  function simulateQqKnownCommand(payload = {}, ctx = {}) {
    ...
    if (/^#群总结$/.test(text)) {
      ...
      ctx.replies.push(historyCount > 0
        ? `模拟结果：会基于模拟器提供的 ${historyCount} 条群历史进入群总结生成/读取流程。`
  ...
  function simulateQqReplayDispatch(payload = {}, ctx = {}) {
    if (payload.dispatchMode !== 'replay') {
      return;
    }
    pushQqSimulatorTimeline(ctx, '链路回放模式', 'start', {
      eventType: payload.eventType,
      note: '按插件常见事件顺序回放命中情况；保持安全模拟，不执行真实写入或群操作。',
    });
  ...
  async function simulateQqAiReply(payload = {}, ctx = {}) {
    ...
    if (!payload.includeAt) {
      pushQqSimulatorLog(ctx, 'info', '消息未 @Bot，模拟器按未直接触发 AI 处理');
      return;
    }
    try {
      ...
      const sandbox = await runSandboxChat({
        prompt: aiPrompt,
        sessionId: `qq-simulator-${payload.groupId}-${payload.userId}`,
      ...
      if (shouldForceMeme) {
        const forcedMeme = await resolveQqSimulatorMemeSegment(ctx, memeRequest, {
          ...
          actionMessage: '用户明确要求表情包，模拟器已按配置补发表情包图片',
          failedActionType: 'meme_request_resolution_failed',
          failedActionMessage: '用户明确要求表情包，但模拟器解析表情包失败',
        });
  ...
  function simulateQqJoinRequest(payload = {}, ctx = {}) {
    ...
    if (applicant.listStatus === 'blacklist') {
      ...
      moderation.blacklistEntry = { userId: payload.userId, note: 'QQ 模拟器手动标记黑名单' };
      ...
      moderation.whitelistEntry = { userId: payload.userId, note: 'QQ 模拟器手动标记白名单' };
  ...
  async function buildQqSimulatorSendPayload(payload = {}) {
    ...
    pushQqSimulatorLog(ctx, 'info', '已构造 QQ 模拟事件', {
      eventType: normalized.eventType,
    ...
    if (normalized.eventType === 'message') {
      ...
      const handledCommand = simulateQqKnownCommand(normalized, ctx);
