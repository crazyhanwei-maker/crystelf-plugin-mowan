...
export function createWebConsoleRuntime(options = {}) {
  ...
  const notifyMaster = typeof options.notifyMaster === 'function'
    ? options.notifyMaster
    : null;
  ...
  function getStartupSelfCheckSnapshot() {
    return currentInfo?.startupSelfCheck || null;
  }
      ...
      if (notifyMaster) {
        notifyMaster(
          [
            ...
            '━━━━━━━━━━━━━━',
            `访问地址：${info.url}`,
            `登录口令：${appConfig.authToken}`,
            '',
            '口令已自动保存到配置文件，登录后可在设置中修改。',
          ].join('\n'),
          { label: '口令生成通知' },
        ).catch(() => {});
      }
