...
import { createAgentWorkbenchRoutes } from './agentWorkbenchRoutes.js';
...
export function createWebConsoleHandler(options = {}) {
  ...
  const agentWorkbenchRoutes = createAgentWorkbenchRoutes(options);
  ...
  function buildOperationTaskCenterPayload() {
    ...
    const pluginTasks = typeof listPluginCatalogInstallTasks === 'function'
      ...
      : [];
    const agentTasks = typeof listAgentWorkbenchTasks === 'function'
      ? listAgentWorkbenchTasks({ limit: 20 })
      : [];
    ...
    const tasks = [
      ...
      ...agentTasks.map(task => buildOperationTaskPayload(task, {
        key: `agent:${task.id}`,
        type: 'agent_task',
        typeLabel: 'Agent 任务',
        title: task.title || task.providerLabel || 'Agent 任务',
        target: `${task.providerLabel || task.providerId || '-'} / ${task.workspaceLabel || task.workspaceId || '-'}`,
        stage: task.stage || task.mode || 'agent',
        href: `/agent-workbench.html?task=${encodeURIComponent(task.id || '')}`,
      })),
    ];
    ...
    return {
      ...
      summary: {
        ...
        agentCount: uniqueTasks.filter(task => task.type === 'agent_task').length,
      },
      tasks: uniqueTasks,
    };
  }
  ...
  return async (req, res) => {
    ...
    if (await agentWorkbenchRoutes.handle(req, res, url)) return;
    if (url.pathname === '/api/command-center/bridge' && req.method === 'POST') {
      if (rejectReadOnly(res)) return;
      try {
        const body = await parseRequestBody(req);
        return sendJson(res, await saveCommandBridgeConfig(body));
      } catch (error) {
        return sendJson(res, {
          success: false,
    ...
    if (url.pathname === '/api/command-center') {
      return sendJson(res, await buildCommandCenterPayload());
    }
    if (url.pathname === '/api/global-search') {
      return sendJson(res, await buildGlobalSearchPayload({
        query: url.searchParams.get('q') || url.searchParams.get('query') || '',
        limit: url.searchParams.get('limit') || '',
      }));
    }
    if (url.pathname === '/api/group-summary/diagnostics') {
      return sendJson(res, await buildGroupSummaryDiagnosticsPayload({
        dateKey: url.searchParams.get('date') || url.searchParams.get('dateKey') || '',
        limit: url.searchParams.get('limit') || '',
      }));
    }
