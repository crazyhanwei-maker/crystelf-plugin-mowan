...
export function createTaskCenterConsole(options = {}) {
  const buildScheduledBackupTaskPayload = typeof options.buildScheduledBackupTaskPayload === 'function'
    ? options.buildScheduledBackupTaskPayload
    : null;
  ...
  function buildTaskCenterPayload() {
    ...
    if (buildScheduledBackupTaskPayload) {
      tasks.push(buildScheduledBackupTaskPayload());
    }
    // 晨报任务卡片（由 server.js 注入，未注入时跳过）
    try {
      const morning = typeof options.getMorningReportTask === 'function' ? options.getMorningReportTask() : null;
      if (morning && morning.id) {
        tasks.push({
          ...morning,
          description: '每天定时聚合看门狗事件、AI 用量、备份与资源水位推送给主人。',
          detail: {},
          recentRuns: [],
          manual: {
            supported: true,
            action: 'run-morning-report',
            label: '立即发送一次',
          },
        });
      }
  ...
  const runScheduledBackupAction = typeof options.runScheduledBackup === 'function' ? options.runScheduledBackup : null;
  const runMorningReportAction = typeof options.runMorningReport === 'function' ? options.runMorningReport : null;

  async function runTaskManually(action = '') {
    if (action === 'run-morning-report' && runMorningReportAction) {
      const result = await runMorningReportAction({ trigger: 'manual' });
      return result?.success
        ? { success: true, message: '晨报已发送给主人' }
        : { success: false, error: result?.error || '晨报发送失败' };
    }
    if (action === 'run-scheduled-backup' && runScheduledBackupAction) {
      const result = await runScheduledBackupAction({ trigger: 'manual' });
      return result?.success
        ? { success: true, message: `已备份 ${result.fileCount} 个配置文件${result.removed ? `，清理 ${result.removed} 份旧备份` : ''}` }
        : { success: false, error: result?.error || '备份失败' };
    }
