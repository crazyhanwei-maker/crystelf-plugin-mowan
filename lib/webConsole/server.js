...
import { getUsageOverviewSync, normalizeUsageScene, getUsageTrendByDaysSync, setUsageTimezoneOffsetResolver } from '../ai/usageLogger.js';
...
import { createWechatBridgeNotifier } from './wechatBridgeNotifier.js';
import { createResourceWatchdog } from './resourceWatchdog.js';
import { createMorningReport } from './morningReport.js';
import { setSummaryTimezoneOffsetResolver } from '../groupSummary/dailyGroupSummaryStore.js';
...
import { getOrCreateWorkbenchConsole } from './workbenchSingleton.js';
...
// 统一时区偏移：晨报时刻、用量按天统计、群总结调度与日期分片均按此时区（默认北京时间）。
// ConfigControl 在 server.js 加载时已初始化，此处同步读取一次；运行中改动需重启生效。
setUsageTimezoneOffsetResolver(() => {
  const value = Number((ConfigControl.get('config') || {}).timezoneOffset);
  return Number.isFinite(value) ? value : 480;
});
setSummaryTimezoneOffsetResolver(() => {
  const value = Number((ConfigControl.get('config') || {}).timezoneOffset);
  return Number.isFinite(value) ? value : 480;
});
...
// 微信桥通知通道：桥已登录且轮询运行时，把看门狗告警发给微信白名单用户（icqq 掉线时不经过 QQ 协议的旁路）。
const wechatBridgeNotifier = createWechatBridgeNotifier({ logger });

const botAdapterWatchdog = createBotAdapterWatchdog({
  logger,
  notifyMasters: masterNotifier.notifyMasters,
  notifyWechatBridge: wechatBridgeNotifier.notifyWechatBridge,
  getEnabled: () => (ConfigControl.get('config') || {}).botWatchdogEnabled !== false,
  sendAlertEmail: (...args) => smtpAlertMailer.send(...args),
});

// 资源水位看门狗：磁盘/系统内存/Redis 超阈值时 QQ+微信桥+邮件告警，阈值可配置
...
const morningReport = createMorningReport({
  ...
  getTimezoneOffset: () => (ConfigControl.get('config') || {}).timezoneOffset,
  getWatchdogEvents: () => botAdapterWatchdog.getRecentEvents(),
  getSavedDay: () => {
    try {
      return JSON.parse(fs.readFileSync(path.join(Path.config, 'morning-report-state.json'), 'utf8')).lastSentDay || '';
    } catch {
      return '';
    }
  },
  saveSentDay: (day) => {
    try {
      fs.writeFileSync(path.join(Path.config, 'morning-report-state.json'), `${JSON.stringify({ lastSentDay: day }, null, 2)}\n`, 'utf8');
    } catch (error) {
      logger.warn(`[morning-report] 持久化已发日期失败: ${error.message}`);
    }
  },
  getYesterdayUsage: () => {
  ...
  getWeeklyTrend: () => getUsageTrendByDaysSync(14),
  getBackupStatus: () => {
...
const agentWorkbenchConsole = getOrCreateWorkbenchConsole({
  ConfigControl,
  logger,
