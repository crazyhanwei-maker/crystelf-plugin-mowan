...
function getMemeSavedImageExtension(contentType = '', sourceUrl = '') {
  ...
  try {
    ...
    // Ignore malformed source URLs and fall back to the default extension.
  }
...
async function testMemeApiConnection(payload = {}) {
  ...
  if (localResolved.imagePath) {
    try {
      ...
      // Ignore missing local files and keep the fallback path empty.
    }
  }
  ...
  if (markdownEnabled) {
    for (const item of searchItems.slice(0, 2)) {
      try {
        ...
        // Ignore markdown fetch failures and continue with other sources.
      }
    }
  }
