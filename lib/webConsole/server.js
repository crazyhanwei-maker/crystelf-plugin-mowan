...
const IMAGE_MONITOR_DIR = path.join(process.cwd(), 'data', 'crystelf', 'image-monitor');
...
const IMAGE_MONITOR_MEME_DIR = path.join(process.cwd(), 'data', 'crystelf', 'image-monitor', 'memes');
...
const DEFAULT_CONSOLE_BACKGROUND_SOURCE_URL = 'https://www.loliapi.com/acg/pc/';
...
function sanitizeHelpDiyUploadPrefix(prefix = 'help') {
...
function saveBase64Image(dataUrl = '', prefix = 'help') {
  ...
  const safePrefix = sanitizeHelpDiyUploadPrefix(prefix);
  const fileName = `${safePrefix}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}.${ext}`;
  const filePath = path.resolve(HELP_DIY_UPLOAD_DIR, fileName);
...
function normalizeConsoleBackgroundSourceUrl(value = '') {
...
function getConsoleBackgroundSourceUrl() {
  const config = ConfigControl.get('config') || {};
  return normalizeConsoleBackgroundSourceUrl(
...
async function refreshConsoleBackgroundCache() {
  ...
  const sourceUrl = getConsoleBackgroundSourceUrl();
  if (!sourceUrl) {
    return writeConsoleBackgroundFallbackCache({ reason: 'source_empty' });
  }
  ...
  try {
    ...
    return writeConsoleBackgroundFallbackCache({ reason: error.message || 'remote_failed' });
  }
}

function buildConsoleBackgroundFallbackSvg(version = Date.now()) {
  const palettes = [
...
function writeConsoleBackgroundFallbackCache(options = {}) {
  ensureConsoleBackgroundCacheDir();
  const version = Date.now();
  const buffer = Buffer.from(buildConsoleBackgroundFallbackSvg(version), 'utf8');
  const meta = {
    sourceUrl: 'local-dynamic-fallback',
...
function getWebConsoleConfig() {
  ...
  return {
    ...
    backgroundSourceUrl: getConsoleBackgroundSourceUrl(),
  };
}
...
function buildDependencyReportSummaryOnly(report = {}) {
  ...
  const { projectRoot, manifestPath, ...safeSummary } = summary;
  return {
    summary: safeSummary,
  };
}
...
function normalizeSensitiveConfigKeySegment(segment = '') {
...
function isSensitiveConfigKeySegment(segment = '') {
...
function normalizeConfigBackupFileKey(key = '') {
...
function normalizeConfigBackupFiles(files = {}) {
  const normalizedFiles = {};
  for (const [key, value] of Object.entries(files || {})) {
    const normalizedKey = normalizeConfigBackupFileKey(key);
...
function buildConfigRestorePreviewPayload(payload = {}) {
  ...
  const normalizedFiles = normalizeConfigBackupFiles(files);
...
async function restoreConfigBackupPayload(payload = {}) {
  ...
  const selectedFiles = Array.isArray(payload?.selectedFiles)
    ? payload.selectedFiles.map(item => String(item || '').trim()).filter(Boolean).map(item => normalizeConfigBackupFileKey(item))
    : [];
...
function buildImageMonitorUsageTrendPayload() {
  ...
  const items = parseJsonObjects(content)
    ...
    .filter(Boolean)
    .filter(item => isImageMonitorReviewUsage(item));
  ...
  if (payload.ai) {
    ...
    if (payload.ai.memeConfig) {
      nextAi.memeConfig = {
        ...
        localBaseDir: normalizeWebConsoleMemeLocalBaseDir(payload.ai.memeConfig.localBaseDir),
      };
    }
...
function normalizeWebConsoleMemeLocalBaseDir(value = '') {
  const raw = String(value || '').trim();
  if (!raw) {
    return '';
  }
  ...
  const resolvedPath = path.isAbsolute(raw)
    ? path.resolve(raw)
    : path.resolve(FILE_BROWSER_ROOT_DIR, raw);
  if (!isPathInsideRoot(resolvedPath, FILE_BROWSER_ROOT_DIR)) {
    throw createHttpError(400, '本地表情目录必须位于项目目录内', 'MEME_LOCAL_DIR_OUT_OF_RANGE');
  }

  ensurePathResolvedWithinRoot(resolvedPath, FILE_BROWSER_ROOT_DIR, { allowMissing: true });
  return path.relative(FILE_BROWSER_ROOT_DIR, resolvedPath).replace(/\\/g, '/');
}
...
async function testMemeApiConnection(payload = {}) {
  ...
  try {
    mergedMemeConfig.localBaseDir = normalizeWebConsoleMemeLocalBaseDir(mergedMemeConfig.localBaseDir);
  } catch (error) {
    return {
      success: false,
      error: error.message,
...
async function buildLocalMemeScanPayload(payload = {}) {
  ...
  scanConfig.localBaseDir = normalizeWebConsoleMemeLocalBaseDir(scanConfig.localBaseDir);
...
function buildEditableConfigPayload() {
  ...
  return {
    webConsole: {
      ...
      webConsoleHost: readOnly ? '' : config.webConsoleHost || '127.0.0.1',
...
function resolveImageMonitorCandidatePath(filePath = '') {
  ...
  const resolvedPath = path.isAbsolute(rawPath)
    ...
    : path.resolve(IMAGE_MONITOR_DIR, rawPath);
  if (!isPathInsideDir(resolvedPath, IMAGE_MONITOR_DIR)) {
    return '';
  }
  return resolvedPath;
}

function getImageMonitorLocalImageContentType(filePath = '') {
...
function pushImageMonitorPathCandidate(candidates, filePath = '') {
...
function pushImageMonitorJoinedPathCandidate(candidates, ...segments) {
...
function findImageMonitorMemeEntryByHash(hash = '', memeEntries = []) {
...
function buildImageMonitorLocalPathCandidates(entry = {}, type = 'review', memeEntries = []) {
  const candidates = [];
  ...
  const appendMemeEntryCandidates = memeEntry => {
    ...
    pushImageMonitorPathCandidate(candidates, memeEntry.filePath);
    pushImageMonitorJoinedPathCandidate(candidates, IMAGE_MONITOR_MEME_DIR, memeEntry.relativeDir, memeEntry.fileName);
    pushImageMonitorJoinedPathCandidate(
      candidates,
    ...
    pushImageMonitorJoinedPathCandidate(candidates, IMAGE_MONITOR_MEME_DIR, memeEntry.folder, memeEntry.fileName);
  };
  ...
  pushImageMonitorPathCandidate(candidates, entry.reviewFilePath);
  pushImageMonitorJoinedPathCandidate(candidates, IMAGE_MONITOR_DIR, entry.reviewRelativePath);
  pushImageMonitorJoinedPathCandidate(candidates, IMAGE_MONITOR_DIR, 'reviews', entry.reviewFileName);
  ...
  if (entry.hash && reviewDate) {
    for (const ext of ['jpg', 'png', 'gif', 'webp', 'avif', 'bmp']) {
      pushImageMonitorJoinedPathCandidate(candidates, IMAGE_MONITOR_DIR, 'reviews', reviewDate, reviewGroupId, `${String(entry.hash).slice(0, 16)}.${ext}`);
    }
  }
  pushImageMonitorPathCandidate(candidates, entry.memeFilePath);
  pushImageMonitorPathCandidate(candidates, entry.filePath);
  appendMemeEntryCandidates(findImageMonitorMemeEntryByHash(entry.hash, memeEntries));
  pushImageMonitorJoinedPathCandidate(candidates, IMAGE_MONITOR_MEME_DIR, entry.memeCharacter, entry.memeEmotion || 'default', entry.memeFileName);
  pushImageMonitorJoinedPathCandidate(candidates, IMAGE_MONITOR_MEME_DIR, entry.memeCharacter, entry.memeFileName);
  return candidates;
}

function resolveImageMonitorLocalImagePath(entry = {}, type = 'review', memeEntries = []) {
  const candidates = buildImageMonitorLocalPathCandidates(entry, type, memeEntries);
  for (const candidate of candidates) {
    const resolvedPath = resolveImageMonitorCandidatePath(candidate);
    if (!resolvedPath) {
      continue;
    }
    try {
      ...
      const realBasePath = getRealPathSafe(IMAGE_MONITOR_DIR) || path.resolve(IMAGE_MONITOR_DIR);
...
function buildImageMonitorLocalImageUrl(type = 'review', entryId = '') {
...
function getImageMonitorOriginalImageUrl(entry = {}, type = 'review') {
  return String(type === 'meme' ? entry.sourceUrl || entry.imageUrl || '' : entry.imageUrl || entry.sourceUrl || '').trim();
}

function decorateImageMonitorLogEntry(entry = {}, type = 'review', memeEntries = []) {
  const targetType = String(type || 'review').trim() === 'meme' ? 'meme' : 'review';
  const id = String(entry.id || entry.__line || '').trim();
  const localPath = resolveImageMonitorLocalImagePath(entry, targetType, memeEntries);
  const localImageUrl = localPath ? buildImageMonitorLocalImageUrl(targetType, id) : '';
  const originalImageUrl = getImageMonitorOriginalImageUrl(entry, targetType);
  ...
  return {
    ...
    localImagePath: localPath ? path.relative(IMAGE_MONITOR_DIR, localPath).replace(/\\/g, '/') : '',
    originalImageUrl,
...
function findRawImageMonitorLogEntry(type, entryId) {
  const targetType = String(type || 'review').trim();
...
function serveImageMonitorLocalImage(res, type, entryId) {
  const targetType = String(type || 'review').trim() === 'meme' ? 'meme' : 'review';
  const entry = findRawImageMonitorLogEntry(targetType, entryId);
  if (!entry) {
    return sendJson(res, { success: false, error: 'Not found' }, 404);
  }
  const memeEntries = targetType === 'review' ? readJsonLines(IMAGE_MONITOR_MEME_INDEX) : [];
  const filePath = resolveImageMonitorLocalImagePath(entry, targetType, memeEntries);
  if (!filePath) {
    return sendJson(res, { success: false, error: 'Local image not found' }, 404);
  }
  const contentType = getImageMonitorLocalImageContentType(filePath);
...
function deleteImageMonitorLocalFile(filePath = '') {
...
function normalizeImageMonitorMatcherList(value) {
...
function imageMonitorTextIncludesToken(value = '', token = '') {
  const text = String(value || '').trim().toLowerCase();
  const keyword = String(token || '').trim().toLowerCase();
  return Boolean(text && keyword && text.includes(keyword));
}

function matchesImageMonitorMemeSaveFilter(entry = {}, cfg = {}) {
  const allowedCharacters = normalizeImageMonitorMatcherList(cfg.saveMemeCharacters);
  const allowedKeywords = normalizeImageMonitorMatcherList(cfg.saveMemeKeywords);
  ...
  return allowedCharacters.some(item => imageMonitorTextIncludesToken(character, item))
    || allowedKeywords.some(keyword => searchFields.some(value => imageMonitorTextIncludesToken(value, keyword)));
}

function cleanupImageMonitorNonMemePayload() {
  const reviewEntries = readJsonLines(IMAGE_MONITOR_REVIEW_LOG);
  const memeEntries = readJsonLines(IMAGE_MONITOR_MEME_INDEX);
  ...
  for (const item of [...nonMemeEntries, ...removedMemeEntries]) {
    for (const filePath of [item.filePath, item.memeFilePath, item.reviewFilePath]) {
      const result = deleteImageMonitorLocalFile(filePath);
  ...
  if (keptReviewEntries.length !== reviewEntries.length) {
    writeJsonLines(IMAGE_MONITOR_REVIEW_LOG, keptReviewEntries);
  }
  if (keptMemeEntries.length !== memeEntries.length) {
    writeJsonLines(IMAGE_MONITOR_MEME_INDEX, keptMemeEntries);
  }
  const emptyDirResult = cleanupEmptyDirectories(IMAGE_MONITOR_MEME_DIR);
...
function cleanupImageMonitorUnmatchedMemePayload() {
  const imageMonitorConfig = ConfigControl.get('imageMonitor') || {};
  const allowedCharacters = normalizeImageMonitorMatcherList(imageMonitorConfig.saveMemeCharacters);
  const allowedKeywords = normalizeImageMonitorMatcherList(imageMonitorConfig.saveMemeKeywords);
  if (allowedCharacters.length === 0 && allowedKeywords.length === 0) {
    return {
      success: true,
      skipped: true,
  ...
  const memeEntries = readJsonLines(IMAGE_MONITOR_MEME_INDEX);
  const keptMemeEntries = memeEntries.filter(item => matchesImageMonitorMemeSaveFilter(item, imageMonitorConfig));
  const removedMemeEntries = memeEntries.filter(item => !matchesImageMonitorMemeSaveFilter(item, imageMonitorConfig));
  const fileResults = [];

  for (const item of removedMemeEntries) {
    const result = deleteImageMonitorLocalFile(item.filePath);
    if (!result.skipped || result.reason !== 'empty_path') {
      fileResults.push(result);
    }
  }
...
function buildImageMonitorLogPayload(filters = {}) {
  ...
  const memeEntries = targetType === 'review' ? readJsonLines(IMAGE_MONITOR_MEME_INDEX) : [];
  const raw = readJsonLines(targetType === 'meme' ? IMAGE_MONITOR_MEME_INDEX : IMAGE_MONITOR_REVIEW_LOG)
    .map(item => ({
      ...item,
      id: String(item.__line || ''),
    }))
  ...
  return {
    ...payload,
    items: payload.items.map(item => decorateImageMonitorLogEntry(item, targetType, memeEntries)),
  };
}

function findImageMonitorLogEntry(type, entryId) {
  const targetType = String(type || 'review').trim() === 'meme' ? 'meme' : 'review';
  const item = findRawImageMonitorLogEntry(targetType, entryId);
  if (!item) {
    return null;
  }
  const memeEntries = targetType === 'review' ? readJsonLines(IMAGE_MONITOR_MEME_INDEX) : [];
  return decorateImageMonitorLogEntry(item, targetType, memeEntries);
}
...
async function buildSandboxConfigStatusPayload(query = {}) {
  ...
  const isPlaceholder = !key || key === 'your-api-key' || key === 'your api key';
  const baseApi = String(aiConfig.baseApi || '').trim();
  return {
...
function serveStatic(req, res) {
  ...
  const bootstrapMode = isBootstrapSetupRequest(req);
  if (isPathInsideRoot(targetPath, HELP_DIY_UPLOAD_DIR) && !isAuthorized(req)) {
...
function createHandler() {
  return async (req, res) => {
    ...
    if (url.pathname === '/api/logs/image-monitor/cleanup-non-meme' && req.method === 'POST') {
      ...
      try {
        return sendJson(res, cleanupImageMonitorNonMemePayload());
      } catch (error) {
        return sendJson(res, { success: false, error: error.message }, getHttpErrorStatus(error, 500));
      }
    }
    if (url.pathname === '/api/logs/image-monitor/cleanup-unmatched-meme' && req.method === 'POST') {
      ...
      try {
        return sendJson(res, cleanupImageMonitorUnmatchedMemePayload());
      } catch (error) {
        return sendJson(res, { success: false, error: error.message }, getHttpErrorStatus(error, 500));
      }
    }
    ...
    if (url.pathname === '/api/logs/image-monitor/local-image') {
      ...
      const type = url.searchParams.get('type') || 'review';
      try {
        return serveImageMonitorLocalImage(res, type, entryId);
      } catch (error) {
        return sendJson(res, { success: false, error: error.message }, getHttpErrorStatus(error, 500));
      }
    }
    ...
    if (url.pathname === '/api/export') {
      ...
      if (isLogExportType(type) && !requireLogsExposed(res)) {
        return;
      }
