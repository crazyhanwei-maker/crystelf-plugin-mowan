...
function buildFileBrowserTreePayload(pathValue = '') {
  ...
  return {
    ...
    root: {
      ...
      absolutePath: FILE_BROWSER_ROOT_DIR,
    },
    current: {
      path: target.relativePath,
      name: getFileBrowserDisplayName(target.relativePath),
...
function deleteHelpDiyImage(urlPath = '') {
  ...
  if (!normalized.startsWith('/uploads/help-diy/')) {
    throw new Error('只允许删除 help DIY 上传目录中的图片');
  }
  ...
  return [
    {
      ...
      payload: {
        ...
        categories: {
          ...
          manage: ['????', '', '- #????', '- #????', '- #??????'].join('\n'),
          fun: ['????', '', '- 60s', '- ???', '- #rss?? ???'].join('\n'),
          debug: ['????', '', '- ????? http://127.0.0.1:27891/', '- ????????????????'].join('\n'),
        },
      },
    },
    {
...
async function proxyRemoteImage(targetUrl) {
  ...
  if (!response.ok) {
    throw new Error(`拉取控制台背景图失败，HTTP 状态码: ${response.status}`);
  }
...
function normalizeImageContentType(contentType = '') {
  ...
  if (!normalized.startsWith('image/')) {
    throw new Error('控制台背景图返回的不是图片资源');
  }
...
function resolveDependencyInstallRequest(payload = {}) {
  ...
  if (!dependencyName) {
    throw createHttpError(400, '????????', 'DEPENDENCY_NAME_REQUIRED');
  }
  ...
  if (!group || !group.installSupported) {
    throw createHttpError(400, '????????', 'UNSUPPORTED_DEPENDENCY_TYPE');
  }

  const target = resolveDependencyInstallTarget(payload);
  const manifest = readJsonFileSafe(target.manifestPath, null);
  if (!manifest || typeof manifest !== 'object') {
    throw createHttpError(500, '???? package.json ????', 'TARGET_MANIFEST_READ_FAILED');
  }
  ...
  if (!declaredDependencies || typeof declaredDependencies !== 'object' || !(dependencyName in declaredDependencies)) {
    throw createHttpError(409, '???? package.json ???????', 'DEPENDENCY_DECLARATION_MISSING');
  }
  ...
  if (requestedVersion && declaredVersion && requestedVersion !== declaredVersion) {
    throw createHttpError(409, '????????????????', 'DEPENDENCY_VERSION_CHANGED');
  }

  const installedInfo = resolveInstalledDependencyInfo(dependencyName, target.targetDir);
  if (installedInfo.installed) {
    throw createHttpError(409, '????????????', 'DEPENDENCY_ALREADY_INSTALLED');
  }
...
function getDependencyInstallTask(taskId = '') {
  ...
  if (!normalizedTaskId) {
    throw createHttpError(400, '?? ID ????', 'TASK_ID_REQUIRED');
  }
...
async function executeDependencyInstallTask(task = {}) {
  return runDependencyInstallTargetExclusive(task.targetKey || task.absoluteTargetDir, async () => {
    ...
    try {
      ...
      logger.mark(`[webConsole] ??????: ${commandLine.join(' ')} @ ${task.absoluteTargetDir}`);
      ...
      if (result.status !== 0) {
        task.status = 'error';
        task.error = task.stderrTail || task.stdoutTail || `??????????: ${result.status}`;
        return task;
      }
      ...
      task.error = task.timedOut
        ? '??????'
        : String(error?.message || task.stderrTail || task.stdoutTail || '??????').trim();
      logger.error(`[webConsole] ??????: ${task.name} -> ${task.error}`);
      return task;
    } finally {
...
function buildConfigPayload() {
  ...
  const mainConfig = webConsole.maskSensitiveConfig
    ? {
        ...(allConfigs.config || {}),
  ...
  try {
    ...
    if (response.body && typeof response.body.cancel === 'function') {
      try {
        ...
        // Ignore body cancellation failures after the response has been fully consumed.
      }
    }
