...
export function createLogRoutes(options = {}) {
  ...
  async function handle(req, res, url) {
    ...
    if (url.pathname === '/api/task-center/backups') {
      sendJson(res, { success: true, backups: (listScheduledBackups ? listScheduledBackups() : []).slice(0, 30) });
      return true;
    }
    ...
    if (url.pathname === '/api/bot-watchdog') {
      sendJson(res, { success: true, ...(typeof buildBotAdapterWatchdogStatus === 'function' ? buildBotAdapterWatchdogStatus() : {}) });
      return true;
    }
