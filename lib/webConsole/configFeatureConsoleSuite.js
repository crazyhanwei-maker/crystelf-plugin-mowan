...
export function createConfigFeatureConsoleSuite(options = {}) {
  ...
  const configSourceDiagnostics = createApiConfigSourceDiagnostics({
    runtimeConfigDir: Path.config,
    defaultConfigDir: Path.defaultConfigPath,
    getMigrationReport: () => ConfigControl.getMigrationReport?.() || null,
  });

  function buildConfigDiagnosticsPayload() {
    return {
      success: true,
      data: configSourceDiagnostics.build(getAllConfigs()),
    };
  }
