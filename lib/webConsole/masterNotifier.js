// 主人通知通道：Bot 未就绪（如刚重启、适配器未连接）时自动排队重试，超时放弃。
// 同一消息向多个主人发送时按"全部送达"判定，部分失败会整体重发（未送达方收到，已送达方可能收到重复）。
// persist 类消息（看门狗告警等）不受超时限制：Bot 长时间不在线也无限期保留，恢复瞬间送达。
const RETRY_INTERVAL_MS = 15 * 1000;
const RETRY_DEADLINE_MS = 10 * 60 * 1000;

export function createMasterNotifier({ logger = console, loadMasterIds: customLoadMasterIds } = {}) {
  let masterIds = null;
  const queue = [];
  let timer = null;
  let startedAt = 0;
  let flushing = false;

  async function loadMasterIds() {
    if (masterIds) return masterIds;
    if (typeof customLoadMasterIds === 'function') {
      masterIds = await customLoadMasterIds();
      return masterIds;
    }
    try {
      const mod = await import('../../../../lib/config/config.js');
      const cfg = mod?.default || mod?.cfg || null;
      masterIds = Array.isArray(cfg?.masterQQ)
        ? cfg.masterQQ.map(item => String(item ?? '').trim()).filter(Boolean)
        : [];
    } catch {
      masterIds = [];
    }
    return masterIds;
  }

  // masterQQ 常含 Yunzai 内置的 "stdin" 控制台伪账号，pickUser 对它必然失败；
  // 若不过滤，"全部送达才出队"的判定会因它永远不成立，导致通知死循环重发。
  function getEmailableMasterIds(ids = []) {
    return ids.filter(id => /^[1-9]\d{4,11}$/.test(id));
  }

  async function sendToMasters(message, label) {
    const bot = typeof globalThis !== 'undefined' ? globalThis.Bot : null;
    if (!bot?.pickUser) return { sent: 0, total: 0 };
    const ids = getEmailableMasterIds(await loadMasterIds());
    if (!ids.length) return { sent: 0, total: 0, noMasters: true };
    let sent = 0;
    for (const id of ids) {
      try {
        await bot.pickUser(id).sendMsg(message);
        sent += 1;
      } catch (error) {
        logger.warn?.(`[webConsole] ${label}发送失败 ${id}: ${error.message}`);
      }
    }
    return { sent, total: ids.length };
  }

  function stopTimer() {
    if (timer) {
      clearInterval(timer);
      timer = null;
    }
  }

  async function flush() {
    if (flushing || queue.length === 0) return;
    flushing = true;
    try {
      if (Date.now() - startedAt > RETRY_DEADLINE_MS) {
        const persistItems = queue.filter(item => item.persist);
        for (const item of queue) {
          if (item.persist) continue;
          logger.warn?.(`[webConsole] ${item.label}重试超时（10 分钟），放弃发送`);
        }
        queue.length = 0;
        for (const item of persistItems) queue.push(item);
        if (queue.length === 0) {
          stopTimer();
        } else {
          startedAt = Date.now();
        }
        if (persistItems.length > 0) {
          logger.warn?.(`[webConsole] ${persistItems.length} 条持久通知保留队列（Bot 恢复后送达）`);
        }
        return;
      }
      for (let index = queue.length - 1; index >= 0; index--) {
        const item = queue[index];
        const { sent, total, noMasters } = await sendToMasters(item.message, item.label);
        if (noMasters) {
          logger.warn?.(`[webConsole] ${item.label}未发送：未配置 masterQQ`);
          queue.splice(index, 1);
          continue;
        }
        // 只要有一个主人收到即视为送达出队（重要通知宁可重复也不想丢失）；
        // 持续整轮失败的才留在队列里等下一次重试。
        if (total > 0 && sent > 0) {
          if (sent < total) {
            logger.warn?.(`[webConsole] ${item.label}部分主人未收到（${sent}/${total}），不再重试`);
          }
          queue.splice(index, 1);
        }
      }
      if (queue.length === 0) {
        stopTimer();
      }
    } finally {
      flushing = false;
    }
  }

  return {
    async notifyMasters(message, { label = '通知', persist = false } = {}) {
      const item = { message, label, persist };
      queue.push(item);
      if (!timer) {
        startedAt = Date.now();
        timer = setInterval(() => {
          flush().catch(() => {});
        }, RETRY_INTERVAL_MS);
      }
      await flush();
      if (queue.includes(item)) {
        logger.warn?.(`[webConsole] ${label}暂未送达（Bot 未就绪或发送失败），${persist ? '将无限期保留直至 Bot 恢复' : '将每 15 秒自动重试'}`);
        return 'queued';
      }
      return 'sent';
    },
  };
}
