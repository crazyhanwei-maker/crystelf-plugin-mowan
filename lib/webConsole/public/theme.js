...
function getCurrentResourceVersion() {
  try {
    const currentScript = document.currentScript || Array.from(document.scripts || []).find(script => String(script.src || '').includes('/theme.js'));
    return new URL(currentScript?.src || window.location.href, window.location.href).searchParams.get('v') || '';
  } catch {
    return '';
  }
}
...
(() => {
  if (
    ...
    || document.body?.classList.contains('file-manager-page')
    || document.body?.classList.contains('qq-simulator-page')
  ) {
    return;
  }
  const version = getCurrentResourceVersion();
  if (!document.querySelector('link[data-console-modern-style]')) {
    const style = document.createElement('link');
    style.rel = 'stylesheet';
    style.href = version ? `/console-modern.css?v=${encodeURIComponent(version)}` : '/console-modern.css';
    style.dataset.consoleModernStyle = '1';
  ...
  script.src = version ? `/console-shell.js?v=${encodeURIComponent(version)}` : '/console-shell.js';
  script.defer = true;
