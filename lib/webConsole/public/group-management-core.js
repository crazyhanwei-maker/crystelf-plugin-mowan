...
const GROUP_MANAGEMENT_COLLAPSE_STORAGE_KEY = 'crystelf.groupManagement.collapsed.v2';
const GROUP_MANAGEMENT_WORKSPACE_STORAGE_KEY = 'crystelf.groupManagement.workspace.v1';
...
function isGroupManagementMobileLayout() {
  return window.matchMedia?.('(max-width: 980px)')?.matches === true;
}

function normalizeGroupManagementWorkspace(value = '') {
  const workspace = String(value || '').trim();
  return ['current', 'settings', 'operations'].includes(workspace) ? workspace : 'current';
}

function readGroupManagementWorkspace() {
  try {
    return normalizeGroupManagementWorkspace(localStorage.getItem(GROUP_MANAGEMENT_WORKSPACE_STORAGE_KEY));
  } catch {
    return 'current';
  }
}

function setGroupManagementWorkspace(value = 'current', options = {}) {
  const workspace = normalizeGroupManagementWorkspace(value);
  const hints = {
    current: '选择群聊并调整该群的功能与风控规则。',
    settings: '管理所有群共用的安全边界、默认规则和批量开关。',
    operations: '查看健康检查、实时事件和群管理日志。',
  };
  groupManagementState.activeWorkspace = workspace;
  document.querySelectorAll('[data-workspace-panel]').forEach((element) => {
    const visible = element.dataset.workspacePanel === workspace;
    element.classList.toggle('group-management-workspace-hidden', !visible);
  ...
  const hint = $('group-management-workspace-hint');
  if (hint) hint.textContent = hints[workspace];
  try {
    localStorage.setItem(GROUP_MANAGEMENT_WORKSPACE_STORAGE_KEY, workspace);
  } catch {
    // Ignore storage failures in private mode.
  }
  if (options.scroll === true) {
    $('group-management-workspace-nav')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
  }
}

function readGroupManagementCollapseState() {
  try {
    const data = JSON.parse(localStorage.getItem(GROUP_MANAGEMENT_COLLAPSE_STORAGE_KEY) || '{}');
...
function writeGroupManagementCollapseState(state = {}) {
  try {
    localStorage.setItem(GROUP_MANAGEMENT_COLLAPSE_STORAGE_KEY, JSON.stringify(state || {}));
  } catch {
    // Ignore storage failures in private mode.
  }
}

function getGroupManagementDefaultCollapsed(key = '') {
  const value = String(key || '');
  if (/^(nested|members):/.test(value)) return true;
  if (!isGroupManagementMobileLayout()) return false;
  return /^(static|detail):/.test(value);
}

function isGroupManagementCollapsed(key = '', fallbackElement = null) {
  const state = readGroupManagementCollapseState();
  if (Object.prototype.hasOwnProperty.call(state, key)) {
    return state[key] === true;
  }
  return getGroupManagementDefaultCollapsed(key, fallbackElement);
}

function setGroupManagementCollapsed(key = '', collapsed = false) {
  const state = readGroupManagementCollapseState();
  state[key] = collapsed === true;
  writeGroupManagementCollapseState(state);
}

function getGroupManagementCollapseLabel(element = null) {
  const heading = element?.querySelector?.(':scope > .section-head h2, :scope > .group-management-fold-head h3, :scope > .group-management-fold-head h4, :scope > .group-management-block-head h4, :scope > .group-management-members-head h3, :scope > h3, :scope > h4');
  return String(heading?.textContent || '模块').trim().replace(/\s+/g, ' ');
}

function normalizeGroupManagementCollapseKeyPart(value = '') {
  return String(value || '')
    .trim()
    .replace(/\s+/g, '-')
    .replace(/[^\w\u4e00-\u9fa5-]+/g, '')
...
function updateGroupManagementCollapseButton(element = null) {
  if (!element) return;
  const button = element.querySelector(':scope .group-management-collapse-toggle');
  if (!button) return;
  const collapsed = element.classList.contains('is-collapsed');
  button.textContent = collapsed ? '展开' : '收起';
  button.setAttribute('aria-expanded', collapsed ? 'false' : 'true');
  button.title = collapsed ? '展开这个群管理模块' : '收起这个群管理模块';
}

function applyGroupManagementCollapseState(element = null, collapsed = false) {
  if (!element) return;
  element.classList.toggle('is-collapsed', collapsed === true);
  updateGroupManagementCollapseButton(element);
}

function ensureGroupManagementCollapseButton(header = null, element = null) {
  if (!header || !element) return null;
  let actions = header.querySelector(':scope > .actions');
  if (!actions && header.classList.contains('section-head')) {
    actions = document.createElement('div');
    actions.className = 'actions';
    header.appendChild(actions);
  }
  const container = actions || header;
  let button = container.querySelector(':scope > .group-management-collapse-toggle');
  if (!button) {
    button = document.createElement('button');
    button.type = 'button';
    button.className = 'mini-btn group-management-collapse-toggle';
    button.dataset.action = 'toggle-collapse';
    button.setAttribute('aria-label', '展开或收起模块');
    container.appendChild(button);
  }
  button.dataset.collapseKey = element.dataset.gmCollapseKey || '';
  return button;
}

function createGroupManagementFoldHead(element = null) {
  if (!element) return null;
  const existing = element.querySelector(':scope > .group-management-fold-head, :scope > .group-management-block-head, :scope > .group-management-members-head');
  if (existing) return existing;
  const heading = Array.from(element.children).find(child => child?.matches?.('h3,h4'));
  if (!heading) return null;
  const head = document.createElement('div');
  head.className = 'group-management-fold-head';
  element.insertBefore(head, heading);
  head.appendChild(heading);
  return head;
}

function prepareGroupManagementCollapsible(element = null, options = {}) {
  if (!element || element.dataset.gmCollapsibleReady === '1') return;
  const label = options.label || getGroupManagementCollapseLabel(element);
  const key = options.key || `${options.prefix || 'section'}:${normalizeGroupManagementCollapseKeyPart(label)}`;
  element.dataset.gmCollapseKey = key;
  element.dataset.gmCollapsibleReady = '1';
  element.classList.add('group-management-collapsible');
  if (options.block === true) {
    element.classList.add('group-management-collapsible-block');
  }
  const header = options.header || element.querySelector(':scope > .section-head') || createGroupManagementFoldHead(element);
  if (!header) return;
  header.classList.add('group-management-collapse-head');
  ensureGroupManagementCollapseButton(header, element);
  applyGroupManagementCollapseState(element, isGroupManagementCollapsed(key, element));
}

function enhanceGroupManagementStaticCollapses() {
  [
    ['.group-management-event-panel', 'static:events'],
    ['.group-management-health-panel', 'static:health'],
    ['.group-management-safety-panel', 'static:safety'],
    ['.group-management-default-panel', 'static:defaults'],
    ['.group-management-bulk-panel', 'static:bulk'],
    ['.group-management-log-panel', 'static:logs'],
  ].forEach(([selector, key]) => {
    const element = document.querySelector(selector);
    if (element) prepareGroupManagementCollapsible(element, { key });
  });
}

function enhanceGroupManagementDetailCollapses() {
  const detail = $('group-management-detail');
  if (!detail) return;
  detail.querySelectorAll(':scope .group-management-control-block').forEach((element, index) => {
    const label = getGroupManagementCollapseLabel(element);
    prepareGroupManagementCollapsible(element, {
      key: `detail:${normalizeGroupManagementCollapseKeyPart(label)}:${index}`,
      block: true,
    });
  });
  detail.querySelectorAll(':scope .group-management-nested-block').forEach((element, index) => {
    const label = getGroupManagementCollapseLabel(element);
    prepareGroupManagementCollapsible(element, {
      key: `nested:${normalizeGroupManagementCollapseKeyPart(label)}:${index}`,
      block: true,
    });
  });
  detail.querySelectorAll(':scope .group-management-members-panel').forEach((element, index) => {
    const label = getGroupManagementCollapseLabel(element);
    prepareGroupManagementCollapsible(element, {
      key: `members:${normalizeGroupManagementCollapseKeyPart(label)}:${index}`,
      block: true,
    });
  });
}

function toggleGroupManagementCollapse(button = null) {
  const key = String(button?.dataset?.collapseKey || '').trim();
  const element = key
    ? Array.from(document.querySelectorAll('.group-management-collapsible')).find(item => item.dataset.gmCollapseKey === key)
    : button?.closest?.('.group-management-collapsible');
  if (!element) return;
  const collapsed = !element.classList.contains('is-collapsed');
  setGroupManagementCollapsed(element.dataset.gmCollapseKey || key, collapsed);
  applyGroupManagementCollapseState(element, collapsed);
}
