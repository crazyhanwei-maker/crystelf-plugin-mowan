...
const GROUP_RECOMMENDATIONS = {
  ...
  '访问与安全': '建议开启鉴权；如果只是自己本机调试，可以再决定要不要关闭只读模式。',
...
const FIELD_RECOMMENDATIONS = {
  ...
  'config.webConsoleRequireAuth': '推荐：开启',
...
const FIELD_RECOMMENDATION_VALUES = {
  ...
  'config.webConsoleRequireAuth': true,
...
function renderFieldNotice(item) {
  const tone = getFieldTone(item);
  if (tone === 'danger') {
    return '<div class="plugin-settings-field-notice plugin-settings-field-notice-danger">该字段包含访问令牌或敏感凭证，建议仅在可信环境下修改，并避免直接展示给其他人。</div>';
  }
  if (tone === 'recommended') {
...
function renderConsoleSection() {
  ...
  const sections = [
    runtimeSection,
    renderConsoleFieldSection('访问与安全', '控制台是否启用、是否鉴权、是否只读，以及访问令牌等安全项。', [
      'config.webConsole',
      'config.webConsoleRequireAuth',
  ...
  document.getElementById('plugin-settings-console-meta').innerHTML = renderPanelMetaChips([
    ...
    { label: runtime.webConsoleRequireAuth ? '鉴权：已开启' : '鉴权：未开启', tone: runtime.webConsoleRequireAuth ? 'success' : 'warning' },
...
async function refreshPluginSettings() {
  const [payload, overview, editableConfig] = await Promise.all([
...
async function savePluginSettings() {
  ...
  pluginSettingsState.editableConfig = (await fetchJson('/api/config/editable'));
  syncDraftFromPayload();
  renderCategoryTabs();
  renderGroupTabs();
  renderForm();
  renderFeatureSection();
  renderConsoleSection();
  document.getElementById('plugin-settings-summary').textContent = '保存成功，部分配置可能需要重启后完全生效';
}
...
document.getElementById('plugin-settings-save-btn').addEventListener('click', () => {
  savePluginSettings().catch(error => {
    document.getElementById('plugin-settings-risk').textContent = `保存失败：${error.message}`;
    document.getElementById('plugin-settings-risk').classList.remove('hidden');
  });
});

refreshPluginSettings().catch(error => {
  document.getElementById('plugin-settings-risk').textContent = `加载失败：${error.message}`;
  document.getElementById('plugin-settings-risk').classList.remove('hidden');
});
