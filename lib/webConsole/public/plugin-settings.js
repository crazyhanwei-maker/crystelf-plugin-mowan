...
async function refreshPluginSettings() {
  ...
  const [payload, overview, editableConfig, skillsEditorResponse, privateAiSafetyResponse, personaPresetsResponse] = await Promise.all([
    ...
    fetchJson('/api/persona-presets').catch(() => null),
  ]);
  ...
  pluginSettingsState.personaPresets = Array.isArray(personaPresetsResponse?.data?.presets)
    ? personaPresetsResponse.data.presets
    : [];
