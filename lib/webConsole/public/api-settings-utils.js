...
function normalizeViolationAction(value, legacyAutoRecall) {
  const raw = String(value || '').trim().toLowerCase();
  if (raw === 'recall' || raw === 'warn' || raw === 'ignore') return raw;
  // 兼容旧字段 autoRecallViolation（true → recall，false → warn）
  if (legacyAutoRecall === true) return 'recall';
  if (legacyAutoRecall === false) return 'warn';
  return 'recall';
}

function readNumberInputBounded(id, fallback, min, max) {
  const input = $(id);
  if (!(input instanceof HTMLInputElement)) return fallback;
...
function normalizeFallbackFailureThreshold(value, fallback = 2) {
  const num = Number(value);
  if (!Number.isFinite(num)) return fallback;
  return Math.min(10, Math.max(1, Math.round(num)));
}

function normalizeFallbackCooldownMs(value, fallback = 300000) {
  const num = Number(value);
  if (!Number.isFinite(num)) return fallback;
  return Math.min(1800000, Math.max(30000, Math.round(num)));
}
