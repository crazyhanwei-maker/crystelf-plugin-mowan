...
function buildGroupSaveDiffLines(group = {}, payload = {}) {
  ...
  addGroupSaveDiffLine(diffs, '伪人模式', config.ai?.pseudoHuman?.enabled === true, payload.ai?.pseudoHuman?.enabled === true);
  addGroupSaveDiffLine(diffs, '伪人触发概率', Number(config.ai?.pseudoHuman?.probability ?? 10), Number(payload.ai?.pseudoHuman?.probability ?? 10));
...
async function buildSavePayload() {
  ...
  return {
    ...
    ai: {
      ...
      pseudoHuman: {
        enabled: $('gm-pseudo-human-enabled')?.checked === true,
        probability: readNumber('gm-pseudo-human-probability', 10, 0, 100),
      },
    },
    imageMonitor: {
