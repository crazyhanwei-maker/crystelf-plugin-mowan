(function () {
  function createScenarioHandlers(deps = {}) {
    ...
    function normalizeScenarioPayload(payload = {}) {
      ...
      return {
        eventType: ['message', 'private_message', 'join_request', 'group_increase', 'poke'].includes(source.eventType) ? source.eventType : 'message',
        ...
        dispatchMode: ['replay', 'live-image'].includes(source.dispatchMode) ? source.dispatchMode : 'safe',
    ...
    function buildScenarioName(payload = {}) {
      ...
      return `${eventLabel} · ${payload.eventType === 'private_message' ? '私聊' : `群${payload.groupId || '未填'}`} · ${brief}`;
    }
    ...
    function describeScenarioPayload(payload = {}) {
      ...
      const parts = [
        `${eventLabel}`,
        payload.eventType === 'private_message' ? '私聊' : `群 ${payload.groupId || '未填写'}`,
        ...
        payload.eventType === 'private_message' ? '私聊直达' : (payload.includeAt ? '@机器人' : '未@机器人'),
