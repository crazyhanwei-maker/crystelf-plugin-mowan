...
function renderSummary(payload) {
  ...
  const cards = [
    ...
    ['带 package.json', summary.packageCount, '可读取 package.json 的插件数量'],
...
function renderPluginList() {
  ...
  target.innerHTML = plugins.length > 0
    ...
    : '<div class="list-item">没有匹配的插件。</div>';
  renderDetail();
}
...
async function refreshBotPlugins() {
  ...
  setStatus('正在扫描 Bot 插件目录...', 'neutral');
  try {
    ...
    setMeta('Bot 插件管理加载失败');
    setStatus(`Bot 插件管理加载失败：${error.message}`, 'error');
  } finally {
