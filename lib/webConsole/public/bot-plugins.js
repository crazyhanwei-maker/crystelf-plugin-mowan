...
const botPluginsState = {
  ...
  initialPluginQuery: getInitialPluginQuery(),
};
...
function getInitialPluginQuery() {
  try {
    const params = new URLSearchParams(window.location.search);
    return String(params.get('plugin') || '').trim();
  } catch {
    return '';
  }
}

function applyInitialPluginSearch() {
  const query = botPluginsState.initialPluginQuery;
  if (!query) return;
  const input = $('bot-plugins-search');
...
function isInitialPluginMatch(plugin = {}, query = '') {
...
function renderPluginList() {
  ...
  if (botPluginsState.initialPluginQuery) {
    const matched = plugins.find(plugin => isInitialPluginMatch(plugin, botPluginsState.initialPluginQuery));
    if (matched?.id) botPluginsState.selectedId = matched.id;
    botPluginsState.initialPluginQuery = '';
  }
...
function renderDetail() {
  ...
  target.innerHTML = `
    <div class="bot-plugin-detail">
      ...
      <div class="setting-help">${canDelete ? '删除会先把插件目录移入 plugins/.crystelf-plugin-trash 暂存，不会直接物理删除。' : '当前插件或受保护目录不允许删除。'}</div>
