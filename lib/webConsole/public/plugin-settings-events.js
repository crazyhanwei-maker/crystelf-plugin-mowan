...
document.addEventListener('click', async event => {
  ...
  if (target.dataset.personaPresetSave) {
    saveCurrentAsPersonaPreset().catch(error => setPersonaPresetStatus(`保存预设失败：${error?.message || error}`));
    return;
  }
  if (target.dataset.personaPresetApply) {
    applyPersonaPresetToDraft().catch(error => setPersonaPresetStatus(`恢复预设失败：${error?.message || error}`));
    return;
  }
  if (target.dataset.personaPresetDelete) {
    deletePersonaPresetByName().catch(error => setPersonaPresetStatus(`删除预设失败：${error?.message || error}`));
    return;
  }
  ...
  if (target.dataset.settingsGroup || target.closest?.('[data-settings-group]')) {
    // 分组按钮内层有 <span>（名称/计数），点在文字上时 target 是 span——用 closest 向上找按钮
    const groupBtn = target.closest('[data-settings-group]');
    pluginSettingsState.activeGroup = groupBtn.dataset.settingsGroup;
    pluginSettingsState.recommendationStatus = '';
    pluginSettingsState.recommendationUndo = null;
    renderGroupTabs();
    renderForm();
...
document.addEventListener('change', event => {
  const target = event.target;
  if (target instanceof HTMLSelectElement && target.dataset.personaPresetSelect !== undefined) {
    pluginSettingsState.personaPresetSelected = target.value || '';
    return;
  }
