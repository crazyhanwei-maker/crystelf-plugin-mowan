...
function buildImageMonitorDisplayUrls(value) {
  if (typeof value === 'string') {
    const originalImageUrl = value.trim();
    return {
      previewUrl: originalImageUrl ? `/api/image-proxy?url=${encodeURIComponent(originalImageUrl)}` : '',
      openImageUrl: originalImageUrl,
      originalImageUrl,
    };
  }
  const data = value || {};
  ...
  const previewUrl = String(data.previewUrl || '').trim()
    || (originalImageUrl ? `/api/image-proxy?url=${encodeURIComponent(originalImageUrl)}` : '');
  return {
    previewUrl,
    openImageUrl: String(data.openImageUrl || '').trim() || originalImageUrl,
    originalImageUrl,
  };
}
...
document.getElementById('cleanup-non-meme-btn')?.addEventListener('click', async () => {
  const confirmed = window.confirm('确认清理审核日志中的非表情包图片记录？会删除 isMeme=false 的审核记录，并清理同 hash 的非表情包本地入库文件。');
  if (!confirmed) return;
  try {
...
document.getElementById('cleanup-unmatched-meme-btn')?.addEventListener('click', async () => {
  const confirmed = window.confirm('确认清理未命中当前入库角色/关键词白名单的表情包？会删除对应入库记录、本地图片文件，并清理空目录。请先确认白名单配置正确。');
  if (!confirmed) return;
  try {
