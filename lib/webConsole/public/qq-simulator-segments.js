(function () {
  ...
  function buildAdapterMessagePreview(payload = {}) {
    ...
    const isMessageEvent = payload.eventType === 'message' || payload.eventType === 'private_message';
    const isPrivateMessageEvent = payload.eventType === 'private_message';
    const message = isMessageEvent
      ? adapterFormat === 'icqq'
        ? buildIcqqPreviewMessage(payload)
        : buildOnebotPreviewMessage(payload)
      : [];
    const plainMessage = isMessageEvent ? buildPreviewRawMessage(payload) : String(payload.comment || '');
    const cqMessage = isMessageEvent ? buildPreviewCqMessage(payload) : String(payload.comment || '');
    return {
      adapterFormat,
      ...
      event: {
        adapterFormat,
        eventType: payload.eventType,
        post_type: isMessageEvent
          ? 'message'
          : payload.eventType === 'join_request'
            ? 'request'
            : 'notice',
        message_type: isPrivateMessageEvent ? 'private' : payload.eventType === 'message' ? 'group' : undefined,
        group_id: isPrivateMessageEvent ? undefined : (payload.groupId || '<group-id>'),
