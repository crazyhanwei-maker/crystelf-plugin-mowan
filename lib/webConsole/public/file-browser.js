...
function getCurrentTarget() {
  if (fileBrowserState.selectedFile?.path) {
    return {
      ...
      absolutePath: fileBrowserState.selectedFile.absolutePath,
  ...
  if (fileBrowserState.currentDirectoryInfo) {
    return {
      ...
      absolutePath: fileBrowserState.currentDirectoryInfo.absolutePath,
...
function renderSummary() {
  $('file-browser-root-path').textContent = fileBrowserState.root?.absolutePath || '-';
...
function renderSelectedFile() {
  ...
  if (fileBrowserState.selectedFile) {
    ...
    meta.textContent = `${formatBytes(file.size)} · ${formatTime(file.mtimeMs)} · ${file.absolutePath || '-'}`;
    if (editor) {
  ...
  if (fileBrowserState.currentDirectoryInfo) {
    title.textContent = fileBrowserState.currentDirectoryInfo.path
      ...
      : '[目录] /';
    meta.textContent = fileBrowserState.currentDirectoryInfo.absolutePath || '-';
    if (editor) {
      editor.value = '';
    }
    updateFileStats();
