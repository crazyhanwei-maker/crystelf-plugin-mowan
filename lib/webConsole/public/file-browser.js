...
function bindEvents() {
  ...
  $('file-browser-upload-btn')?.addEventListener('click', () => {
    if (fileBrowserState.auth?.readOnly) return;
    $('file-browser-upload-input')?.click();
  });

  $('file-browser-upload-input')?.addEventListener('change', event => {
    const input = event.target;
    const files = input?.files ? Array.from(input.files) : [];
    if (files.length) {
      uploadFilesToCurrentDirectory(files);
    }
    input.value = '';
  });
  ...
  $('file-browser-root-btn')?.addEventListener('click', async () => {
    if (!await confirmDiscardIfNeeded()) return;
    await loadDirectory('');
  });

  $('file-browser-up-btn')?.addEventListener('click', async () => {
    if (!await confirmDiscardIfNeeded()) return;
    const current = fileBrowserState.currentDir || '';
    if (!current) return; // 已在根目录
