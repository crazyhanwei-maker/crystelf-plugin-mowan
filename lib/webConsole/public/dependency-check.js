...
function getInstallTaskFailureAdvice(task = {}) {
  ...
  if (/err_pnpm_included_deps_conflict|included_deps_conflict|includeddeps|included deps conflict/.test(context)) {
    pushAdvice(
      'pnpm 安装范围冲突',
      '这是旧版本修复命令带 --prod 导致的 pnpm node_modules 范围冲突。刷新页面后重新修复，会改用 pnpm add 重新执行。',
    );
  }
...
function getInstallTaskLogText(task = {}) {
  ...
  const metaLines = [
    ...
    task.progressLabel ? `进度：${normalizeProgressPercent(task.progressPercent)}% ${task.progressLabel}` : '',
  ...
  const outputLines = (Array.isArray(task.outputEvents) ? task.outputEvents : []).map(event => [
    `[${formatTime(event.time)}]`,
    `[${event.stream || 'stdout'}]`,
    event.stage ? `[${event.stage}]` : '',
    event.text || '',
...
function buildInstallTaskDiagnostic(task = {}) {
  return {
    ...
    outputEvents: Array.isArray(task.outputEvents) ? task.outputEvents : [],
