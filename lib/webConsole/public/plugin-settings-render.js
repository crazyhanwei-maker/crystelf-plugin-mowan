...
function renderPersonaPresetPanel() {
  if (pluginSettingsState.activeCategory !== 'ai' || pluginSettingsState.activeGroup !== '机器人身份与人设') {
    return '';
  }
  const presets = Array.isArray(pluginSettingsState.personaPresets) ? pluginSettingsState.personaPresets : [];
  const selectedName = String(pluginSettingsState.personaPresetSelected || '');
  const optionsHtml = presets.length > 0
    ? presets.map(preset => {
        const savedAt = preset.savedAt ? `（${new Date(preset.savedAt).toLocaleString('zh-CN', { hour12: false })}）` : '';
        const selected = preset.name === selectedName ? ' selected' : '';
        return `<option value="${escapeHtml(preset.name)}"${selected}>${escapeHtml(preset.name)}${savedAt}</option>`;
      }).join('')
    : '<option value="">暂无预设</option>';
  return `
    <section class="plugin-settings-special-panel plugin-settings-persona-preset-panel">
      <div class="plugin-settings-special-panel-head">
        <div>
          <h3>人设预设</h3>
          <div class="setting-status">把昵称、人设描述、人格状态和回复风格保存为预设；恢复时填入下方表单，检查后点保存设置生效。</div>
        </div>
      </div>
      <div class="plugin-settings-persona-preset-body">
        <div class="plugin-settings-persona-preset-row">
          <select id="persona-preset-select" data-persona-preset-select ${presets.length === 0 ? 'disabled' : ''}>
            ${optionsHtml}
          </select>
          <button type="button" class="feature-manage-btn feature-manage-btn-secondary" data-persona-preset-apply="1" ${presets.length === 0 ? 'disabled' : ''}>恢复到表单</button>
          <button type="button" class="feature-manage-btn feature-manage-btn-danger" data-persona-preset-delete="1" ${presets.length === 0 ? 'disabled' : ''}>删除</button>
        </div>
        <div class="plugin-settings-persona-preset-row">
          <input id="persona-preset-name-input" type="text" maxlength="24" placeholder="新预设名称（1-24 字）" data-persona-preset-name />
          <button type="button" class="feature-manage-btn" data-persona-preset-save="1">保存当前表单为预设</button>
        </div>
        <div class="setting-help" data-persona-preset-status>${escapeHtml(pluginSettingsState.personaPresetStatus || '预设保存在运行配置目录 persona-presets.json，上限 50 个，同名保存会覆盖。')}</div>
      </div>
    </section>
  `;
}
...
function renderForm() {
  ...
  ` : '';
  const personaPresetPanel = renderPersonaPresetPanel();
  document.getElementById('plugin-settings-form').innerHTML = (knowledgePanel || '') + (personaPresetPanel || '') + (normalItems.length > 0
  ...
  document.getElementById('plugin-settings-private-safety-panel')?.classList.toggle('hidden', pluginSettingsState.activeGroup !== '私聊 AI 安全');
}
