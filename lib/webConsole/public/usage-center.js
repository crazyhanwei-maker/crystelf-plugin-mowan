...
function renderFreeChatUsage(data = {}) {
  const target = document.getElementById('free-chat-usage');
  if (!target) return;
  if (data.success !== true) {
    target.innerHTML = renderEmptyState('FREE CHAT 用量读取失败', data.error || '接口暂时不可用');
    return;
  }
  ...
  target.innerHTML = [
    '<div class="config-summary-grid">',
    buildSummaryCard('Token 配额', [
      { label: '今日已用', value: `${formatNumber(tokens.used || 0)} / ${formatNumber(tokens.limit || 0)}` },
      { label: '剩余 Token', value: formatNumber(tokens.remaining || 0) },
      { label: '使用率', value: `${Number(tokens.percent || 0).toFixed(2)}%` },
    ]),
    buildSummaryCard('请求配额', [
      { label: '今日请求', value: `${formatNumber(requests.count || 0)} / ${formatNumber(requests.limit || 0)}` },
      { label: '成功 / 失败', value: `${formatNumber(requests.success || 0)} / ${formatNumber(requests.failed || 0)}` },
      { label: '剩余请求', value: formatNumber(requests.remaining || 0) },
      { label: '使用率', value: `${Number(requests.percent || 0).toFixed(2)}%` },
    ]),
    buildSummaryCard('统计周期', [
      { label: '统计日期', value: escapeHtml(data.period?.date || '暂无') },
      { label: '额度重置', value: escapeHtml(formatTime(data.period?.resetAt)) },
      { label: '额度状态', value: tokens.exceeded || requests.exceeded ? '已超额' : '正常' },
      { label: '数据时间', value: escapeHtml(formatTime(data.generatedAt || data.checkedAt)) },
    ]),
    '</div>',
  ].join('');
}
...
const usageDaysState = { days: 7, loading: false };

async function loadUsageDaysTrend(days = usageDaysState.days) {
  const target = document.getElementById('usage-days-chart');
  if (!target || usageDaysState.loading) return;
  usageDaysState.loading = true;
  document.querySelectorAll('.usage-days-toggle').forEach(btn => {
    btn.classList.toggle('active', Number(btn.dataset.days) === Number(days));
    btn.disabled = true;
  });
  try {
    target.textContent = '正在加载按天趋势数据...';
    const data = await fetchJson(`/api/trend/usage-days?days=${Number(days) || 7}`);
    const items = Array.isArray(data?.items) ? data.items : [];
    if (items.length === 0 || items.every(it => !it.requests && !it.total_tokens)) {
      target.textContent = '近 ' + days + ' 天暂无 AI 用量数据';
      return;
    }
    target.innerHTML = renderUsageDaysChart(items);
  } catch (error) {
    target.textContent = '按天趋势加载失败：' + (error.message || '未知错误');
  } finally {
    usageDaysState.loading = false;
    document.querySelectorAll('.usage-days-toggle').forEach(btn => {
      btn.disabled = false;
    });
  }
...
function renderUsageDaysChart(items) {
  const W = 1000;
  const H = 280;
  const padL = 56;
  const padR = 60;
  ...
  const barW = Math.max(6, Math.min(26, step * 0.55));
  const reqMax = Math.max(...items.map(it => Number(it.requests || 0)), 1);
  const tokenMax = Math.max(...items.map(it => Number(it.total_tokens || 0)), 1);
  const fmt = v => v >= 1e6 ? (v / 1e6).toFixed(1) + 'M' : v >= 1e3 ? (v / 1e3).toFixed(1) + 'k' : String(v);
  const xAt = i => padL + step * i + step / 2;
  const yReq = v => padT + innerH - (v / reqMax) * innerH;
  const yTok = v => padT + innerH - (v / tokenMax) * innerH;

  const bars = items.map((it, i) => {
    const requests = Number(it.requests || 0);
    const tokens = Number(it.total_tokens || 0);
    const errors = Number(it.errors || 0);
    const tipDate = it.date || '';
    const titleText = `${tipDate}  请求 ${requests}  Tokens ${tokens}${errors ? `  失败 ${errors}` : ''}`;
    const reqH = requests > 0 ? Math.max(2, padT + innerH - yReq(requests)) : 0;
    const tokH = tokens > 0 ? Math.max(2, padT + innerH - yTok(tokens)) : 0;
    return `
      <g class="usage-days-group">
        <title>${titleText}</title>
        <rect x="${(xAt(i) - barW / 2).toFixed(1)}" y="${(yReq(requests)).toFixed(1)}" width="${barW.toFixed(1)}" height="${reqH.toFixed(1)}" rx="3" fill="rgba(82, 121, 216, 0.72)"/>
        <rect x="${(xAt(i) + 2).toFixed(1)}" y="${(yTok(tokens)).toFixed(1)}" width="${Math.max(3, barW * 0.3).toFixed(1)}" height="${tokH.toFixed(1)}" rx="2" fill="rgba(36, 157, 178, 0.55)"/>
        ${n <= 14 || i % Math.ceil(n / 14) === 0 ? `<text x="${xAt(i).toFixed(1)}" y="${H - 14}" text-anchor="middle" font-size="10" fill="currentColor" opacity="0.66">${escapeHtml(String(tipDate).slice(5))}</text>` : ''}
      </g>`;
  }).join('');
  ...
  return `
    <div class="usage-trend">
      <div class="usage-trend-legend">
        <span class="usage-trend-legend-item"><span class="usage-trend-legend-dot" style="background:rgba(82,121,216,0.9)"></span>请求数（左轴）</span>
        <span class="usage-trend-legend-item"><span class="usage-trend-legend-dot" style="background:rgba(36,157,178,0.75)"></span>Tokens（右轴）</span>
      </div>
      <svg class="usage-trend-svg" viewBox="0 0 ${W} ${H}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="近 ${n} 天 AI 用量趋势">
        ${[0, 0.25, 0.5, 0.75, 1].map(r => `
          <line x1="${padL}" x2="${W - padR}" y1="${(padT + innerH * r).toFixed(1)}" y2="${(padT + innerH * r).toFixed(1)}" stroke="currentColor" stroke-opacity="0.14" stroke-dasharray="3 4"/>
          <text x="${padL - 8}" y="${(padT + innerH * r + 4).toFixed(1)}" text-anchor="end" font-size="10" fill="currentColor" opacity="0.6">${fmt(Math.round(reqMax * (1 - r)))}</text>
          <text x="${W - padR + 8}" y="${(padT + innerH * r + 4).toFixed(1)}" text-anchor="start" font-size="10" fill="currentColor" opacity="0.6">${fmt(Math.round(tokenMax * (1 - r)))}</text>
        `).join('')}
        ${bars}
      </svg>
      <div class="usage-trend-hint">左轴为请求数，右轴为 Tokens；悬停柱体查看当日请求、Token 与失败数。</div>
    </div>`;
}
...
async function refreshUsageCenter() {
  ...
  state.refreshPromise = (async () => {
    try {
      const [overview, freeChatUsage, trend, logs, auditLogs] = await Promise.all([
        fetchJson('/api/overview', requestOptions),
        fetchJson('/api/usage/free-chat', requestOptions)
          .catch(error => ({ success: false, error: error.message })),
        fetchJson('/api/trend/usage', requestOptions),
        fetchJson(`/api/logs/usage?${params.toString()}`, requestOptions),
        document.getElementById('audit-log-box')
      ...
      renderFreeChatUsage(freeChatUsage);
      ...
      setPageMeta(auditLogs
        ? `AI 日志 ${formatNumber(total)} 条，审计日志 ${formatNumber(auditTotal)} 条`
        : `AI 日志 ${formatNumber(total)} 条`);
      setRefreshStatus(auditLogs
        ? `最近刷新：${formatTime(Date.now())} / ${elapsedMs} ms / AI 日志 ${formatNumber(total)} 条 / 审计 ${formatNumber(auditTotal)} 条`
        : `最近刷新：${formatTime(Date.now())} / ${elapsedMs} ms / AI 日志 ${formatNumber(total)} 条`, 'neutral');
      return { overview, freeChatUsage, trend, logs, auditLogs };
    } catch (error) {
...
document.addEventListener('click', event => {
  ...
  const panelHead = target.closest('.collapsible-panel > .section-head');
  if (panelHead && !target.closest('a, button, input, select')) {
    const panel = panelHead.closest('.collapsible-panel');
    if (panel) {
...
function bindUsageDaysToggles() {
  document.querySelectorAll('.usage-days-toggle').forEach(btn => {
    btn.addEventListener('click', () => {
      const days = Number(btn.dataset.days) || 7;
      if (usageDaysState.days === days) return;
      usageDaysState.days = days;
      loadUsageDaysTrend(days);
    });
  });
  loadUsageDaysTrend(usageDaysState.days);
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', bindUsageDaysToggles);
} else {
  bindUsageDaysToggles();
}
