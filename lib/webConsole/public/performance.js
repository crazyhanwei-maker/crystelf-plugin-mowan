...
async function loadProcessViewerProcesses() {
  ...
  if (tbody) tbody.innerHTML = '<tr><td colspan="4">正在加载...</td></tr>';
  try {
    ...
    if (tbody) tbody.innerHTML = `<tr><td colspan="4">进程列表加载失败：${escapeHtml(error.message)}</td></tr>`;
  }
}
