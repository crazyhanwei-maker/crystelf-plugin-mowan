...
function renderEffectiveConfigSnapshot(group = {}) {
  ...
  const items = [
    ...
    {
      label: '伪人模式',
      value: config.ai?.pseudoHuman?.effectiveEnabled === true
        ? `已开启（${Number(config.ai?.pseudoHuman?.probability || 0)}%）`
        : '未开启',
      tone: config.ai?.pseudoHuman?.effectiveEnabled === true ? 'tone-success' : '',
      meta: '普通聊天最近 10 条消息随机触发',
    },
    {
      label: '每日总结',
