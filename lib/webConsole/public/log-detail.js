...
function renderMissingLogDetail(message, backHref) {
  document.getElementById('detail-meta').textContent = '\u7b49\u5f85\u9009\u62e9\u65e5\u5fd7';
  ...
  document.getElementById('detail-preview').innerHTML = `
    <div class="preview-section">
      <div class="preview-head"><h3>\u8bf7\u9009\u62e9\u8bb0\u5f55</h3></div>
      <div class="detail-box">
        ${escapeHtml(message)}
        <div class="actions">
          <a class="link-btn" href="${escapeHtml(backHref)}">\u8fd4\u56de\u5217\u8868</a>
        </div>
      </div>
    </div>
  `;
  document.getElementById('linked-profile-summary').textContent = '';
  document.getElementById('raw-json-box').textContent = '{}';
  bindCommonActions();
}
...
function renderPreviewSections(items) {
  document.getElementById('detail-preview').innerHTML = items.map(item => `
    <div class="preview-section${item.preserveLineBreaks ? ' preview-section-full' : ''}">
      <div class="preview-head">
        <h3>${escapeHtml(item.label)}</h3>
        <button class="mini-btn" data-copy-value="${escapeHtml(item.value)}">\u590d\u5236</button>
      </div>
      <pre class="preview-box${item.preserveLineBreaks ? ' preview-box-full' : ''}">${escapeHtml(item.value || '\u65e0')}</pre>
    </div>
  `).join('');
}
...
async function initUsageLogDetail() {
  ...
  if (!id) {
    renderMissingLogDetail('\u8bf7\u5148\u4ece AI \u7528\u91cf\u65e5\u5fd7\u4e2d\u5fc3\u6253\u5f00\u67d0\u6761\u65e5\u5fd7\u8be6\u60c5\u3002', '/usage-center.html');
    return;
  }
  const data = await fetchJson(`/api/logs/usage/detail?id=${encodeURIComponent(id)}`);

  document.getElementById('detail-meta').textContent = `${data.scene_label || formatSceneLabel(data.scene) || '\u672a\u77e5\u573a\u666f'} \u00b7 ${formatTime(data.time)}`;

  renderCards([
    { label: '\u8bf7\u6c42\u6570', value: 1 },
    { label: '总用量', value: Number(data.total_tokens || 0) },
  ...
  renderDetailActions([
    data.session_id && data.user_id
      ? { label: '对话排查', href: `/session-debug.html?sessionId=${encodeURIComponent(data.session_id)}&userId=${encodeURIComponent(data.user_id)}` }
      : null,
    data.session_id && data.user_id
  ...
  renderKeyValues([
    ...
    { label: '\u573a\u666f', value: data.scene_label || formatSceneLabel(data.scene) },
    ...
    { label: '输入用量', value: data.prompt_tokens ?? 0 },
    { label: '输出用量', value: data.completion_tokens ?? 0 },
  ]);

  renderPreviewSections([
    ...(!isImageMonitorReviewUsage(data) ? [{ label: '提示词预览', value: data.display_prompt_preview || data.prompt_preview || '\u65e0' }] : []),
    { label: '回复预览', value: data.display_response_preview || data.response_preview || '\u65e0' },
    ...(!isImageMonitorReviewUsage(data) && data.prompt_full ? [{ label: '用户输入全文', value: data.prompt_full, preserveLineBreaks: true }] : []),
    ...(data.response_full ? [{ label: 'AI 回复全文', value: data.response_full, preserveLineBreaks: true }] : []),
...
async function initAffinityLogDetail() {
  ...
  const id = params.get('id') || '';
  if (!id) {
    renderMissingLogDetail('\u8bf7\u5148\u4ece\u597d\u611f\u65e5\u5fd7\u5217\u8868\u6253\u5f00\u67d0\u6761\u65e5\u5fd7\u8be6\u60c5\u3002', '/index.html#affinity-logs');
    return;
  }
