  ...
  return {
    ai: {
      ...
      fallbackApi: {
        ...
        apiProtocol: d['ai.fallbackApi.apiProtocol'],
      ...
      imageConfig: {
        ...
        outputFormat: d['ai.imageConfig.outputFormat'],
        watermark: d['ai.imageConfig.watermark'],
        webSearch: d['ai.imageConfig.webSearch'],
        sdWebUi: {
          baseApi: d['ai.imageConfig.sdWebUi.baseApi'],
          model: d['ai.imageConfig.sdWebUi.model'],
          username: d['ai.imageConfig.sdWebUi.username'],
          password: d['ai.imageConfig.sdWebUi.password'] === SECRET_SENTINEL ? '' : d['ai.imageConfig.sdWebUi.password'],
          preservePassword: d['ai.imageConfig.sdWebUi.password'] === SECRET_SENTINEL && d['ai.imageConfig.sdWebUi.passwordConfigured'] === true,
          samplerName: d['ai.imageConfig.sdWebUi.samplerName'],
          scheduler: d['ai.imageConfig.sdWebUi.scheduler'],
          steps: d['ai.imageConfig.sdWebUi.steps'],
          cfgScale: d['ai.imageConfig.sdWebUi.cfgScale'],
          width: d['ai.imageConfig.sdWebUi.width'],
          height: d['ai.imageConfig.sdWebUi.height'],
          negativePrompt: d['ai.imageConfig.sdWebUi.negativePrompt'],
          seed: d['ai.imageConfig.sdWebUi.seed'],
          denoisingStrength: d['ai.imageConfig.sdWebUi.denoisingStrength'],
        },
        ...
        fallbackApi: {
          ...
          size: d['ai.imageConfig.fallbackApi.size'],
          responseFormat: d['ai.imageConfig.fallbackApi.responseFormat'],
          outputFormat: d['ai.imageConfig.fallbackApi.outputFormat'],
          watermark: d['ai.imageConfig.fallbackApi.watermark'],
          webSearch: d['ai.imageConfig.fallbackApi.webSearch'],
          sdWebUi: {
            baseApi: d['ai.imageConfig.fallbackApi.sdWebUi.baseApi'],
            model: d['ai.imageConfig.fallbackApi.sdWebUi.model'],
            username: d['ai.imageConfig.fallbackApi.sdWebUi.username'],
            password: d['ai.imageConfig.fallbackApi.sdWebUi.password'] === SECRET_SENTINEL ? '' : d['ai.imageConfig.fallbackApi.sdWebUi.password'],
            preservePassword: d['ai.imageConfig.fallbackApi.sdWebUi.password'] === SECRET_SENTINEL && d['ai.imageConfig.fallbackApi.sdWebUi.passwordConfigured'] === true,
            samplerName: d['ai.imageConfig.fallbackApi.sdWebUi.samplerName'],
            scheduler: d['ai.imageConfig.fallbackApi.sdWebUi.scheduler'],
            steps: d['ai.imageConfig.fallbackApi.sdWebUi.steps'],
            cfgScale: d['ai.imageConfig.fallbackApi.sdWebUi.cfgScale'],
            width: d['ai.imageConfig.fallbackApi.sdWebUi.width'],
            height: d['ai.imageConfig.fallbackApi.sdWebUi.height'],
            negativePrompt: d['ai.imageConfig.fallbackApi.sdWebUi.negativePrompt'],
            seed: d['ai.imageConfig.fallbackApi.sdWebUi.seed'],
            denoisingStrength: d['ai.imageConfig.fallbackApi.sdWebUi.denoisingStrength'],
          },
    ...
    imageMonitor: {
      ...
      storageEnabled: d['imageMonitor.storageEnabled'],
...
document.addEventListener('click', event => {
  ...
  const fallbackSwitchMap = {
    ...
    'image-watermark': 'ai.imageConfig.watermark',
    'image-webSearch': 'ai.imageConfig.webSearch',
    'image-fallback-watermark': 'ai.imageConfig.fallbackApi.watermark',
    'image-fallback-webSearch': 'ai.imageConfig.fallbackApi.webSearch',
  ...
  if (target.id === 'imageMonitor-storageEnabled') {
    apiSettingsState.draft['imageMonitor.storageEnabled'] = !apiSettingsState.draft['imageMonitor.storageEnabled'];
    updateSwitchButton('imageMonitor-storageEnabled', apiSettingsState.draft['imageMonitor.storageEnabled']);
...
document.addEventListener('input', event => {
  ...
  if (target.tagName === 'INPUT' || target.tagName === 'SELECT' || target.tagName === 'TEXTAREA') {
    if (target.id === 'image-imageMode' && target.value === 'ark-agent-plan') {
      if (!$('image-baseApi').value.trim()) $('image-baseApi').value = 'https://ark.cn-beijing.volces.com/api/plan/v3';
      if (!$('image-model').value.trim() || $('image-model').value === 'gpt-image-2') $('image-model').value = 'doubao-seedream-5.0-lite';
      $('image-size').value = '2K';
      $('image-responseFormat').value = 'url';
      $('image-outputFormat').value = 'png';
      apiSettingsState.draft['ai.imageConfig.watermark'] = false;
      updateSwitchButton('image-watermark', false);
    }
    if (target.id === 'image-fallback-imageMode' && target.value === 'ark-agent-plan') {
      if (!$('image-fallback-baseApi').value.trim()) $('image-fallback-baseApi').value = 'https://ark.cn-beijing.volces.com/api/plan/v3';
      if (!$('image-fallback-model').value.trim()) $('image-fallback-model').value = 'doubao-seedream-5.0-lite';
      $('image-fallback-size').value = '2K';
      $('image-fallback-responseFormat').value = 'url';
      $('image-fallback-outputFormat').value = 'png';
      apiSettingsState.draft['ai.imageConfig.fallbackApi.watermark'] = false;
      updateSwitchButton('image-fallback-watermark', false);
    }
    if (target.id === 'image-imageMode' && target.value === 'sd-webui') {
      if (!$('image-sdWebUi-baseApi').value.trim()) $('image-sdWebUi-baseApi').value = 'http://127.0.0.1:7860';
      if (!$('image-sdWebUi-samplerName').value.trim()) $('image-sdWebUi-samplerName').value = 'DPM++ 2M';
    }
    if (target.id === 'image-fallback-imageMode' && target.value === 'sd-webui') {
      if (!$('image-fallback-sdWebUi-baseApi').value.trim()) $('image-fallback-sdWebUi-baseApi').value = 'http://127.0.0.1:7860';
      if (!$('image-fallback-sdWebUi-samplerName').value.trim()) $('image-fallback-sdWebUi-samplerName').value = 'DPM++ 2M';
    }
    if (target.id === 'image-imageMode' && target.value === 'openai' && /^[234]K$/i.test($('image-size').value)) {
      $('image-size').value = 'auto';
    }
    updateDraftFromInputs();
    syncImageModeFieldVisibility();
    if (target.id === 'ai-apiProtocol' || target.id === 'ai-fallback-apiProtocol') {
      syncAiProtocolFieldHints();
    }
