function loadDraftFromConfig() {
  ...
  apiSettingsState.draft = {
    ...
    'ai.apiProtocol': ai.apiProtocol || 'openai',
    ...
    'ai.fallbackApi.apiProtocol': aiFallback.apiProtocol || '',
...
// 地址示例随协议联动：openai/grok 以 /v1 结尾，anthropic 不带 /v1（用户最常踩的坑）
function syncAiProtocolFieldHints() {
  const examples = {
    openai: {
      help: '填写服务商给你的基础地址，以 /v1 结尾，例如 https://api.qnaigc.com/v1；不要填完整的聊天接口路径。',
      placeholder: 'https://api.qnaigc.com/v1',
    },
    anthropic: {
      help: 'Anthropic 格式的地址不带 /v1，例如 https://api.qnaigc.com；插件会自动拼接 /v1/messages。',
      placeholder: 'https://api.qnaigc.com',
    },
    grok: {
      help: 'Grok 官方地址：https://api.x.ai/v1（以 /v1 结尾）；其他中转按 OpenAI 格式填写，不要填完整接口路径。',
      placeholder: 'https://api.x.ai/v1',
    },
  };
  const mainProtocol = $('ai-apiProtocol')?.value || 'openai';
  const mainExample = examples[mainProtocol] || examples.openai;
  const mainHelp = $('ai-baseApi-help');
  if (mainHelp) mainHelp.textContent = mainExample.help;
  const mainInput = $('ai-baseApi');
  if (mainInput) mainInput.placeholder = mainExample.placeholder;

  const fallbackProtocol = $('ai-fallback-apiProtocol')?.value || '';
  const effective = fallbackProtocol || mainProtocol;
  const fallbackExample = examples[effective] || examples.openai;
  const fallbackHelp = $('ai-fallback-baseApi-help');
  if (fallbackHelp) {
    fallbackHelp.textContent = fallbackProtocol
      ? fallbackExample.help
      : `备用地址写法跟随当前协议（${mainProtocol === 'anthropic' ? 'Anthropic：不带 /v1，如 https://api.qnaigc.com' : 'OpenAI/Grok：以 /v1 结尾，如 https://api.qnaigc.com/v1'}）。`;
  }
  const fallbackInput = $('ai-fallback-baseApi');
  if (fallbackInput) fallbackInput.placeholder = fallbackExample.placeholder;
}
...
function syncInputsFromDraft() {
  ...
  $('ai-apiProtocol').value = d['ai.apiProtocol'] || 'openai';
  ...
  $('ai-fallback-apiProtocol').value = d['ai.fallbackApi.apiProtocol'] || '';
...
function updateDraftFromInputs() {
  ...
  d['ai.apiProtocol'] = $('ai-apiProtocol')?.value || 'openai';
  ...
  d['ai.fallbackApi.apiProtocol'] = $('ai-fallback-apiProtocol')?.value || '';
