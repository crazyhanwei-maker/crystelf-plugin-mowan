...
function removeLoginTicketFromUrl() {
  try {
    ...
    // Ignore history failures in older embedded browsers.
  }
}
...
async function bootstrapLoginPage() {
  ...
  if (ticket) {
    try {
      ...
      setLoginMessage('正在验证一次性登录链接...', 'neutral');
      await auth.consumeLoginTicket(ticket);
      removeLoginTicketFromUrl();
      setLoginMessage('一次性登录成功，正在进入控制台...', 'success');
      window.setTimeout(() => auth.redirectAfterLogin(), 240);
      return;
    } catch (error) {
      removeLoginTicketFromUrl();
      setLoginMessage(error.message || '一次性登录链接无效或已过期', 'error');
    } finally {
