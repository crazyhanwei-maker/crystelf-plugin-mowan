...
function renderImageMonitorThumb(url, alt) {
  ...
  return `
    <a class="image-monitor-thumb-link" href="${url}" target="_blank" rel="noopener noreferrer">
      <img class="image-monitor-thumb" src="${previewUrl}" alt="${alt}" loading="lazy" referrerpolicy="no-referrer" onerror="this.closest('.image-monitor-thumb-link').outerHTML='<div class=\'image-monitor-thumb-placeholder\'>图片加载失败</div>'" />
    </a>
  `;
}
function getFilters() {
...
document.getElementById('auth-token').value = getAuthToken();
document.getElementById('save-token-btn').addEventListener('click', () => {
  localStorage.setItem('crystelf-web-console-token', document.getElementById('auth-token').value.trim());
  refresh().catch(error => openModal('刷新失败', error.message, { showCancel: false }));
});
...
refresh().catch(error => {
  document.body.innerHTML = `<pre>控制台初始化失败：${error.message}</pre>`;
});
