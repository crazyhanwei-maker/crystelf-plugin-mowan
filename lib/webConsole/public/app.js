...
function translateHealthIssueText(text) {
  ...
  const exactMap = {
    ...
    'Check baseApi, model name, and apiKey.': '请检查主对话接口地址、模型名称和 API Key。',
    'Search tool missing apiUrl': '搜索工具缺少接口地址',
    'Search is enabled but apiUrl/baseUrl is empty.': '搜索工具已开启，但 apiUrl/baseUrl 为空。',
    'TTS missing apiUrl': '语音合成缺少接口地址',
    'TTS is enabled but apiUrl/baseUrl is empty.': '语音合成已开启，但 apiUrl/baseUrl 为空。',
    'High AI error rate': 'AI 请求失败率偏高',
    'AI has recent failures': 'AI 最近存在失败请求',
    'Usage log not found': '未找到用量日志',
    'No usage log file was found yet.': '当前还没有发现用量日志文件。',
    'Usage log is stale': '用量日志长时间未更新',
    'Usage log has not been updated for more than 30 minutes.': '用量日志已超过 30 分钟没有更新。',
    'Affinity log is stale': '好感度日志长时间未更新',
    'Affinity log has not been updated for more than 24 hours.': '好感度日志已超过 24 小时没有更新。',
    'package.json is missing': '缺少 package.json',
    'Dependency inspection and install features are unavailable.': '依赖巡检和依赖安装功能当前不可用。',
    'Runtime dependencies are missing': '运行依赖缺失',
    'Runtime dependency versions mismatch lockfile': '运行依赖版本与锁文件不一致',
    'Dev dependencies are missing': '开发依赖缺失',
    'Chat history is large': '聊天历史数据较大',
    'Too many sessions': '会话数量较多',
    'User profiles are missing': '缺少用户画像数据',
    'Messages exist but no profile data was found.': '已经存在聊天消息，但没有发现用户画像数据。',
    'Public console has no login token': '公网控制台未配置登录口令',
    'Public console is writable': '公网控制台允许写操作',
  };
  if (exactMap[value]) return exactMap[value];

  let match = value.match(/^Recent requests:\s*(\d+),\s*errors:\s*(\d+)\.$/i);
  if (match) return `近期请求 ${match[1]} 次，失败 ${match[2]} 次。`;
  match = value.match(/^Recent failed requests:\s*(\d+)\.$/i);
  if (match) return `近期失败请求 ${match[1]} 次。`;
  match = value.match(/^Missing runtime dependencies:\s*(\d+)(?:\s*\(([^)]+)\))?\.$/i);
  if (match) return `缺少 ${match[1]} 个运行依赖${match[2] ? `（${match[2].split(',').map(item => item.trim()).filter(Boolean).join('、')}）` : ''}。`;
  match = value.match(/^Mismatched runtime dependencies:\s*(\d+)(?:\s*\(([^)]+)\))?\.$/i);
  if (match) return `有 ${match[1]} 个运行依赖版本不一致${match[2] ? `（${match[2].split(',').map(item => item.trim()).filter(Boolean).join('、')}）` : ''}。`;
  match = value.match(/^Missing dev dependencies:\s*(\d+)\.$/i);
  if (match) return `缺少 ${match[1]} 个开发依赖。`;
  match = value.match(/^Message count:\s*(\d+)\.$/i);
  if (match) return `当前消息记录 ${match[1]} 条。`;
  match = value.match(/^Session count:\s*(\d+)\.$/i);
  if (match) return `当前会话 ${match[1]} 个。`;
  match = value.match(/^Web console host\s+(.+?)\s+is publicly reachable without a configured login token\.$/i);
  if (match) return `控制台监听地址 ${match[1]} 可能被外部访问，但当前没有配置登录口令。`;
  match = value.match(/^Web console host\s+(.+?)\s+allows write operations\. Consider enabling read-only mode for internet exposure\.$/i);
  if (match) return `控制台监听地址 ${match[1]} 允许修改配置；如果暴露到公网，建议开启只读模式。`;
  return value;
}
...
function renderLogDiagnosisResult(data = {}) {
  ...
  const usage = data.usage
    ? `令牌 ${formatNumber(Number(data.usage.total_tokens || data.usage.totalTokens || 0))}`
    : '令牌用量暂无';
...
async function runLogDiagnosis() {
  const box = document.getElementById('log-diagnosis-result');
  if (box) {
    box.innerHTML = '<div class="setting-help">正在读取日志并调用 LLM 排查，请稍等...</div>';
  }
...
function renderCollectionEntries(data) {
  ...
  const entries = [
    ...
    {
      title: 'QQ 通信模拟器',
      description: '构造群消息、连续对话、截图、语音、加群申请、新成员入群和戳一戳事件，安全查看 Bot 回复、动作和调试信息。',
      countLabel: '模式',
      countValue: 6,
      href: '/qq-simulator.html',
      actionLabel: '打开 QQ 通信模拟器',
    },
    {
      title: 'AI 用量日志',
