  ...
  const performanceFallback = {
    apiQuality: {
      alerts: {
        status: 'neutral',
        sampleEnough: false,
        summary: 'API 主备质量接口暂不可用，稍后刷新后会重新采样。',
        alerts: [],
      },
      today: { total: { requestCount: 0 }, fallback: { requestCount: 0 }, fallbackShare: 0 },
      last24h: { total: { requestCount: 0, successRate: 0, errorRate: 0 }, fallback: { requestCount: 0, errorCount: 0, errorRate: 0 }, fallbackShare: 0 },
    },
  };
  try {
    const [overview, health, authStatus, config, configHistory, usageLogs, affinityLogs, auditLogs, tasks, trend, imageMonitorTrend, pokeImageSummaryTrend, performance] = await Promise.all([
      ...
      fetchJsonSafe('/api/performance?slowThresholdMs=10000', performanceFallback, requestOptions),
      fetchJsonSafe('/api/bot-watchdog', { enabled: false, offline: false, onlineCount: 0 }, requestOptions).then(data => {
        if (typeof updateBotWatchdogStatus === 'function') updateBotWatchdogStatus(data);
        return data;
      }),
    ]);
      ...
      fetchJsonSafe('/api/version/check', { success: false, status: 'check_failed', error: '版本检查接口暂不可用' }, requestOptions)
        .then(result => {
          ...
          if (typeof maybeShowVersionUpdateModal === 'function') {
            maybeShowVersionUpdateModal(result);
          }
        });
    ...
    const errorCount = [overview, health, authStatus, config, configHistory, usageLogs, affinityLogs, auditLogs, tasks, trend, imageMonitorTrend, pokeImageSummaryTrend, performance].filter(item => item && item.__error).length;
