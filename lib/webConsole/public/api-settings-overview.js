  ...
  return {
    'AI 主对话': {
      基础接入: {
        ...
        userAgent: d['ai.userAgent'] || `(默认 ${d['ai.defaultUserAgent'] || '插件 UA'})`,
      ...
      备用API: {
        ...
        failureThreshold: d['ai.fallbackApi.failureThreshold'] || 2,
        cooldownMs: d['ai.fallbackApi.cooldownMs'] || 300000,
      },
    },
    图像生成: {
      基础接入: {
        ...
        webSearch: d['ai.imageConfig.webSearch'] === true,
      ...
      备用API: {
        ...
        failureThreshold: d['ai.imageConfig.fallbackApi.failureThreshold'] || 2,
        cooldownMs: d['ai.imageConfig.fallbackApi.cooldownMs'] || 300000,
      },
    },
    图片监控: {
      基础接入: {
      ...
      备用API: {
        ...
        failureThreshold: d['imageMonitor.fallbackApi.failureThreshold'] || 2,
        cooldownMs: d['imageMonitor.fallbackApi.cooldownMs'] || 300000,
      },
      本地保存与入库: {
    ...
    搜索工具: {
      ...
      备用API: {
        ...
        failureThreshold: d['search.fallbackApi.failureThreshold'] || 2,
        cooldownMs: d['search.fallbackApi.cooldownMs'] || 300000,
      },
    },
    表情包: {
      基础接入: {
        ...
        fallbackAutoSwitchEnabled: d['ai.memeConfig.fallbackApi.autoSwitchEnabled'],
        fallbackFailureThreshold: d['ai.memeConfig.fallbackApi.failureThreshold'] || 2,
        fallbackCooldownMs: d['ai.memeConfig.fallbackApi.cooldownMs'] || 300000,
      },
      本地目录: {
...
function buildApiOverview() {
  ...
  sections.push({
    ...
    tags: [
      ...
      d['ai.userAgent'] ? '自定义 UA 已开启' : '使用默认 UA',
      ...
      d['ai.fallbackApi.autoSwitchEnabled'] ? `自动切换 ${d['ai.fallbackApi.failureThreshold'] || 2} 次` : '自动切换关闭',
    ],
  ...
  if (d['ai.imageConfig.enabled'] === false) {
    ...
    const imageModeLabel = imageMode === 'ark-agent-plan'
      ? '火山 Agent Plan'
      : imageMode === 'sd-webui'
        ? 'SD WebUI'
      : imageMode === 'jimeng'
    ...
    if (imageMode !== 'sd-webui' && !d['ai.imageConfig.model']) imageIssues.push('缺少图像模型');
    if (imageMode === 'jimeng') {
      if (!d['ai.imageConfig.jimengApiUrl']) imageIssues.push('即梦模式还没有填写服务地址');
    } else if (imageMode === 'sd-webui') {
      if (!d['ai.imageConfig.sdWebUi.baseApi']) imageIssues.push('SD WebUI 模式还没有填写服务地址');
    } else {
    ...
    if (d['ai.imageConfig.fallbackApi.enabled']) {
      ...
      } else if (fallbackMode === 'sd-webui') {
        if (!d['ai.imageConfig.fallbackApi.sdWebUi.baseApi']) imageWarnings.push('生图备用 API 已开启但缺少备用 SD WebUI 地址');
      } else {
      ...
      if (fallbackMode !== 'sd-webui' && !hasSecretValue(d['ai.imageConfig.fallbackApi.apiKey'], d['ai.imageConfig.fallbackApi.apiKeyConfigured'])) {
    ...
    sections.push({
      ...
      tags: [
        ...
        imageMode === 'jimeng'
          ...
          : imageMode === 'sd-webui'
            ? (d['ai.imageConfig.sdWebUi.passwordConfigured'] ? '已配置 WebUI 认证' : 'WebUI 无认证')
          : (hasImageApiKey ? '单独生图密钥' : (hasMainApiKey ? '复用主对话密钥' : '未配置密钥')),
        d['ai.imageConfig.fallbackApi.enabled'] ? '备用 API 已开启' : '备用 API 已关闭',
        d['ai.imageConfig.fallbackApi.autoSwitchEnabled'] ? `自动切换 ${d['ai.imageConfig.fallbackApi.failureThreshold'] || 2} 次` : '自动切换关闭',
        imageMode === 'ark-agent-plan'
          ? `分辨率 ${d['ai.imageConfig.size'] || '2K'}`
          : imageMode === 'sd-webui'
            ? `${d['ai.imageConfig.sdWebUi.width'] || 1024}x${d['ai.imageConfig.sdWebUi.height'] || 1024} / ${d['ai.imageConfig.sdWebUi.steps'] || 20} 步`
          : `质量 ${d['ai.imageConfig.quality'] || 'high'}`,
        imageMode === 'sd-webui' ? '固定单次 1 张' : `格式 ${d['ai.imageConfig.responseFormat'] || 'b64_json'}`,
        ...(imageMode === 'ark-agent-plan'
          ? [d['ai.imageConfig.webSearch'] ? '联网搜索已开启' : '联网搜索已关闭']
          : []),
      ],
  ...
  if (!d['imageMonitor.enabled']) {
    ...
    sections.push({
      ...
      tags: [
        ...
        d['imageMonitor.fallbackApi.autoSwitchEnabled'] ? `自动切换 ${d['imageMonitor.fallbackApi.failureThreshold'] || 2} 次` : '自动切换关闭',
        d['imageMonitor.storageEnabled'] ? '图片本地入库开启' : '仅记录 MD5 永久去重',
  ...
  if (!d['search.enabled']) {
    ...
    sections.push({
      ...
      tags: [
        ...
        d['search.fallbackApi.autoSwitchEnabled'] ? `自动切换 ${d['search.fallbackApi.failureThreshold'] || 2} 次` : '自动切换关闭',
      ],
  ...
  sections.push({
    ...
    tags: [
      ...
      d['ai.memeConfig.fallbackApi.autoSwitchEnabled'] ? `自动切换 ${d['ai.memeConfig.fallbackApi.failureThreshold'] || 2} 次` : '自动切换关闭',
...
function renderApiOverview() {
  ...
  $('api-settings-overview-list').innerHTML = overview.sections.map(item => `
    <article class="api-settings-overview-item tone-${item.status}" data-section="${escapeHtml(item.key)}" role="button" tabindex="0" aria-label="跳转到 ${escapeHtml(item.title)} 设置">
