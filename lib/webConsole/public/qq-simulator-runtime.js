...
function $(id) {
  ...
  function getPayload(eventType) {
    ...
    const isMessageEvent = selectedEvent === 'message' || selectedEvent === 'private_message';
    const isPrivateMessageEvent = selectedEvent === 'private_message';
    ...
    return {
      ...
      groupId: isPrivateMessageEvent ? '' : $('qq-simulator-group-id').value.trim(),
      ...
      includeAt: selectedEvent === 'message' && $('qq-simulator-include-at').value !== 'false',
      dispatchMode: $('qq-simulator-dispatch-mode').value || 'safe',
      conversationMode,
      history: conversationMode ? buildHistory() : [],
      groupHistory: isPrivateMessageEvent ? [] : parseGroupHistoryInput(),
