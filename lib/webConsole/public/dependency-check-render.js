function renderActiveInstallTaskCard(task) {
  ...
  return `
    <div class="list-item tone-${tone} dependency-active-item">
      ...
      ${renderInstallTaskProgress(task)}
...
function renderInstallTaskProgress(task = {}) {
  const percent = normalizeProgressPercent(task?.progressPercent);
  const label = task?.progressLabel || task?.currentStage || formatInstallTaskStatus(task?.status);
  if (!percent && !label) return '';
  const elapsed = Number(task?.elapsedMs || 0) > 0 ? ` · 耗时 ${formatDurationMs(task.elapsedMs)}` : '';
  const latest = task?.latestOutputAt ? ` · 最近输出 ${formatTime(task.latestOutputAt)}` : '';
  return `
    <div class="dependency-task-progress" aria-label="安装进度 ${escapeHtml(percent)}%">
      <div class="dependency-task-progress-head">
        <span>${escapeHtml(label || '等待执行')}</span>
        <strong>${escapeHtml(percent)}%</strong>
      </div>
      <div class="dependency-task-progress-track">
        <span style="width: ${escapeHtml(percent)}%"></span>
      </div>
      <div class="dependency-task-progress-meta">${escapeHtml(`${formatInstallTaskStatus(task?.status)}${elapsed}${latest}`)}</div>
    </div>
  `;
}

function renderInstallTaskOutputEvents(task = {}) {
  const outputEvents = Array.isArray(task?.outputEvents) ? task.outputEvents.slice(-20) : [];
  if (outputEvents.length <= 0) return '';
  return `
    <div class="dependency-task-output-stream">
      <div class="dependency-task-output-title">实时输出</div>
      ${outputEvents.map(event => `
        <div class="dependency-task-output-item tone-${event.stream === 'stderr' ? 'error' : 'neutral'}">
          <span>${escapeHtml(formatTime(event.time))}</span>
          <strong>${escapeHtml(event.stream || 'stdout')}</strong>
          <em>${escapeHtml(event.stage || '-')}</em>
...
function renderInstallTaskDetails(task, options = {}) {
  ...
  const metaLines = [
    ...
    task.elapsedMs ? `耗时：${formatDurationMs(task.elapsedMs)}` : '',
    task.latestOutputAt ? `最近输出：${formatTime(task.latestOutputAt)}` : '',
  ...
  return `
    <details class="dependency-task-details"${task.status === 'error' || options.live ? ' open' : ''}>
      ...
      ${renderInstallTaskOutputEvents(task)}
...
function getCurrentDependencyCount(summary, key, fallbackKey) {
  const value = summary?.[key];
  if (value !== undefined && value !== null) {
    return getNumber(value);
  }
  return getNumber(summary?.[fallbackKey]);
}

function renderSummary(report) {
  const summary = report?.summary || {};
  const statusText = formatDependencyReportStatus(summary.status);
  const otherSummary = report?.otherPluginDependencies?.summary || {};
  const totalCount = getNumber(summary.totalCount);
  const problemCount = getNumber(summary.problemCount);
  const runtimeCount = getNumber(summary.runtimeCount);
  const runtimeInstalledCount = getNumber(summary.runtimeInstalledCount);
  const runtimeMissingCount = getNumber(summary.runtimeMissingCount);
  const runtimeMismatchCount = getNumber(summary.runtimeMismatchCount);
  const devCount = getNumber(summary.devCount);
  const devInstalledCount = getNumber(summary.devInstalledCount);
  const devMissingCount = getNumber(summary.devMissingCount);
  const devMismatchCount = getNumber(summary.devMismatchCount);
  const currentProblemCount = getCurrentDependencyCount(summary, 'currentPluginProblemCount', 'problemCount');
  const currentTotalCount = getCurrentDependencyCount(summary, 'currentPluginTotalCount', 'totalCount');
  const otherPluginCount = getNumber(summary.otherPluginCount, getNumber(otherSummary.pluginCount));
  const otherPluginTotalCount = getNumber(summary.otherPluginTotalCount, getNumber(otherSummary.totalCount));
  const otherPluginProblemCount = getNumber(summary.otherPluginProblemCount, getNumber(otherSummary.problemCount));
  const otherPluginRuntimeMissingCount = getNumber(summary.otherPluginRuntimeMissingCount, getNumber(otherSummary.runtimeMissingCount));
  const otherPluginDevMissingCount = getNumber(summary.otherPluginDevMissingCount, getNumber(otherSummary.devMissingCount));
  const note = !totalCount
    ? '暂无依赖检查数据。'
    : problemCount > 0
      ? `当前发现 ${formatNumber(problemCount)} 项依赖问题，优先处理运行依赖缺失。`
      : '当前依赖完整，未发现缺失或版本偏差。';

  document.getElementById('dependency-summary').innerHTML = [
    '<div class="config-summary-grid">',
    `<div class="config-summary-card"><h3>总体状态</h3><div class="config-summary-list"><div class="config-summary-row"><span class="config-summary-label">状态</span><span class="config-summary-value tone-${summary.status === 'error' ? 'error' : summary.status === 'warn' ? 'error' : 'success'}">${statusText}</span></div><div class="config-summary-row"><span class="config-summary-label">问题数</span><span class="config-summary-value">${formatNumber(problemCount)}</span></div></div></div>`,
    `<div class="config-summary-card"><h3>运行依赖</h3><div class="config-summary-list"><div class="config-summary-row"><span class="config-summary-label">已安装</span><span class="config-summary-value">${formatNumber(runtimeInstalledCount)} / ${formatNumber(runtimeCount)}</span></div><div class="config-summary-row"><span class="config-summary-label">缺失 / 偏差</span><span class="config-summary-value">${formatNumber(runtimeMissingCount)} / ${formatNumber(runtimeMismatchCount)}</span></div></div></div>`,
    `<div class="config-summary-card"><h3>开发依赖</h3><div class="config-summary-list"><div class="config-summary-row"><span class="config-summary-label">已安装</span><span class="config-summary-value">${formatNumber(devInstalledCount)} / ${formatNumber(devCount)}</span></div><div class="config-summary-row"><span class="config-summary-label">缺失 / 偏差</span><span class="config-summary-value">${formatNumber(devMissingCount)} / ${formatNumber(devMismatchCount)}</span></div></div></div>`,
    `<div class="config-summary-card"><h3>其他插件</h3><div class="config-summary-list"><div class="config-summary-row"><span class="config-summary-label">已检查</span><span class="config-summary-value">${formatNumber(otherPluginCount)} 个插件</span></div><div class="config-summary-row"><span class="config-summary-label">问题 / 依赖</span><span class="config-summary-value">${formatNumber(otherPluginProblemCount)} / ${formatNumber(otherPluginTotalCount)}</span></div></div></div>`,
    ...
    '</div>',
    `<div class="config-summary-note">${escapeHtml(`当前插件 ${formatNumber(currentProblemCount)} 项问题 / ${formatNumber(currentTotalCount)} 条依赖；其他插件运行缺失 ${formatNumber(otherPluginRuntimeMissingCount)} 条，开发缺失 ${formatNumber(otherPluginDevMissingCount)} 条。`)}</div>`,
...
function renderDependencyCard(item) {
  ...
  if ((item.installScope || 'current') === 'plugin') {
    details.unshift(`<div>所属插件：${escapeHtml(item.pluginName || item.pluginId || '-')}</div>`);
  }
...
function renderOtherPluginDependencies(report) {
  ...
  const filtered = items
    ...
    .sort((a, b) => {
      ...
      const statusOrder = { missing: 0, version_mismatch: 1, not_installed: 2, ok: 3 };
  ...
  if (!summary.pluginsDirExists) {
    ...
    meta.textContent = `已查看 ${formatNumber(summary.pluginCount)} 个其他插件，共 ${formatNumber(summary.totalCount)} 条依赖；问题 ${formatNumber(summary.problemCount)} 项，运行缺失 ${formatNumber(summary.runtimeMissingCount)} 条，开发缺失 ${formatNumber(summary.devMissingCount)} 条。`;
  }
...
function renderReport(report) {
  ...
  const otherPluginItems = Array.isArray(report?.otherPluginDependencies?.items)
    ...
    : [];
  const problemItems = [...items, ...otherPluginItems]
    .filter(item => item.status !== 'ok')
    .sort((a, b) => {
      if (a.level !== b.level) {
        return a.level === 'error' ? -1 : 1;
      }
      const scopeCompare = String(a.installScope || 'current').localeCompare(String(b.installScope || 'current'));
  ...
  const summary = report?.summary || {};
  setMeta(`状态：${formatDependencyReportStatus(summary.status)}，缺失运行依赖 ${formatNumber(summary.runtimeMissingCount || 0)} 个，共 ${formatNumber(summary.problemCount || 0)} 个问题`);
}
