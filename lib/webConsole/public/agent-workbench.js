...
function renderConversation(task = null) {
  ...
  const outputText = output || (running ? 'Agent 正在准备输出...' : '任务没有返回文本。');
  const extraOutput = stderr ? `<details class="agent-artifact-card"><summary>运行诊断</summary><pre>${formatAgentText(stderr)}</pre></details>` : '';
  const errorOutput = task.error ? `<div class="agent-message-error"><strong>任务未完成</strong><span>${formatAgentText(task.error)}</span></div>` : '';
  const eventOutput = events.length ? `
    <details class="agent-event-card" ${running ? 'open' : ''}>
  ...
  ` : '';
  const diffOutput = task.workspaceChanged || task.diffStat || task.diffText
    ? `<details class="agent-artifact-card"><summary>文件改动 <span>${task.changedFiles?.length || 0} 个文件</span></summary><pre>${formatAgentText(buildTaskDiffOutput(task))}</pre></details>`
    : '';
  target.innerHTML = `
    <article class="agent-message agent-message-user">
      <div class="agent-message-avatar">你</div>
      <div class="agent-message-body">
        <div class="agent-message-label">你的任务 <time>${escapeHtml(formatTime(task.createdAt))}</time></div>
        <div class="agent-message-bubble">${formatAgentText(task.prompt || task.promptPreview || '未记录任务内容')}</div>
      </div>
    </article>
    <article class="agent-message agent-message-agent">
      <div class="agent-message-avatar agent-avatar-agent">⌘</div>
      <div class="agent-message-body">
        <div class="agent-message-label">${escapeHtml(task.providerLabel || 'Agent')} <span class="agent-message-stage">${escapeHtml(getModeLabel(task.mode))} · ${escapeHtml(status.label)}</span></div>
...
function renderSelectedTask(data = {}) {
  ...
  getElement('agent-output-meta').textContent = `${task.providerLabel || task.providerId} · ${task.workspaceLabel || task.workspaceId} · ${getModeLabel(task.mode)} · ${status.label} · ${formatDuration(task.elapsedMs)}`;
...
function renderTasks(data = {}) {
  const tasks = Array.isArray(data.tasks) ? data.tasks : [];
  getElement('agent-history-meta').textContent = tasks.length ? `${tasks.length} 个最近任务` : '暂无任务。';
    ...
    return `
      <button type="button" class="agent-task-item ${task.id === agentWorkbenchState.selectedTaskId && !agentWorkbenchState.newSession ? 'is-selected' : ''}" data-task-id="${escapeHtml(task.id)}">
        <span class="agent-task-main">
          ...
          <span>${escapeHtml(task.promptPreview || task.providerLabel || task.providerId)}</span>
        </span>
        ...
        : writeMode && !data.config?.writeEnabled
          ...
          : writeMode && providerId !== 'opencode'
            ...
            : writeMode && !data.config?.writableWorkspaces?.[workspaceId]
              ? '所选工作目录尚未获得写入授权。'
              : writeMode
                ? '将使用 build Agent 修改文件；要求 Git 工作区干净。'
...
async function runTask(event) {
  ...
  const confirmed = await webConsoleConfirm(
    writeMode
      ? `将使用 ${provider?.label || providerId} 直接修改“${workspace?.label || workspaceId}”中的文件。该目录必须是干净的 Git 仓库；任务可创建和编辑文件，但不能执行终端命令或访问目录外内容。是否继续？`
      : `将使用 ${provider?.label || providerId} 在“${workspace?.label || workspaceId}”启动${mode === 'patch' ? '补丁建议' : '只读分析'}任务。任务不会写入文件，是否继续？`,
    { title: writeMode ? '确认实际修改文件' : '启动 Agent 任务', confirmText: writeMode ? '确认并开始修改' : '开始任务' },
  );
  if (!confirmed) return;
