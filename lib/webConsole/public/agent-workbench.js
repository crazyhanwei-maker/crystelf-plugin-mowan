...
function renderConversation(task = null) {
  ...
  // 任务执行期间只展示本轮，避免历史助手回复暂时占据当前输出区。
  const lastUserIndex = messages.reduce((index, message, currentIndex) => message.role === 'user' ? currentIndex : index, -1);
  const visibleMessages = running && lastUserIndex >= 0 ? messages.slice(lastUserIndex) : messages;
...
function updateRunState(data = {}) {
  ...
  if (agentWorkbenchState.slashCommandBusy) {
    ...
    runButton.textContent = fullAccess ? '发送并执行' : (writeMode ? '发送并修改' : '发送');
    runButton.disabled = !enabled || !providerAvailable || (writeMode && !writeAllowed);
  }
...
async function runTask(event) {
  ...
  updateConversationFollowButton();
  setButtonBusy('agent-run-btn', true, fullAccess ? '发送并执行' : (writeMode ? '发送并修改' : '发送'), '启动中...');
  renderPendingConversation(prompt, agentWorkbenchState.composerAttachments);
  try {
    ...
    setButtonBusy('agent-run-btn', false, fullAccess ? '发送并执行' : (writeMode ? '发送并修改' : '发送'), '启动中...');
    updateRunState(agentWorkbenchState.payload || {});
  }
}
