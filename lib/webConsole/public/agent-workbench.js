...
function renderNativeProviders(data = {}) {
  ...
  getElement('agent-native-provider-list').innerHTML = renderNativeList(providers, '没有读取到 OpenCode Provider。', provider => `
    <article class="agent-native-item">
      ...
      <div class="agent-native-model-chips">${renderNativeList(provider.models || [], '暂无模型', model => `<button type="button" class="agent-native-chip" data-native-model="${escapeHtml(model.id)}" title="使用 ${escapeHtml(model.id)}">${escapeHtml(model.name || model.id)}</button>`)}</div>
    </article>
  `);
}
...
function renderNativeTools(data = {}) {
  ...
  getElement('agent-native-skill-list').innerHTML = renderNativeList(skills, '没有发现 Skills。', item => `<div class="agent-native-row"><strong>${escapeHtml(item.name)}</strong><span>${escapeHtml(item.description || item.location || '')}</span></div>`);
  getElement('agent-native-command-list').innerHTML = renderNativeList(commands, '没有发现 Commands。', item => `<div class="agent-native-row"><strong>${escapeHtml(item.name)}</strong><span>${escapeHtml(item.description || item.source || '')}</span></div>`);
...
function renderNativeDiff(diffs = []) {
  ...
  target.innerHTML = renderNativeList(diffs, '选择会话后读取 OpenCode Diff。', diff => {
    ...
    return `<details class="agent-native-item agent-native-diff-item"><summary><strong>${escapeHtml(file)}</strong><span>${escapeHtml(diff.status || '')}</span></summary><pre>${formatAgentText(text)}</pre></details>`;
  });
}
...
async function loadNativePayload() {
  const result = await fetchJson(`/api/agent-workbench/native?workspace=${encodeURIComponent(getNativeWorkspaceId())}`);
