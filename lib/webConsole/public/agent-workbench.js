  ...
  const liveOutput = running ? `
    <article class="agent-message agent-message-agent agent-message-live">
      ...
      <div class="agent-message-body">
        ...
        <div class="agent-message-bubble agent-agent-bubble">${formatAgentText(normalizeAgentOutput(task.outputText) || (task.status === 'pending' ? `任务正在排队${task.queuePosition ? `，当前位置 ${task.queuePosition}` : ''}...` : 'Agent 正在准备输出...'))}</div>
  ...
  target.innerHTML = `
    <article class="agent-message agent-message-user">
      ...
      <div class="agent-message-body">
        ...
        <div class="agent-message-bubble">${formatAgentText(prompt)}${renderMessageAttachments(attachments)}</div>
      </div>
    </article>
    <article class="agent-message agent-message-agent agent-message-live">
      <div class="agent-message-avatar agent-avatar-agent">⌘</div>
...
function bindEvents() {
  ...
  getElement('agent-native-history-load').addEventListener('click', async () => {
    if (await loadSessionHistory()) getElement('agent-native-mask').classList.add('hidden');
  });
