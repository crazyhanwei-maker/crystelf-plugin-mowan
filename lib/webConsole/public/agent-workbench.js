...
function renderModelManagerEditor() {
  ...
  const modelRows = (provider.models || []).map((model, modelIndex) => `
    <div class="agent-manager-model-row" data-manager-model-index="${modelIndex}">
      ...
      <span class="agent-manager-model-badges" data-manager-badges="${escapeHtml(model)}"></span>
      <button type="button" class="agent-manager-model-remove" data-manager-model-remove="${modelIndex}" aria-label="删除模型">🗑</button>
    </div>`).join('');
  editor.innerHTML = `
...
// 模型徽章（视觉/上下文窗口）异步查表填充
function renderModelManagerBadges() {
  const editor = getElement('agent-model-manager-editor');
  if (!editor) return;
  const nodes = [...editor.querySelectorAll('[data-manager-badges]')];
  const names = nodes.map(node => node.dataset.managerBadges).filter(Boolean);
  if (!names.length) return;
  fetchJson(`/api/agent-workbench/model-badges?models=${encodeURIComponent(names.join(','))}`).then(result => {
    const badges = result?.badges || {};
    for (const node of nodes) {
      const badge = badges[node.dataset.managerBadges];
      if (!badge) continue;
      const chips = [];
      if (badge.contextLimit >= 1000) {
        chips.push(badge.contextLimit >= 1000000
          ? `${Math.round(badge.contextLimit / 100000) / 10}M`
          : `${Math.round(badge.contextLimit / 1000)}K`);
      }
      if (badge.vision === true) chips.push('视觉');
      node.innerHTML = chips.map(chip => `<small class="agent-manager-badge">${escapeHtml(chip)}</small>`).join('');
    }
  }).catch(() => { });
}
...
function bindEvents() {
  ...
  getElement('agent-model-manager-editor').addEventListener('click', event => {
    ...
    if (event.target.closest?.('[data-manager-model-add-btn]')) {
      ...
      if (!value) return;
      const provider = agentWorkbenchState.modelProviderDraft[agentWorkbenchState.modelManagerActiveIndex];
      if (provider && !provider.models.includes(value)) provider.models.push(value);
      renderModelManager();
