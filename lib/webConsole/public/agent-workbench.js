...
function renderNativeTerminals(terminals = []) {
  ...
  const output = selected?.output || (terminals.length ? '终端暂无输出。' : '受控终端默认关闭，请先在设置中授权。');
  if (ensureXterm()) {
    ...
    const fallback = getElement('agent-terminal-fallback');
...
function bindEvents() {
  ...
  document.addEventListener('click', event => {
    if (event.target.closest?.('#agent-composer-options, #agent-composer-tools-btn')) return;
    closeComposerOptions();
  });
