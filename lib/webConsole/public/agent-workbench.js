...
function renderTasks(data = {}) {
  ...
  getElement('agent-task-list').innerHTML = tasks.length ? tasks.map(task => {
    ...
    return `
      <div class="agent-task-item ${task.id === agentWorkbenchState.selectedTaskId && !agentWorkbenchState.newSession ? 'is-selected' : ''}" role="button" tabindex="0" data-task-id="${escapeHtml(task.id)}" title="${escapeHtml(`${title} · ${status.label} · ${formatTime(activityTime)}`)}" aria-label="${escapeHtml(`${title}，${status.label}，${relativeTime}`)}">
