...
function syncMobileActionTray() {
  ...
  // 问答卡已固定悬浮在输入栏上方（agent-question-dock），托盘只负责移动端等审批卡
  const sections = [getElement('agent-permission-section')].filter(Boolean);
...
function bindEvents() {
  ...
  // 悬浮问答卡标题可折叠：只收起列表，标题条保留计数提醒
  const questionDock = getElement('agent-question-section');
  const questionDockToggle = getElement('agent-question-dock-toggle');
  questionDockToggle.addEventListener('click', () => {
    const collapsed = questionDock.classList.toggle('is-collapsed');
    questionDockToggle.setAttribute('aria-expanded', String(!collapsed));
  });
