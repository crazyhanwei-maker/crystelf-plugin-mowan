...
function renderModelManagerEditor() {
  ...
  const modelRows = (provider.models || []).map((model, modelIndex) => {
    ...
    const context = typeof model === 'object' ? Math.round(Number(model?.context) || 0) : 0;
    const vision = typeof model === 'object' && model?.vision === true;
    const noVision = typeof model === 'object' && model?.vision === false;
    const visionValue = vision ? 'true' : noVision ? 'false' : '';
    return `
    <div class="agent-manager-model-row" data-manager-model-index="${modelIndex}">
      <strong class="agent-manager-model-name">${escapeHtml(modelName)}</strong>
      <input type="text" class="agent-manager-model-context" data-manager-model-context="${modelIndex}" value="${context > 0 ? context : ''}" maxlength="10" placeholder="上下文" title="上下文窗口（token 数），留空用 models.dev 表值" />
      <select class="agent-manager-model-vision" data-manager-model-vision="${modelIndex}" title="视觉（图片输入）：自动 = 用 models.dev 表值">
        <option value=""${!visionValue ? ' selected' : ''}>视觉自动</option>
        <option value="true"${vision ? ' selected' : ''}>支持视觉</option>
        <option value="false"${noVision ? ' selected' : ''}>纯文本</option>
      </select>
      <span class="agent-manager-model-badges" data-manager-badges="${escapeHtml(modelName)}"></span>
  ...
  editor.innerHTML = `
    ...
    <div class="agent-manager-model-add-row"><input type="text" id="agent-manager-model-input" maxlength="160" placeholder="模型名，回车或点"添加模型"加入" /></div>`;
...
function bindEvents() {
  ...
  getElement('agent-model-manager-editor').addEventListener('click', event => {
    ...
    if (event.target.closest?.('[data-manager-model-add-btn]')) {
      const input = getElement('agent-manager-model-input');
      const value = String(input?.value || '').trim();
      if (!value) return;
      const provider = agentWorkbenchState.modelProviderDraft[agentWorkbenchState.modelManagerActiveIndex];
      if (provider && !(provider.models || []).some(item => (typeof item === 'string' ? item : item?.name) === value)) {
        provider.models.push(value);
      }
      renderModelManager();
      getElement('agent-manager-model-input')?.focus();
    }
  });
  ...
  // 行内控件值同步进 draft（对象化字符串条目）；保存前必须调用，防止 change 未触发丢值
  function syncModelMetaToDraft() {
    const provider = agentWorkbenchState.modelProviderDraft[agentWorkbenchState.modelManagerActiveIndex];
    if (!provider) return;
    const rows = [...managerEditor.querySelectorAll('.agent-manager-model-row')];
    for (const row of rows) {
      const index = Number(row.dataset.managerModelIndex);
      const model = provider.models?.[index];
      if (!model) continue;
      const name = typeof model === 'string' ? model : model.name;
      const contextInput = row.querySelector('[data-manager-model-context]');
      const visionSelect = row.querySelector('[data-manager-model-vision]');
      // 字符串条目有自定义值时升级为对象
      if (typeof model === 'string' && ((contextInput?.value || '').trim() || visionSelect?.value)) {
        provider.models[index] = { name };
      }
      const target = provider.models[index];
      const context = Math.max(0, Math.round(Number(contextInput?.value) || 0));
      if (context > 0) target.context = context; else delete target.context;
      if (visionSelect?.value === 'true') target.vision = true;
      else if (visionSelect?.value === 'false') target.vision = false;
      else delete target.vision;
    }
  }

  managerEditor.addEventListener('change', event => {
    const contextInput = event.target.closest?.('[data-manager-model-context]');
    if (contextInput) {
      const provider = agentWorkbenchState.modelProviderDraft[agentWorkbenchState.modelManagerActiveIndex];
      const model = provider?.models?.[Number(contextInput.dataset.managerModelContext)];
      if (model && typeof model === 'object') {
        const context = Math.max(0, Math.round(Number(contextInput.value) || 0));
        if (context > 0) model.context = context; else delete model.context;
      }
      renderModelManagerRail();
      return;
    }
    const visionSelect = event.target.closest?.('[data-manager-model-vision]');
    if (visionSelect) {
      const provider = agentWorkbenchState.modelProviderDraft[agentWorkbenchState.modelManagerActiveIndex];
      const model = provider?.models?.[Number(visionSelect.dataset.managerModelVision)];
      if (model && typeof model === 'object') {
        if (visionSelect.value === 'true') model.vision = true;
        else if (visionSelect.value === 'false') model.vision = false;
        else delete model.vision;
      }
      renderModelManagerRail();
      return;
    }
  ...
  managerEditor.addEventListener('keydown', event => {
    if (event.target.id !== 'agent-manager-model-input') return;
  ...
  getElement('agent-model-manager-save').addEventListener('click', () => {
    // 输入后直接点保存不会触发 change：先把行内上下文/视觉值强制同步进 draft
    syncModelMetaToDraft();
