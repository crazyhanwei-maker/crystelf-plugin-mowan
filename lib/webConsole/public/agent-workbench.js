...
function syncMobileActionTray() {
  ...
  const workspaceSection = sidebar?.querySelector('.agent-workspace-files');
  const sections = [getElement('agent-permission-section'), getElement('agent-question-section')].filter(Boolean);
  if (!tray || !sidebar || !workspaceSection || sections.length === 0) return;
  ...
  sections.forEach(section => {
    if (mobile) {
      ...
      sidebar.insertBefore(section, workspaceSection);
    }
  });
