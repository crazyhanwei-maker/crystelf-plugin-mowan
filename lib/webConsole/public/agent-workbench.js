...
function renderAssistantArtifacts(message = {}, options = {}) {
  ...
  ` : '';
  return `${errorOutput}${reasoningOutput}${renderAgentTools(message.tools)}${usageOutput}${diagnosticOutput}${diffOutput}${eventOutput}`;
}
...
function formatAgentMarkdown(value = '') {
  ...
  return renderer ? renderer.render(text) : escapeHtml(text).replace(/\r?\n/g, '<br>');
}
...
function renderContext(task = null) {
  const contextUsage = Math.max(0, Number(task?.contextUsage || 0));
  const contextLimit = Math.max(0, Number(task?.contextLimit || 0));
  ...
  if (contextLimit > 0) {
    const percent = Math.min(100, Math.round((contextUsage / contextLimit) * 100));
    const compactedAt = task?.lastCompactedAt ? ` · 最近压缩 ${formatTime(task.lastCompactedAt)}` : '';
    contextLabel.textContent = `${formatTokenCount(contextUsage)} / ${formatTokenCount(contextLimit)}`;
    contextBar.style.width = `${percent}%`;
    contextBar.classList.toggle('is-warn', percent >= 80);
    contextNote.textContent = `${percent}% · ${agentWorkbenchState.payload?.config?.autoCompactEnabled === false ? '自动压缩已关闭' : '达到阈值后自动压缩'}${compactedAt}`;
  } else {
    contextLabel.textContent = contextUsage ? `${formatTokenCount(contextUsage)} Token` : '未读取';
    contextBar.style.width = '0%';
    contextBar.classList.remove('is-warn');
    contextNote.textContent = contextUsage ? '当前模型未返回上下文上限。' : '完成一轮对话后显示当前会话用量。';
  }
  const files = Array.isArray(task?.changedFiles) ? task.changedFiles : [];
...
function syncMobileActionTray() {
  const tray = getElement('agent-mobile-action-tray');
  const sidebar = document.querySelector('.agent-context-sidebar');
  const usageSection = sidebar?.querySelector('.agent-context-usage-section');
  const sections = [getElement('agent-permission-section'), getElement('agent-question-section')].filter(Boolean);
  if (!tray || !sidebar || !usageSection || sections.length === 0) return;

  const mobile = window.matchMedia('(max-width: 760px)').matches;
  sections.forEach(section => {
    if (mobile) {
      if (section.parentElement !== tray) tray.append(section);
    } else if (section.parentElement !== sidebar) {
      sidebar.insertBefore(section, usageSection);
    }
  });
  tray.classList.toggle('has-actions', mobile && sections.some(section => !section.classList.contains('hidden')));
}
...
function mergeStreamTaskDelta(change = {}) {
  ...
  const bubble = liveMessage?.querySelector('.agent-agent-bubble');
  if (bubble) {
    const placeholder = task.status === 'pending'
      ? `任务正在排队${task.queuePosition ? `，当前位置 ${task.queuePosition}` : ''}...`
      : 'Agent 正在准备输出...';
    bubble.innerHTML = formatAgentText(normalizeAgentOutput(task.outputText) || placeholder);
  }
  const status = getStatusMeta(task.status);
  const stage = liveMessage?.querySelector('.agent-message-stage');
  if (stage) stage.textContent = `${getModeLabel(task.mode)} · ${status.label}`;
...
function renderConversation(task = null) {
  ...
  // 任务执行期间从上一条已完成的助手消息之后开始展示，确保当前轮的多条跟进消息都可见。
  const lastAssistantIndexBeforeLive = messages.reduce((index, message, currentIndex) => message.role === 'assistant' ? currentIndex : index, -1);
  const visibleMessages = running && lastAssistantIndexBeforeLive >= 0
    ? messages.slice(lastAssistantIndexBeforeLive + 1)
    : messages;
  ...
  const html = visibleMessages.map((message, index) => {
    if (message.role === 'user') {
      const deliveryLabel = message.delivery === 'steer'
        ? '<span class="agent-followup-label">立即调整</span>'
      ...
      return `
        <article class="agent-message agent-message-user">
          ...
          <div class="agent-message-body">
            <div class="agent-message-label">你 ${deliveryLabel}<time>${escapeHtml(formatTime(message.createdAt))}</time></div>
    ...
    return `
      <article class="agent-message agent-message-agent">
        ...
        <div class="agent-message-body">
          ...
          <div class="agent-message-bubble agent-agent-bubble agent-markdown">${formatAgentMarkdown(outputText)}</div>
          ${renderNativeMessageActions(task, message)}
  ...
  const liveOutput = running ? `
    <article class="agent-message agent-message-agent agent-message-live">
      ...
      <div class="agent-message-body">
        ...
        <div class="agent-message-bubble agent-agent-bubble agent-markdown">${formatAgentMarkdown(normalizeAgentOutput(task.outputText) || (task.status === 'pending' ? `任务正在排队${task.queuePosition ? `，当前位置 ${task.queuePosition}` : ''}...` : 'Agent 正在准备输出...'))}</div>
...
function renderPendingConversation(prompt = '', attachments = []) {
  ...
  target.innerHTML = `
    <article class="agent-message agent-message-user">
      <div class="agent-message-avatar">你</div>
      <div class="agent-message-body">
        <div class="agent-message-label">你 <time>刚刚</time></div>
    ...
    <article class="agent-message agent-message-agent agent-message-live">
      <div class="agent-message-avatar agent-avatar-agent">⌘</div>
      <div class="agent-message-body">
        <div class="agent-message-label">OpenCode <span class="agent-message-stage">正在启动</span></div>
...
function renderSelectedTask(data = {}) {
  ...
  if (agentWorkbenchState.formTaskId !== task.id) {
    ...
    agentWorkbenchState.followUpDelivery = task.followUpDelivery === 'steer' ? 'steer' : 'queue';
...
function renderTasks(data = {}) {
  ...
  getElement('agent-task-list').innerHTML = tasks.length ? tasks.map(task => {
    const status = getStatusMeta(task.status);
    const running = ['running', 'pending'].includes(task.status);
    return `
      <div class="agent-task-row">
        <button type="button" class="agent-task-item ${task.id === agentWorkbenchState.selectedTaskId && !agentWorkbenchState.newSession ? 'is-selected' : ''}" data-task-id="${escapeHtml(task.id)}">
          <span class="agent-task-main">
            ...
            <span>${escapeHtml(task.promptPreview || task.providerLabel || task.providerId)} · ${escapeHtml(task.turnCount || 1)} 轮 · ${getPriorityLabel(task.priority)}${task.status === 'pending' && task.queuePosition ? ` · 队列 ${escapeHtml(task.queuePosition)}` : ''}</span>
          </span>
        ...
        <button type="button" class="agent-task-delete" data-task-delete="${escapeHtml(task.id)}" title="${running ? '运行中的会话不能删除' : '删除会话'}" aria-label="删除会话：${escapeHtml(task.title || 'Agent 任务')}" ${running ? 'disabled' : ''}>删除</button>
      </div>
    `;
...
function updateRunState(data = {}) {
  ...
  const followUpControl = getElement('agent-followup-control');
  followUpControl?.classList.toggle('hidden', !canFollowUp);
  followUpControl?.querySelectorAll('[data-agent-followup]').forEach(button => {
    button.classList.toggle('is-active', button.dataset.agentFollowup === agentWorkbenchState.followUpDelivery);
  });
  if (agentWorkbenchState.slashCommandBusy) {
    runButton.disabled = true;
    runButton.textContent = agentWorkbenchState.slashCommandBusyText || '处理中...';
  } else {
    runButton.textContent = canFollowUp
      ? (agentWorkbenchState.followUpDelivery === 'steer' ? '立即调整' : '加入队列')
  ...
  getElement('agent-composer-hint').textContent = !enabled
    ...
    : canFollowUp
      ? (agentWorkbenchState.followUpDelivery === 'steer'
        ? '立即调整：补充要求会注入当前执行，适合纠正方向。'
        : `排队跟进：当前响应完成后继续执行${selectedTask.pendingFollowUpCount ? `，已有 ${selectedTask.pendingFollowUpCount} 条等待中` : ''}。`)
    : !providerAvailable
      ? '没有可用的 Agent CLI。'
      : active >= limit
...
async function runTask(event) {
  event?.preventDefault?.();
  if (getElement('agent-mobile-action-tray')?.contains(document.activeElement)) return;
  ...
  if (slashCommand) {
    ...
    setStatus(isCompact ? '正在压缩当前会话上下文，请稍候...' : `正在执行 /${slashCommand.name}...`);
    try {
  ...
  const normalRunText = activeFollowUp
    ? (followUpDelivery === 'steer' ? '立即调整' : '加入队列')
    : (fullAccess ? '发送并执行' : (writeMode ? '发送并修改' : '发送'));
  setButtonBusy('agent-run-btn', true, normalRunText, activeFollowUp
    ? (followUpDelivery === 'steer' ? '调整中...' : '排队中...')
    : '启动中...');
  ...
  try {
    const result = await postJson('/api/agent-workbench/tasks', {
      ...
      ...(activeFollowUp ? { delivery: followUpDelivery } : {}),
      useWorktree,
      worktreeConfirmed,
    });
...
function closeComposerOptions() {
  ...
  const button = getElement('agent-composer-tools-btn');
...
async function archiveSelectedTask(forceArchived) {
  ...
  await postJson(`/api/agent-workbench/tasks/${encodeURIComponent(task.id)}/${action}`, {});
  await refreshAgentWorkbench({ quiet: true });
  return true;
}
...
function startNewAgentSession() {
  ...
  agentWorkbenchState.followUpDelivery = 'queue';
  updateModelVariantOptions('');
  renderTasks(agentWorkbenchState.payload || {});
  renderSelectedTask(agentWorkbenchState.payload || {});
  getElement('agent-prompt').focus();
...
function bindEvents() {
  ...
  getElement('agent-followup-control')?.addEventListener('click', event => {
    const button = event.target.closest?.('[data-agent-followup]');
    if (!button) return;
    agentWorkbenchState.followUpDelivery = button.dataset.agentFollowup === 'steer' ? 'steer' : 'queue';
    updateRunState(agentWorkbenchState.payload || {});
  });
  ...
  document.addEventListener('click', event => {
    if (!event.target.closest?.('#agent-composer-options, #agent-composer-tools-btn')) closeComposerOptions();
  ...
  getElement('agent-task-list').addEventListener('click', event => {
    ...
    if (deleteButton) {
      ...
      deleteAgentTask(deleteButton.dataset.taskDelete).catch(error => webConsoleAlert(error.message));
      return;
    }
...
document.addEventListener('DOMContentLoaded', () => {
  bindEvents();
  syncMobileActionTray();
  window.addEventListener('resize', syncMobileActionTray, { passive: true });
