...
function renderTasks(data = {}) {
  ...
  getElement('agent-task-list').innerHTML = tasks.length ? tasks.map(task => {
    ...
    return `
      <button type="button" class="agent-task-item ${task.id === agentWorkbenchState.selectedTaskId && !agentWorkbenchState.newSession ? 'is-selected' : ''}" data-task-id="${escapeHtml(task.id)}" title="${escapeHtml(`${title} · ${status.label} · ${formatTime(activityTime)}`)}" aria-label="${escapeHtml(`${title}，${status.label}，${relativeTime}`)}">
