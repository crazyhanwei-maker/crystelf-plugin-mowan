...
function renderModelMenuPane() {
  ...
  if (activeKey === '__default__') {
    ...
    paneHtml = models.map(modelName => {
      const isCurrent = currentProviderId === activeKey && currentModel === modelName;
      return `<button type="button" class="agent-model-item${isCurrent ? ' is-current' : ''}" data-model-provider="${escapeHtml(activeKey)}" data-model-value="${escapeHtml(modelName)}"><span><strong>${escapeHtml(modelName)}</strong></span>${isCurrent ? '<span class="agent-model-check">✓</span>' : ''}</button>`;
