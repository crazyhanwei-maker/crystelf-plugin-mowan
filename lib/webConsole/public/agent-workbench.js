...
function renderModelMenu() {
  ...
  items.push(`<button type="button" class="agent-model-item ${!currentModel ? 'is-current' : ''}" data-model-provider="" data-model-value="">
    <span><strong>默认模型</strong><small>主对话 API 的模型配置</small></span>
    ${!currentModel ? '<span class="agent-model-check">✓</span>' : ''}
  </button>`);

  for (const provider of providers) {
    items.push(`<div class="agent-model-group">${escapeHtml(provider.name)}<small>${provider.baseApi ? escapeHtml(provider.baseApi.replace(/^https?:\/\//, '').slice(0, 34)) : ''}</small></div>`);
    const providerId = customModelProviderId(provider.name);
    for (const modelName of provider.models) {
      const isCurrent = currentProviderId === providerId && currentModel === modelName;
      items.push(`<button type="button" class="agent-model-item ${isCurrent ? 'is-current' : ''}" data-model-provider="${escapeHtml(providerId)}" data-model-value="${escapeHtml(modelName)}">
        <span><strong>${escapeHtml(modelName)}</strong></span>
        ${isCurrent ? '<span class="agent-model-check">✓</span>' : ''}
      </button>`);
    }
  }

  items.push(`<button type="button" class="agent-model-item" data-model-action="custom">
    <span><strong>自定义</strong><small>手动输入模型名（设置面板内）</small></span><span>›</span>
  </button>`);
  items.push(`<button type="button" class="agent-model-item" data-model-action="manage">
    <span><strong>管理模型</strong><small>添加供应商与模型列表</small></span><span>›</span>
  </button>`);

  list.innerHTML = items.join('');
  const { model, providerId } = getCurrentModelSelection();
  getElement('agent-model-menu-provider').textContent =
    providerId && providerId !== 'opencode'
      ? (getModelProviders().find(item => customModelProviderId(item.name) === providerId)?.name || providerId)
      : '默认';
  getElement('agent-model-menu-model').textContent = model || '默认模型';
}
...
function renderVariantMenu() {
  ...
  const items = [...select.options].map(option => {
    ...
    return `<button type="button" class="agent-variant-item${selected ? ' is-selected' : ''}" data-variant-value="${escapeHtml(value)}"><span>${escapeHtml(label)}</span><span class="agent-variant-check"${selected ? '' : ' style="visibility:hidden"'}>✓</span></button>`;
  });
...
function renderModelManager() {
  ...
  if (!draft.length) {
    list.innerHTML = '<p class="agent-model-manager-hint">还没有自定义供应商，点下方「添加供应商」。</p>';
    return;
  }
  list.innerHTML = draft.map((provider, index) => `
    <div class="agent-model-provider-row" data-provider-index="${index}">
      <div class="agent-model-provider-grid">
        <label>名称<input type="text" data-provider-field="name" value="${escapeHtml(provider.name || '')}" maxlength="40" placeholder="如：硅基流动" /></label>
        <label>API 地址<input type="text" data-provider-field="baseApi" value="${escapeHtml(provider.baseApi || '')}" maxlength="300" placeholder="https://api.example.com/v1" /></label>
        <label>密钥<input type="password" data-provider-field="apiKey" value="" placeholder="${provider.apiKeyConfigured ? '已配置（留空保持）' : '未配置'}" autocomplete="new-password" /></label>
        <label>接入协议<select data-provider-field="apiProtocol"><option value="">跟随主接口</option><option value="openai"${provider.apiProtocol === 'openai' ? ' selected' : ''}>OpenAI 兼容格式</option><option value="anthropic"${provider.apiProtocol === 'anthropic' ? ' selected' : ''}>Anthropic（Claude Messages API）</option><option value="grok"${provider.apiProtocol === 'grok' ? ' selected' : ''}>Grok（xAI，OpenAI 兼容）</option></select></label>
        <label>模型（一行一个）<textarea data-provider-field="models" rows="3" placeholder="gpt-4o&#10;claude-sonnet-4">${escapeHtml((provider.models || []).join('\n'))}</textarea></label>
      </div>
      <button type="button" class="agent-quiet-btn" data-provider-remove="${index}">删除</button>
    </div>
  `).join('');
}

function collectModelManagerDraft() {
  const rows = [...getElement('agent-model-manager-list').querySelectorAll('.agent-model-provider-row')];
  return rows.map(row => {
    const value = field => row.querySelector(`[data-provider-field="${field}"]`)?.value || '';
    return {
      name: value('name').trim(),
      baseApi: value('baseApi').trim(),
...
function bindEvents() {
  ...
  getElement('agent-model-menu-list').addEventListener('click', event => {
    ...
    if (!item) return;
    if (item.dataset.modelAction === 'manage') {
      setModelMenuOpen(false);
      openModelManager();
      return;
    }
    if (item.dataset.modelAction === 'custom') {
      setModelMenuOpen(false);
      openComposerOptions();
      getElement('agent-model').focus();
      return;
    }
  ...
  getElement('agent-model-manager-list').addEventListener('click', event => {
    ...
    if (!button) return;
    agentWorkbenchState.modelProviderDraft.splice(Number(button.dataset.providerRemove), 1);
    renderModelManager();
  });
  getElement('agent-model-manager-save').addEventListener('click', () => {
    const draft = collectModelManagerDraft();
