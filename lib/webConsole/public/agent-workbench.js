...
function submitModelForm() {
  ...
  if (duplicateIndex >= 0) {
    closeModelForm();
    renderModelManager();
    // 高亮已存在的行，让"为什么保存没反应"一眼可见
    const row = document.querySelector(`.agent-manager-model-row[data-manager-model-index="${duplicateIndex}"]`);
    if (row) flashComposerInput(row);
    webConsoleAlert(`模型 ${name} 已在列表中（同一供应商不允许重名）。点该行可编辑它的参数。`);
    return;
  }
