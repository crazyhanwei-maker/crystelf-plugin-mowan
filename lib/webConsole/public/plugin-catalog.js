...
function renderList() {
  ...
  target.innerHTML = items.length > 0
    ...
    : '<div class="list-item">没有匹配的插件目录项。</div>';
  renderDetail();
}
...
function renderTasks(tasks = []) {
  ...
  list.innerHTML = tasks.length > 0
    ...
    `).join('')
    : '<div class="list-item">暂无插件安装任务。</div>';
}
...
async function installCatalogItem(catalogId = '') {
  ...
  try {
    const payload = await postJson('/api/plugin-catalog/install', {
      catalogId,
      directoryName: getInstallDirectoryName() || getCatalogDefaultDirectoryName(item),
      installDependencies: getInstallDependencies(),
    });
    const task = payload?.task || {};
...
function bindEvents() {
  ...
  $('plugin-catalog-category-filter')?.addEventListener('change', () => renderList());
  $('plugin-catalog-state-filter')?.addEventListener('change', () => renderList());
