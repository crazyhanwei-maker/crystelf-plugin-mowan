...
const pluginCatalogState = {
  ...
  initialCatalogQuery: getInitialCatalogQuery(),
  tasks: [],
};
...
function getInitialCatalogQuery() {
  try {
    ...
    return String(params.get('catalog') || '').trim();
  } catch {
    return '';
  }
}
...
function normalizeProgressPercent(value = 0) {
  const number = Number(value);
...
function buildPluginInstallConfirmMessage(item = {}, options = {}) {
  ...
  const lines = [
    ...
    `仓库：${item.repo || '-'}`,
    `目标目录：${precheck.targetDir || targetDir}`,
  ...
  if (checks.length > 0) {
    ...
    checks.forEach(check => {
      lines.push(`- ${check.ok ? '通过' : '失败'} ${check.label}${check.detail ? `：${check.detail}` : ''}`);
    });
  }
  ...
  lines.push(
    ...
    '- 依赖安装会在克隆完成后按 packageManager 和锁文件自动选择 pnpm/npm/yarn。',
  );
  ...
  if (installDependencies) {
    lines.push(
      '',
      '依赖安装已开启：',
      `- 将在 ${targetDir} 自动检测包管理器后执行 install --ignore-scripts`,
      '- 该命令可能修改新插件目录内的锁文件和 node_modules。',
    );
  } else {
...
function applyInitialCatalogSearch() {
  const query = pluginCatalogState.initialCatalogQuery;
  if (!query) return;
  const searchInput = $('plugin-catalog-search');
  const listSearchInput = $('plugin-catalog-list-search');
  if (searchInput && !searchInput.value) searchInput.value = query;
  if (listSearchInput && !listSearchInput.value) listSearchInput.value = query;
}
...
function getTaskStepStatusLabel(status = '') {
...
function getTaskStepTone(status = '') {
  if (status === 'success') return 'success';
  if (status === 'error') return 'error';
  return 'neutral';
}
...
function renderTaskStep(step = {}) {
  ...
  const tone = getTaskStepTone(status);
  const meta = [
    step.startedAt ? `开始 ${formatTime(step.startedAt)}` : '',
    step.finishedAt ? `结束 ${formatTime(step.finishedAt)}` : '',
    step.exitCode !== null && step.exitCode !== undefined ? `退出码 ${step.exitCode}` : '',
  ].filter(Boolean);
  return `
    <div class="plugin-catalog-task-step tone-${tone}">
      <div class="plugin-catalog-task-step-head">
        <strong>${escapeHtml(step.label || step.key || '任务步骤')}</strong>
        <span class="detail-tag tone-${tone}">${escapeHtml(getTaskStepStatusLabel(status))}</span>
      </div>
      ${step.message ? `<div class="plugin-catalog-task-step-message">${escapeHtml(trimText(step.message, 220))}</div>` : ''}
      ${step.command?.length ? `<div class="plugin-catalog-task-step-command">${escapeHtml(step.command.join(' '))}</div>` : ''}
      ${meta.length > 0 ? `<div class="plugin-catalog-task-step-meta">${escapeHtml(meta.join(' · '))}</div>` : ''}
    </div>
  `;
}
...
function renderTaskSteps(task = {}) {
  ...
  return `<div class="plugin-catalog-task-steps">${steps.map(step => renderTaskStep(step)).join('')}</div>`;
}

function renderTaskProgress(task = {}) {
  const percent = normalizeProgressPercent(task.progressPercent);
  const label = task.progressLabel || task.stage || getTaskStatusLabel(task.status);
  ...
  return `
    <div class="plugin-catalog-task-progress" aria-label="插件安装进度 ${escapeHtml(percent)}%">
      <div class="plugin-catalog-task-progress-head">
        <span>${escapeHtml(label || '等待执行')}</span>
        <strong>${escapeHtml(percent)}%</strong>
      </div>
      <div class="plugin-catalog-task-progress-track"><span style="width: ${escapeHtml(percent)}%"></span></div>
      <div class="plugin-catalog-task-progress-meta">${escapeHtml(`${getTaskStatusLabel(task.status)}${elapsed}${latest}`)}</div>
    </div>
  `;
}

function renderTaskEvents(task = {}) {
  const events = Array.isArray(task.events) ? task.events.slice(-12) : [];
  if (events.length <= 0) return '';
  return `
    <details class="plugin-catalog-task-events"${isTaskActive(task) || task.status === 'error' ? ' open' : ''}>
      <summary>任务时间线</summary>
      <div class="plugin-catalog-task-event-list">
        ${events.map(event => `
          <div class="plugin-catalog-task-event tone-${event.level === 'error' ? 'error' : 'neutral'}">
...
function renderTaskOutputEvents(task = {}) {
  const outputEvents = Array.isArray(task.outputEvents) ? task.outputEvents.slice(-24) : [];
  if (outputEvents.length <= 0) return '';
  return `
    <details class="plugin-catalog-task-output"${isTaskActive(task) || task.status === 'error' ? ' open' : ''}>
      <summary>实时输出</summary>
      <div class="plugin-catalog-task-output-list">
        ${outputEvents.map(event => `
          <div class="plugin-catalog-task-output-item tone-${event.stream === 'stderr' ? 'error' : 'neutral'}">
            <span>${escapeHtml(formatTime(event.time))}</span>
            <strong>${escapeHtml(event.stream || 'stdout')}</strong>
            <em>${escapeHtml(event.process || event.stage || '-')}</em>
...
function renderTaskResidue(task = {}) {
  const residue = task.residue || {};
  if (residue.exists !== true) return '';
  const pathText = residue.path || task.targetDir || '';
  return `
    <div class="plugin-catalog-task-residue">
      <strong>发现可能残留的插件目录</strong>
      <span>${escapeHtml(residue.reason || '任务未成功完成，请手动检查该目录。')}</span>
      <div class="plugin-catalog-task-residue-actions">
        <code>${escapeHtml(pathText || '-')}</code>
        ${pathText ? `<a class="mini-btn" href="/file-browser.html?path=${encodeURIComponent(pathText)}">打开目录</a>` : ''}
      </div>
    </div>
  `;
}
...
function getFilteredItems() {
  ...
  return items.filter(item => {
    ...
    return [
      ...
      item.sourceUrl,
      item.readmeUrl,
      item.defaultDirectoryName,
      item.installState?.defaultDirectoryName,
      item.localState?.directoryName,
      item.localState?.relativePath,
      ...(Array.isArray(item.tags) ? item.tags : []),
    ].some(value => String(value || '').toLowerCase().includes(query));
  });
}

function isInitialCatalogMatch(item = {}, query = '') {
...
function renderList() {
  ...
  if (pluginCatalogState.initialCatalogQuery) {
    const matched = items.find(item => isInitialCatalogMatch(item, pluginCatalogState.initialCatalogQuery));
    if (matched?.id) pluginCatalogState.selectedId = matched.id;
    pluginCatalogState.initialCatalogQuery = '';
  }
  ...
  list.innerHTML = tasks.length > 0
    ? tasks.map(task => `
      <div class="list-item tone-${getTaskTone(task.status)} plugin-catalog-task-card">
        ...
        <div class="plugin-catalog-task-meta-line">
          阶段：${escapeHtml(task.stage || '-')} · 依赖：${escapeHtml(task.installDependencies ? '开启' : '关闭')}
          ${task.packageManager ? ` · 包管理器：${escapeHtml(task.packageManager)}` : ''}
          ${Array.isArray(task.packageManagerCandidates) && task.packageManagerCandidates.length > 0 ? ` · 候选：${escapeHtml(task.packageManagerCandidates.join(' / '))}` : ''}
        </div>
        ${renderTaskProgress(task)}
        ${renderTaskSteps(task)}
        ${renderTaskEvents(task)}
        ${renderTaskOutputEvents(task)}
        ${renderTaskResidue(task)}
        ...
        <div class="plugin-catalog-task-actions actions">
          ...
          ${isTaskActive(task) ? `<button type="button" class="mini-btn danger" data-plugin-catalog-cancel="${escapeHtml(task.id)}">取消</button>` : ''}
...
async function installCatalogItem(catalogId = '') {
  ...
  try {
    ...
    const precheck = await postJson('/api/plugin-catalog/install-precheck', {
      catalogId,
      directoryName,
      installDependencies,
    });
    if (precheck?.ok === false) {
      const failedChecks = Array.isArray(precheck.checks) ? precheck.checks.filter(check => check.ok !== true) : [];
      setStatus(`插件安装预检未通过：${failedChecks.map(check => check.label).join('、') || '存在风险项'}`, 'error');
      return;
    }
...
async function cancelInstallTask(taskId = '') {
  if (!taskId) return;
  const confirmed = await webConsoleConfirm('确认取消该插件安装任务？\n\n控制台只会终止当前安装进程，不会删除已经创建的目录或依赖文件。', {
    title: '取消插件安装',
    confirmText: '确认取消',
    cancelText: '返回',
  });
  if (!confirmed) return;
  try {
    await postJson('/api/plugin-catalog/install-cancel', { taskId });
    setStatus('已请求取消插件安装任务。', 'neutral');
    await refreshTasks();
  } catch (error) {
    setStatus(`取消插件安装失败：${error.message}`, 'error');
  }
}
...
function bindEvents() {
  ...
  $('plugin-catalog-task-list')?.addEventListener('click', event => {
    const cancelButton = event.target.closest('[data-plugin-catalog-cancel]');
    if (cancelButton) {
      cancelInstallTask(cancelButton.dataset.pluginCatalogCancel || '');
      return;
    }
