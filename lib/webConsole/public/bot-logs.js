(() => {
  ...
  function getStructuredTokenClass(content = '', index = 0) {
    const value = String(content || '');
    if (index === 0 && /^\d{1,2}:\d{2}:\d{2}(?:\.\d{1,6})?$/.test(value)) return 'log-token-time';
    if (/^(?:ERROR|FATAL|SEVERE)$/i.test(value)) return 'log-token-error';
    if (/^(?:WARN|WARNING)$/i.test(value)) return 'log-token-warn';
    if (/^(?:INFO|NOTICE)$/i.test(value)) return 'log-token-info';
    if (/^(?:DEBUG|TRACE)$/i.test(value)) return 'log-token-debug';
    if (/^MARK$/i.test(value)) return 'log-token-mark';
    if (/^开始处理/.test(value)) return 'log-token-action-start';
    if (/^完成/.test(value)) return 'log-token-action-done';
    if (/^(?:CQ|AQ|JSON|image):/i.test(value)) return 'log-token-embedded';
    return 'log-token-detail';
  }

  function renderStructuredLogText(text, keyword) {
    ...
    while ((match = bracketPattern.exec(value))) {
      html += highlightEscapedText(value.slice(cursor, match.index), keyword);
      const token = match[0];
      const tokenClass = getStructuredTokenClass(match[1], bracketIndex);
      html += `<span class="${tokenClass}">${highlightEscapedText(token, keyword)}</span>`;
      cursor = match.index + token.length;
      bracketIndex += 1;
    }
    html += highlightEscapedText(value.slice(cursor), keyword);
    return html;
  }
  ...
  function renderAnsiLogText(text, keyword) {
    ...
    const appendText = segment => {
      ...
      const content = highlightEscapedText(segment, keyword);
      html += className ? `<span class="log-ansi ${className}">${content}</span>` : content;
    };
    ...
    while ((match = pattern.exec(value))) {
      ...
      for (const code of codes) {
        if (code === 0) {
          ...
          for (const colorClass of Object.values(ANSI_COLOR_CLASSES)) classes.delete(colorClass);
        } else if (ANSI_COLOR_CLASSES[code]) {
          for (const colorClass of Object.values(ANSI_COLOR_CLASSES)) classes.delete(colorClass);
          classes.add(ANSI_COLOR_CLASSES[code]);
        }
      }
  ...
  function renderLogText(text, keyword) {
    return /\x1b\[[0-9;]*m/.test(String(text || ''))
      ? renderAnsiLogText(text, keyword)
      : renderStructuredLogText(text, keyword);
  }
  ...
  function renderLines() {
    ...
    for (const line of state.lines) {
      ...
      textElement.innerHTML = renderLogText(line.rawText || line.text, filter.keyword);
