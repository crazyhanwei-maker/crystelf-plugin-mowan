...
async function initSessionDebug() {
  ...
  if (!sessionId) {
    ...
    document.getElementById('session-debug-main').innerHTML = '<div class="detail-box">请先从最近会话列表打开某个会话的调试页。<div class="actions"><a class="link-btn" href="/session-center.html">返回会话列表</a></div></div>';
...
initSessionDebug().catch(error => {
  ...
  pre.textContent = `对话调试加载失败: ${error?.message || String(error)}`;
