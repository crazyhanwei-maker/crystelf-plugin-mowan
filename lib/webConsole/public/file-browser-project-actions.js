async function runProjectSearch(rawQuery = '') {
  ...
  const scopePath = $('file-manager-subdir-toggle')?.checked === false
    ? normalizeRelativePath(fileBrowserState.currentDir || '')
    : '';
  fileBrowserState.projectSearch.scopePath = scopePath;
  renderProjectSearchPanel();
  updateActionButtons();
  try {
    const scopeQuery = scopePath ? `&path=${encodeURIComponent(scopePath)}` : '';
    const data = await fetchJson(
      `/api/file-browser/search?query=${encodeURIComponent(query)}&caseSensitive=${fileBrowserState.projectSearch.caseSensitive ? '1' : '0'}&limit=${encodeURIComponent(fileBrowserState.projectSearch.limit)}${scopeQuery}`,
    );
