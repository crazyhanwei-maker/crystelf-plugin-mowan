  ...
  sessions: {
    ...
    renderList(data) {
      ...
      document.getElementById('collection-list').innerHTML = items.length > 0
        ...
        `).join('')
        : renderEmptyState('暂无会话记录', '还没有记录到会话消息；也可以清空筛选后再看一次。');
    },
  },
  affinity: {
    title: '好感度 Top',
    ...
    renderList(data) {
      ...
      document.getElementById('collection-list').innerHTML = items.length > 0
        ...
        `).join('')
        : renderEmptyState('暂无好感记录', '当前筛选条件下没有好感数据；有互动记录后这里会自动出现。');
    },
  },
  profiles: {
    title: '用户画像',
    ...
    renderList(data) {
      ...
      document.getElementById('collection-list').innerHTML = items.length > 0
        ...
        `).join('')
        : renderEmptyState('暂无用户画像', '当前筛选条件下没有画像记录；画像生成后这里会自动汇总。');
    },
  },
};
...
const state = { page: 1, refreshRequestId: 0, refreshPromise: null, refreshController: null };
...
function setListState(kind, title, detail = '') {
  if (consoleUi.setState) {
    consoleUi.setState('collection-list', { kind, title, detail });
    return;
  }
...
const scheduleRefresh = createDebouncedTask(() => {
  refreshCollection().catch(error => webConsoleAlert(error.message));
}, 260);
...
async function refreshCollection() {
  ...
  setListState('loading', pageConfig.loadingText, '数据返回后会自动更新列表。');
  state.refreshPromise = (async () => {
    try {
      const data = await fetchJson(`${pageConfig.endpoint}?${params.toString()}`, requestOptions);
      ...
      setListState('error', `${pageConfig.title}加载失败`, error.message);
      throw error;
    } finally {
      if (requestId === state.refreshRequestId) {
...
async function handleAction(target) {
  if (target.dataset.action === 'show-affinity-history') {
    ...
    await webConsoleAlert(lines.join('\n'), { title: '好感度历史' });
    return;
  }
  if (target.dataset.action === 'reset-affinity') {
    if (!await webConsoleConfirm(`确认重置 ${target.dataset.groupId} / ${target.dataset.userId} 的好感数据？`)) return;
  ...
  if (target.dataset.action === 'delete-profile') {
    if (!await webConsoleConfirm(`确认删除画像 ${target.dataset.sessionId} / ${target.dataset.userId}？`)) return;
  ...
  if (target.dataset.action === 'reset-session') {
    if (!await webConsoleConfirm(`确认重置会话 ${target.dataset.sessionId}？这会清空消息和相关状态。`)) return;
    await postJson('/api/sessions/reset', { sessionId: target.dataset.sessionId });
    await refreshCollection();
    return;
  }
  if (target.dataset.action === 'reset-session-messages') {
    if (!await webConsoleConfirm(`确认只清空会话 ${target.dataset.sessionId} 的消息记录？`)) return;
...
document.addEventListener('click', event => {
  ...
  handleAction(target).catch(error => webConsoleAlert(error.message));
});
