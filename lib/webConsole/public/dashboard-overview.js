...
// 运维监控横幅：聚合看门狗（botAdapterWatchdogStatusCache 来自 dashboard-health.js）渲染在 dashboard-strip 前
function formatOpsWatchdogDuration(ms = 0) {
...
function renderOpsMonitorBanner() {
  const box = document.getElementById('ops-monitor-banner');
  if (!box) return;
  const wd = typeof botWatchdogStatusCache !== 'undefined' ? botWatchdogStatusCache : null;
  if (!wd || wd.enabled === false) {
    box.innerHTML = '<div class="ops-monitor-item tone-neutral"><span class="ops-monitor-label">看门狗</span><span class="ops-monitor-value">未启用</span></div>';
    return;
  }
  const offline = wd.offline === true;
  const onlineCount = Number(wd.onlineCount || 0);
  const events = Array.isArray(wd.events) ? wd.events : [];
  const recentEvents = events.slice(-3);
  const emailOn = wd.emailConfigured === true;
  const items = [
    `<div class="ops-monitor-item ${offline ? 'tone-error' : onlineCount > 0 ? 'tone-success' : 'tone-neutral'}">
      <span class="ops-monitor-label">QQ 适配器</span>
      <span class="ops-monitor-value">${offline ? `掉线中（已 ${formatOpsWatchdogDuration(wd.offlineDurationMs)}）` : onlineCount > 0 ? `在线 ${onlineCount} 个` : '待连接'}</span>
    </div>`,
    `<div class="ops-monitor-item tone-neutral">
      <span class="ops-monitor-label">邮件告警</span>
      <span class="ops-monitor-value">${emailOn ? '已配置' : '未配置'}</span>
    </div>`,
    `<div class="ops-monitor-item ${recentEvents.length > 0 ? 'tone-warn' : 'tone-success'}">
      <span class="ops-monitor-label">近 24h 连接事件</span>
      <span class="ops-monitor-value">${recentEvents.length > 0
        ? recentEvents.map(ev => `${ev.type === 'offline' ? '掉线' : '恢复'}`).join(' → ') || '无'
        : '无'}</span>
    </div>`,
  ];
...
function buildConfigMigrationOverviewCard(report = null) {
  const changedFileCount = Number(report?.changedFileCount || 0);
  const changedCount = Number(report?.changedCount || 0);
  const migratedAt = report?.migratedAt ? formatTime(report.migratedAt) : '';
  const files = Array.isArray(report?.files) ? report.files : [];
  const hasRecord = Boolean(report?.migratedAt);
  const reasons = Array.from(new Set(files.flatMap(item => Array.isArray(item?.reasons) ? item.reasons : []))).filter(Boolean);
  const fileNames = files.map(item => item?.file).filter(Boolean).slice(0, 4);
  const metaLines = [
    hasRecord ? `最近迁移：${migratedAt}` : '暂无自动迁移记录',
    changedFileCount > 0 ? `涉及配置：${fileNames.join('、') || `${changedFileCount} 个文件`}` : '',
    reasons.length > 0 ? `原因：${reasons.join('、')}` : '',
  ].filter(Boolean);

  return {
    label: '配置迁移',
    valueText: hasRecord ? `${changedFileCount} 个文件` : '未发生',
    valueClass: 'overview-card-value-text',
    metaHtml: [
      `<div class="detail-tags"><span class="detail-tag ${changedCount > 0 ? 'tone-success' : 'tone-neutral'}">${changedCount > 0 ? `补齐 ${formatNumber(changedCount)} 项` : '配置稳定'}</span></div>`,
      `<div class="overview-card-meta-note">${escapeHtml(metaLines.join('\n') || '启动时没有发现需要迁移的配置。').replace(/\n/g, '<br>')}</div>`,
    ].join(''),
    tag: '配置',
    size: 4,
    tone: changedCount > 0 ? 'success' : 'accent',
    action: '<a class="link-btn" href="/plugin-settings.html">打开设置</a>',
  };
}
...
function buildVersionCheckTitle(result = null) {
  ...
  const lines = [
    ...
    result.remoteUrl ? `来源：${result.remoteUrl}` : '',
...
function renderVersionCheck(result = null) {
  ...
  if (!target) return;
  const clickable = Boolean(result && (result.updateAvailable === true || result.status === 'update_available'));
  target.className = `version-check-status ${getVersionCheckClass(result)}${clickable ? ' is-clickable' : ''}`;
  target.textContent = getVersionCheckText(result);
  target.title = clickable
    ? `${buildVersionCheckTitle(result)}\n\n点击查看更新日志`
    : buildVersionCheckTitle(result);
  if (clickable) {
    target.onclick = () => {
      if (typeof maybeShowVersionUpdateModal === 'function') {
        maybeShowVersionUpdateModal(result, { force: true });
      }
    };
  } else {
    target.onclick = null;
...
function renderOverview(data) {
  ...
  const cards = [
    ...
    buildConfigMigrationOverviewCard(data.configMigration),
    {
      label: '总用量',
