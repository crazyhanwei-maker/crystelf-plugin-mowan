...
const LOG_FILE_PATTERN = /\.(log|txt|out|err)$/i;
...
function normalizeInteger(value, fallback = 0, min = 0, max = 1000000) {
...
export function createLogDiagnosisConsole(options = {}) {
  ...
  function getCandidateDirectories(source = 'auto') {
  ...
  function listCandidateLogFiles(source = 'auto') {
    const files = [];
  ...
  function readLogTailText(filePath = '', maxLength = 12000) {
  ...
  function collectDiagnosisContext(payload = {}) {
    ...
    const files = listCandidateLogFiles(['auto', 'bot', 'plugin', 'all'].includes(source) ? source : 'auto');
    const selectedFiles = [];
    const sections = [];
    let totalHitCount = 0;
