...
export function createFeatureConfigConsole(options = {}) {
  ...
  async function toggleFeatureState(feature, enabled) {
    const allConfigs = getAllConfigs() || {};
    if (['ai', 'privateAi', 'music', 'rss', 'auth', 'welcome', 'groupManagement', 'groupTitle', 'poke', 'status', 'webConsole', 'voiceModel'].includes(feature)) {
  ...
  function getManagedFeatureKeys() {
    return ['poke', 'status', '60s', 'zwa', 'rss', 'help', 'welcome', 'faceReply', 'imageMonitor', 'ai', 'privateAi', 'music', 'voiceModel', 'auth', 'groupManagement', 'groupTitle', 'autoUpdate'];
  }
