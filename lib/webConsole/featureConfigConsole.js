...
export function createFeatureConfigConsole(options = {}) {
  ...
  async function toggleFeatureState(feature, enabled) {
    const allConfigs = getAllConfigs() || {};
    if (['ai', 'music', 'rss', 'auth', 'welcome', 'groupManagement', 'groupTitle', 'poke', 'status', 'webConsole'].includes(feature)) {
  ...
  function getManagedFeatureKeys() {
    return ['poke', 'status', '60s', 'zwa', 'rss', 'help', 'welcome', 'faceReply', 'imageMonitor', 'ai', 'music', 'auth', 'groupManagement', 'groupTitle', 'autoUpdate'];
  }
