import {
  ...
  PUBLIC_REPOSITORY_GIT_CONFIG_ARGS,
  PUBLIC_REPOSITORY_URLS,
} from '../system/publicRepository.js';
  ...
  async function buildVersionCheckPayload(options = {}) {
    ...
    versionCheckInFlight = (async () => {
      ...
      try {
        ...
        // 全程使用公共匿名 HTTPS。Windows 强制改用 Git 自带 OpenSSL，
        // 避免 Schannel 错误触发系统凭据；Gitee 不可用时才回退 GitHub。
        const repoUrls = PUBLIC_REPOSITORY_URLS;
        const remoteBranch = PUBLIC_REPOSITORY_BRANCH;
        let remote = null;
        let lastRemoteError = null;
        for (const url of repoUrls) {
          try {
            const remoteOutput = await readGitOutputRequired([
              ...PUBLIC_REPOSITORY_GIT_CONFIG_ARGS,
              'ls-remote',
              '--heads',
              url,
