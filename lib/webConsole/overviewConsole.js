export function createOverviewConsole(options = {}) {
  ...
  function normalizeGitRemoteUrl(value = '') {
    return String(value || '').trim().replace(/\/\/([^:@/\s]+):([^@/\s]+)@/g, '//$1:[hidden]@');
  }

  async function fetchGiteeReleaseNotes(remoteUrl = '', timeoutMs = 6000) {
    const match = String(remoteUrl).match(/gitee\.com[:/]([^/]+)\/([^/.]+?)(?:\.git)?(?:\/|$)/i);
    ...
    try {
      ...
      return {
        ...
        publishedAt: String(data.created_at || data.published_at || '').trim(),
      };
    } catch {
      return null;
    } finally {
  ...
  async function buildVersionCheckPayload(options = {}) {
    ...
    versionCheckInFlight = (async () => {
      ...
      try {
        ...
        branch = await readGitOutput(['branch', '--show-current'], 'main');
        remoteUrl = normalizeGitRemoteUrl(await readGitOutput(['config', '--get', 'remote.origin.url'], 'origin'));
        dirty = Boolean(await readGitOutput(['status', '--porcelain'], ''));

        const remoteBranch = branch || 'main';
        const remoteOutput = await readGitOutputRequired(['ls-remote', '--heads', 'origin', remoteBranch]);
        const remote = parseLsRemoteHead(remoteOutput);
        if (!remote.commit) {
          throw new Error(`远端分支 origin/${remoteBranch} 不存在或无法读取`);
        }

        const updateAvailable = remote.commit !== localCommit;
        let releaseNotes = null;
        if (updateAvailable) {
          releaseNotes = await fetchGiteeReleaseNotes(remoteUrl, Math.min(versionCheckTimeoutMs, 6000));
        }
