import { PUBLIC_REPOSITORY_BRANCH, PUBLIC_REPOSITORY_URL } from '../system/publicRepository.js';
  ...
  async function buildVersionCheckPayload(options = {}) {
    ...
    versionCheckInFlight = (async () => {
      ...
      try {
        ...
        remoteUrl = PUBLIC_REPOSITORY_URL;
        dirty = Boolean(await readGitOutput(['status', '--porcelain'], ''));

        const remoteBranch = PUBLIC_REPOSITORY_BRANCH;
        const remoteOutput = await readGitOutputRequired(['ls-remote', '--heads', PUBLIC_REPOSITORY_URL, remoteBranch]);
