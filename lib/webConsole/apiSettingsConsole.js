...
export function createApiSettingsConsole(options = {}) {
  ...
  // AI 主对话接入协议（openai/anthropic/grok），非法值回退 openai
  function normalizeAiProtocolValue(value) {
    const raw = String(value || '').trim().toLowerCase();
    if (raw === 'anthropic' || raw === 'claude' || raw === 'messages') return 'anthropic';
    if (raw === 'grok' || raw === 'gork' || raw === 'xai') return 'grok';
    return 'openai';
  }

  // 备用接口协议：空/非法值保留为 ''（= 跟随主接口），与主接口归一化语义不同
  function normalizeFallbackApiProtocolValue(value) {
    const raw = String(value || '').trim().toLowerCase();
    if (['openai', 'anthropic', 'grok'].includes(raw)) return raw;
    if (raw === 'claude' || raw === 'messages') return 'anthropic';
    if (raw === 'gork' || raw === 'xai') return 'grok';
    return '';
  }
  ...
  function normalizeSdWebUiConsoleConfig(source = {}) {
    const normalized = normalizeSdWebUiSettings({ sdWebUi: source });
    return {
      baseApi: normalized.baseApi,
      model: normalized.model,
      username: normalized.username,
      password: String(source.password || ''),
      samplerName: normalized.samplerName,
      scheduler: normalized.scheduler,
      steps: normalized.steps,
      cfgScale: normalized.cfgScale,
      width: normalized.width,
      height: normalized.height,
      negativePrompt: normalized.negativePrompt,
      seed: normalized.seed,
      denoisingStrength: normalized.denoisingStrength,
    };
  }

  function validateSdWebUiFields(source = {}, label = 'SD WebUI', errors = [], warnings = []) {
    const rawBaseApi = String(source.baseApi || '').trim();
    if (!rawBaseApi) {
      warnings.push(`${label} 地址未填写。`);
    } else if (!isHttpUrl(rawBaseApi)) {
      errors.push(`${label} 地址必须是 http(s) URL。`);
    }
    const width = Number(source.width);
    const height = Number(source.height);
    if (!Number.isFinite(width) || width < 64 || width > 4096 || width % 8 !== 0) {
      errors.push(`${label} 宽度必须是 64-4096 之间的 8 的倍数。`);
    }
    if (!Number.isFinite(height) || height < 64 || height > 4096 || height % 8 !== 0) {
      errors.push(`${label} 高度必须是 64-4096 之间的 8 的倍数。`);
    }
    const steps = Number(source.steps);
    if (!Number.isFinite(steps) || steps < 1 || steps > 150) errors.push(`${label} 采样步数必须在 1-150 之间。`);
    const cfgScale = Number(source.cfgScale);
    if (!Number.isFinite(cfgScale) || cfgScale < 1 || cfgScale > 30) errors.push(`${label} CFG 必须在 1-30 之间。`);
    const denoisingStrength = Number(source.denoisingStrength);
    if (!Number.isFinite(denoisingStrength) || denoisingStrength < 0 || denoisingStrength > 1) {
      errors.push(`${label} 重绘强度必须在 0-1 之间。`);
    }
  }
  ...
  function buildApiSettingsPayload() {
    ...
    const imageSdWebUi = aiConfig.imageConfig?.sdWebUi || {};
    const imageFallbackSdWebUi = imageFallback.sdWebUi || {};
    ...
    return {
      ai: {
        ...
        apiProtocol: normalizeAiProtocolValue(aiConfig.apiProtocol),
        ...
        fallbackApi: {
          ...
          apiProtocol: normalizeFallbackApiProtocolValue(aiFallback.apiProtocol),
          apiKey: '',
        ...
        imageConfig: {
          ...
          webSearch: aiConfig.imageConfig?.webSearch === true,
          sdWebUi: {
            ...normalizeSdWebUiConsoleConfig(imageSdWebUi),
            password: '',
            passwordConfigured: hasConfiguredSecret(imageSdWebUi.password),
          },
          ...
          fallbackApi: {
            ...
            webSearch: imageFallback.webSearch === true,
            sdWebUi: {
              ...normalizeSdWebUiConsoleConfig(imageFallbackSdWebUi),
              password: '',
              passwordConfigured: hasConfiguredSecret(imageFallbackSdWebUi.password),
            },
  ...
  function precheckApiSettings(payload = {}) {
    ...
    if (imageConfig.enabled !== false) {
      ...
      } else if (!isSdWebUiImageMode(imageMode) && /^[234]K$/i.test(imageSize)) {
      ...
      if (imageMode === 'jimeng') {
        ...
        validateSdWebUiFields(imageConfig.sdWebUi || {}, 'SD WebUI', errors, warnings);
      } else {
      ...
      if (imageFallback.enabled === true) {
        ...
        } else if (isSdWebUiImageMode(fallbackImageMode)) {
          validateSdWebUiFields(imageFallback.sdWebUi || {}, '备用 SD WebUI', errors, warnings);
          const primarySdUrl = isSdWebUiImageMode(imageMode) ? imageConfig.sdWebUi?.baseApi : imageConfig.baseApi;
          if (sameEndpoint(primarySdUrl, imageFallback.sdWebUi?.baseApi)) {
            warnings.push('备用 SD WebUI 与主图像接口地址相同，生图故障时可能无法真正兜底。');
          }
        } else {
        ...
        if (!isSdWebUiImageMode(fallbackImageMode)
          && !hasSecretForPrecheck(imageFallback.apiKey, imageFallback.preserveApiKey, currentImageFallback.apiKey)) {
    ...
    addChange(changes, 'AI 主对话接入协议', normalizeAiProtocolValue(currentAi.apiProtocol), normalizeAiProtocolValue(ai.apiProtocol));
    ...
    addChange(changes, 'AI 主对话备用接入协议', normalizeFallbackApiProtocolValue(currentAiFallback.apiProtocol) || '跟随主接口', normalizeFallbackApiProtocolValue(aiFallback.apiProtocol) || '跟随主接口');
    ...
    addChange(changes, 'Agent Plan 联网搜索', currentImageConfig.webSearch === true, imageConfig.webSearch === true);
    addChange(changes, 'SD WebUI 地址', currentImageConfig.sdWebUi?.baseApi, imageConfig.sdWebUi?.baseApi);
    addChange(changes, 'SD WebUI 模型', currentImageConfig.sdWebUi?.model, imageConfig.sdWebUi?.model);
    addChange(changes, 'SD WebUI 采样器', currentImageConfig.sdWebUi?.samplerName, imageConfig.sdWebUi?.samplerName);
    addChange(changes, 'SD WebUI 调度器', currentImageConfig.sdWebUi?.scheduler, imageConfig.sdWebUi?.scheduler);
    addChange(changes, 'SD WebUI 采样步数', currentImageConfig.sdWebUi?.steps, imageConfig.sdWebUi?.steps);
    addChange(changes, 'SD WebUI CFG', currentImageConfig.sdWebUi?.cfgScale, imageConfig.sdWebUi?.cfgScale);
    addChange(changes, 'SD WebUI 图片宽度', currentImageConfig.sdWebUi?.width, imageConfig.sdWebUi?.width);
    addChange(changes, 'SD WebUI 图片高度', currentImageConfig.sdWebUi?.height, imageConfig.sdWebUi?.height);
    addChange(changes, 'SD WebUI 随机种子', currentImageConfig.sdWebUi?.seed, imageConfig.sdWebUi?.seed);
    addChange(changes, 'SD WebUI 重绘强度', currentImageConfig.sdWebUi?.denoisingStrength, imageConfig.sdWebUi?.denoisingStrength);
    if (String(imageConfig.sdWebUi?.password || '').trim()) {
      changes.push({ label: 'SD WebUI 认证密码', before: hasConfiguredSecret(currentImageConfig.sdWebUi?.password) ? '已配置' : '空', after: '将更新' });
    }
    ...
    addChange(changes, '备用 Agent Plan 联网搜索', currentImageFallback.webSearch === true, imageFallback.webSearch === true);
    addChange(changes, '备用 SD WebUI 地址', currentImageFallback.sdWebUi?.baseApi, imageFallback.sdWebUi?.baseApi);
    addChange(changes, '备用 SD WebUI 模型', currentImageFallback.sdWebUi?.model, imageFallback.sdWebUi?.model);
    addChange(changes, '备用 SD WebUI 采样器', currentImageFallback.sdWebUi?.samplerName, imageFallback.sdWebUi?.samplerName);
    addChange(changes, '备用 SD WebUI 采样步数', currentImageFallback.sdWebUi?.steps, imageFallback.sdWebUi?.steps);
    if (String(imageFallback.sdWebUi?.password || '').trim()) {
      changes.push({ label: '备用 SD WebUI 认证密码', before: hasConfiguredSecret(currentImageFallback.sdWebUi?.password) ? '已配置' : '空', after: '将更新' });
    }
  ...
  function buildApiSettingsConfigSnapshot(payload = {}) {
    ...
    if (payload.ai) {
      ...
      const nextAi = {
        ...
        apiProtocol: normalizeAiProtocolValue(payload.ai.apiProtocol),
      ...
      if (payload.ai.fallbackApi) {
        nextAi.fallbackApi = {
          ...
          apiProtocol: normalizeFallbackApiProtocolValue(payload.ai.fallbackApi.apiProtocol),
      ...
      if (payload.ai.imageConfig) {
        nextAi.imageConfig = {
          ...
          webSearch: payload.ai.imageConfig.webSearch === true,
        };
        if (payload.ai.imageConfig.sdWebUi) {
          nextAi.imageConfig.sdWebUi = {
            ...(currentAi.imageConfig?.sdWebUi || {}),
            ...normalizeSdWebUiConsoleConfig(payload.ai.imageConfig.sdWebUi),
            password: resolveSecretSaveValue(
              payload.ai.imageConfig.sdWebUi.password,
              currentAi.imageConfig?.sdWebUi?.password,
              payload.ai.imageConfig.sdWebUi.preservePassword === true,
            ),
          };
        }
        if (payload.ai.imageConfig.fallbackApi) {
          nextAi.imageConfig.fallbackApi = {
            ...
            webSearch: payload.ai.imageConfig.fallbackApi.webSearch === true,
            sdWebUi: {
              ...(currentAi.imageConfig?.fallbackApi?.sdWebUi || {}),
              ...normalizeSdWebUiConsoleConfig(payload.ai.imageConfig.fallbackApi.sdWebUi || {}),
              password: resolveSecretSaveValue(
                payload.ai.imageConfig.fallbackApi.sdWebUi?.password,
                currentAi.imageConfig?.fallbackApi?.sdWebUi?.password,
                payload.ai.imageConfig.fallbackApi.sdWebUi?.preservePassword === true,
              ),
            },
  ...
  async function testApiTargetConnection(target = 'ai-main', role = 'primary', draftPayload = null) {
    ...
    if (normalizedTarget === 'ai-image') {
      ...
      if (isSdWebUiImageMode(imageMode)) {
        const settings = normalizeSdWebUiSettings(active);
        return attachConfigSource(await probeHttpEndpoint({
          target: normalizedTarget,
          role: normalizedRole,
          label: normalizedRole === 'fallback' ? '备用 SD WebUI' : 'SD WebUI',
          url: buildSdWebUiApiUrl(settings.baseApi, '/sdapi/v1/sd-models'),
          timeoutMs: imageConfig.timeout,
          headers: buildSdWebUiAuthHeaders(settings),
        }));
      }
      if (isArkAgentPlanImageMode(imageMode)) {
        return attachConfigSource(await probeHttpEndpoint({
