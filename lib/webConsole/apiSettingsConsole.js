...
export function createApiSettingsConsole(options = {}) {
  ...
  function flattenConfig(value, prefix = '', result = new Map()) {
    ...
    result.set(prefix, {
      value: isSensitiveConfigPath(prefix) ? normalizeDiffValue(value) : normalizeDiffValue(value),
  ...
  function buildConfigSourcePayload(allConfigs = {}) {
    ...
    return {
      ...
      files: CONFIG_SOURCE_FILES.map(item => {
        ...
        const effectiveConfig = allConfigs[item.key] || runtimeConfig || {};
  ...
  function buildApiSettingsPayload() {
    ...
    return {
      ...
      configSource: buildConfigSourcePayload(allConfigs),
    };
  }
