  ...
  function precheckApiSettings(payload = {}) {
    ...
    if (imageConfig.enabled !== false) {
      ...
      if (isArkAgentPlanImageMode(imageMode)) {
        if (imageSize && !/^(?:1K|2K|4K|\d+x\d+)$/i.test(imageSize)) {
          errors.push('火山 Agent Plan 图像尺寸应为 1K、2K、4K 或 WIDTHxHEIGHT。');
        }
        if (!['png', 'jpeg'].includes(String(imageConfig.outputFormat || 'png').trim().toLowerCase())) {
          errors.push('火山 Agent Plan 输出格式必须是 png 或 jpeg。');
        }
        if (!['url', 'b64_json'].includes(String(imageConfig.responseFormat || 'url').trim().toLowerCase())) {
          errors.push('火山 Agent Plan 响应格式必须是 url 或 b64_json。');
        }
      } else if (/^[124]K$/i.test(imageSize)) {
        errors.push('1K、2K、4K 是火山 Agent Plan 专用尺寸，请切换 Agent Plan 模式或选择具体宽高。');
      }
      if (imageMode === 'jimeng') {
