...
export function createApiSettingsConsole(options = {}) {
  ...
  async function testApiConnection() {
    ...
    try {
      const response = await fetchFn(buildOpenAiCompatibleUrl(baseApi, '/v1/models'), {
        method: 'GET',
        headers: { Authorization: `Bearer ${apiKey}`, ...buildAiUserAgentHeaders(aiConfig) },
  ...
  async function testImageApiConnection() {
    ...
    const startedAt = Date.now();
    try {
      const requestConfig = { ...(allConfigs.ai || {}), ...imageConfig };
      const response = await fetchFn(buildOpenAiCompatibleUrl(baseApi, '/v1/models'), {
        method: 'GET',
        headers: { Authorization: `Bearer ${apiKey}`, ...buildAiUserAgentHeaders(requestConfig) },
        signal: AbortSignal.timeout(timeoutMs),
      });
      const latencyMs = Date.now() - startedAt;
      if (response.ok) {
  ...
  async function testImageMonitorApiConnection() {
    ...
    const startedAt = Date.now();
    try {
      const requestConfig = { ...(allConfigs.ai || {}), ...monitorConfig };
      const response = await fetchFn(buildOpenAiCompatibleUrl(baseApi, '/v1/models'), {
        method: 'GET',
        headers: { ...(apiKey ? { Authorization: `Bearer ${apiKey}` } : {}), ...buildAiUserAgentHeaders(requestConfig) },
        signal: AbortSignal.timeout(timeoutMs),
      });
      const latencyMs = Date.now() - startedAt;
      if (response.ok) {
