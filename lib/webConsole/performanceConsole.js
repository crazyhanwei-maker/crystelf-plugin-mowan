import os from 'os';
import { getApiCircuitBreakerSnapshot, resetApiCircuitBreakerState } from '../ai/apiCircuitBreaker.js';
...
function normalizeFallbackPolicy(config = {}) {
  return {
    ...
    failureThreshold: Math.min(10, Math.max(1, Math.round(toNumber(config.failureThreshold) || 2))),
    cooldownMs: Math.min(1800000, Math.max(30000, Math.round(toNumber(config.cooldownMs) || 300000))),
  };
}
...
function hasConfiguredSecret(config = {}, key = 'apiKey') {
  return hasText(config?.[key]) || config?.[`${key}Configured`] === true;
}

function buildConfiguredCircuitTargets(configs = {}) {
  const ai = configs.ai || {};
  const image = ai.imageConfig || {};
  const imageMonitor = configs.imageMonitor || {};
  const search = configs.coreConfig?.tools?.search || {};
  const meme = ai.memeConfig || {};
  const tts = configs.coreConfig?.tools?.tts || {};
  const items = [];
  const push = (item = {}) => {
    const policy = normalizeFallbackPolicy(item.fallback || {});
    if (!policy.enabled && item.includeWhenDisabled !== true) return;
    items.push({
      scene: item.scene,
      type: item.type,
      title: item.title,
      ...
      fallbackConfigured: Boolean(item.fallbackConfigured),
      primaryConfigured: Boolean(item.primaryConfigured),
    });
  };
  ...
  push({
    ...
    type: 'chat',
    title: 'AI 主对话',
    fallback: ai.fallbackApi || {},
    primaryConfigured: hasText(ai.baseApi) && hasConfiguredSecret(ai) && hasText(ai.modelType || ai.workingModel),
    fallbackConfigured: hasText(ai.fallbackApi?.baseApi) && hasConfiguredSecret(ai.fallbackApi || {}) && hasText(ai.fallbackApi?.modelType || ai.fallbackApi?.workingModel),
  });
  const imageFallback = image.fallbackApi || {};
  const imageFallbackMode = String(imageFallback.imageMode || image.imageMode || 'openai').trim();
  push({
    scene: 'image_generate',
    type: 'image_generation',
    title: '图像生成',
    fallback: imageFallback,
    primaryConfigured: hasText(image.model) && (
      image.imageMode === 'jimeng'
        ? hasText(image.jimengApiUrl)
        : (hasText(image.baseApi || ai.baseApi) && (hasConfiguredSecret(image) || hasConfiguredSecret(ai)))
    ),
    fallbackConfigured: imageFallbackMode === 'jimeng'
      ? hasText(imageFallback.jimengApiUrl)
      : (hasText(imageFallback.baseApi) && hasText(imageFallback.model) && hasConfiguredSecret(imageFallback)),
  });
  push({
    scene: 'image_monitor_review',
    type: 'image_monitor',
    title: '图片监控',
    fallback: imageMonitor.fallbackApi || {},
    primaryConfigured: imageMonitor.enabled === true && hasText(imageMonitor.apiBase) && hasConfiguredSecret(imageMonitor) && hasText(imageMonitor.model),
    fallbackConfigured: hasText(imageMonitor.fallbackApi?.apiBase) && hasConfiguredSecret(imageMonitor.fallbackApi || {}) && hasText(imageMonitor.fallbackApi?.model),
  });
  push({
    scene: 'search_web',
    type: 'search',
    title: '搜索工具',
    fallback: search.fallbackApi || {},
    primaryConfigured: search.enabled === true && hasText(search.apiUrl),
    fallbackConfigured: hasText(search.fallbackApi?.apiUrl),
  });
  push({
    scene: 'fetch_web_markdown',
    type: 'search',
    title: '网页读取',
    fallback: search.fallbackApi || {},
    primaryConfigured: search.enabled === true && hasText(search.markdownApiUrl) && hasText(search.markdownStatusUrl),
    fallbackConfigured: hasText(search.fallbackApi?.markdownApiUrl) && hasText(search.fallbackApi?.markdownStatusUrl),
  });
  push({
    scene: 'meme_random',
    type: 'meme',
    title: '表情包',
    fallback: meme.fallbackApi || {},
    primaryConfigured: hasText(meme.apiBase),
    fallbackConfigured: hasText(meme.fallbackApi?.apiBase),
  });
  push({
    scene: 'tts_synthesis',
    type: 'tts',
    title: '语音合成',
    fallback: tts.fallbackApi || {},
    primaryConfigured: hasText(tts.apiUrl),
    fallbackConfigured: hasText(tts.fallbackApi?.apiUrl || tts.fallbackApi?.modelsUrl),
  });
...
export function createPerformanceConsole(options = {}) {
  ...
  function buildPayload(options = {}) {
    ...
    return {
      ...
      apiQuality: {
        ...
        configuredCircuitTargets: buildConfiguredCircuitTargets(getAllConfigs()),
      },
      ai: {
  ...
  function resetCircuitBreaker(payload = {}) {
    const result = resetApiCircuitBreakerState({
      all: payload?.all === true,
      id: payload?.id || payload?.circuitId || '',
    });
    return {
      ...result,
      snapshot: getApiCircuitBreakerSnapshot(),
    };
  }
