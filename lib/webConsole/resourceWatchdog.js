// 资源水位看门狗：周期检查磁盘 / 进程内存 / Redis 内存，超过阈值时通过 QQ（persist 队列）+ 微信桥 + 邮件告警。
...
export function createResourceWatchdog(options = {}) {
  ...
  // 采集器可注入（测试用），默认读真实系统数据
  const collectors = options.collectors || {};
  const collectDisks = typeof collectors.disks === 'function' ? collectors.disks : collectDiskUsage;
  const collectMemory = typeof collectors.memory === 'function' ? collectors.memory : collectSystemMemoryPercent;
  const collectRedis = typeof collectors.redis === 'function' ? collectors.redis : collectRedisUsage;
  ...
  async function evaluate() {
    ...
    for (const disk of collectDisks()) {
    ...
    const redisUsage = await collectRedis();
  ...
  async function check() {
    ...
    if (alerts.length > 0) {
      ...
      if (!active) {
        ...
        notifyWechatBridge?.(buildAlertLines(kind, alerts), { label: '资源水位告警' });
        ...
        notifyWechatBridge?.(buildAlertLines(kind, alerts), { label: '资源水位重提醒' });
      }
      return;
    }
    ...
    if (state.activeAlerts.has(kind)) {
      ...
      notifyWechatBridge?.(buildRecoverLines(kind), { label: '资源水位恢复' });
    }
  }
