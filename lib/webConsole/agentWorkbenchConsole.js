    ...
    if (!agentModelVisionCache.map || agentModelVisionCache.mtimeMs !== mtimeMs) {
      ...
      for (const provider of Object.values(data)) {
        ...
        for (const [modelId, meta] of Object.entries(models)) {
          ...
          if (!map.has(shortId)) map.set(shortId, vision);
        }
      }
      agentModelVisionCache = { mtimeMs, map };
    }
    return agentModelVisionCache.map.has(key) ? agentModelVisionCache.map.get(key) : null;
  } catch {
    return null;
  }
}
...
export function createAgentWorkbenchConsole(options = {}) {
  ...
  async function runTask(task) {
    ...
    if (task.cancelRequested) {
      ...
      if (canAutoRetry) {
        ...
        addTaskEvent(task, 'warn', `模型返回了空响应（LLM 故障），已自动重试（第 ${task.retryCount} 次）`);
        persistTasks();
        emitTaskUpdate(task, 'task', true);
  ...
  async function maybeAutoCompactSession(task, server, openCodeQuery) {
    ...
    try {
      ...
      const after = await readSessionContextState(task).catch(() => ({ usage: 0, limit: state.limit, manual: state.manual }));
      task.contextUsage = after.usage;
      if (after.manual !== true) task.contextLimit = after.limit || (state.manual === true ? 0 : state.limit);
