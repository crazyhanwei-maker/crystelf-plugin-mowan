...
function getAgentRuntimeProfile(task = {}, mainApiConfig = {}) {
  // 权限档已下沉到按请求 agent（crystelf-read/write/full），运行时只按 API 配置区分；
  // 同一 API 下只读与完全访问复用同一个 OpenCode 进程。
  // 主 API 的协议/地址/密钥/模型必须参与池键：否则改配置后任务会复用旧配置的运行时，
  // 报「Model not found: crystelf-chat/<旧模型>」
  ...
  return crypto.createHash('sha256')
    .update(`${customApi.enabled === true}:${customApi.baseApi || ''}:${customApi.apiKey || ''}:${customApi.apiProtocol || ''}:${mainIdentity}`)
    .digest('hex')
    .slice(0, 12);
}
...
export function createAgentWorkbenchConsole(options = {}) {
  ...
  async function runTask(task) {
    ...
    if (task.providerId === 'opencode') {
      ...
      emitTaskUpdate(task);
      task.openCodeRuntimeProfile = getAgentRuntimeProfile(task, ConfigControl?.get?.('ai') || {});
