...
function buildSafetyPrompt(mode, prompt, options = {}) {
  ...
  const imageInstruction = agentWorkbenchImageDisplayEnabled
    ? '【图片展示规则】当任务涉及图片（截图、生成的图、下载的图片、工作区已有图片等）时，必须直接在回复中用 Markdown 图片语法把图片插入展示给用户，格式：![简短描述](相对路径)，例如 ![截图](temp/images/demo.png)。路径写相对于工作区根目录的形式，不要用绝对路径或盘符。只描述图片内容或只说“已保存到某路径”都不算展示，除非用户明确说不需要看图。'
    : '';
