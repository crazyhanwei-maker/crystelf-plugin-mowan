...
const AGENT_RUNTIME_IDLE_TIMEOUT_MS = 2 * 60 * 1000;
...
function getAgentRuntimeProfile(task = {}) {
  const access = isFullAccessMode(task.mode)
    ? 'full'
    : (isWriteMode(task.mode) ? 'write' : 'read');
  const customApi = task.customApi || {};
  const apiFingerprint = crypto.createHash('sha256')
    .update(`${customApi.enabled === true}:${customApi.baseApi || ''}:${customApi.apiKey || ''}`)
    .digest('hex')
    .slice(0, 12);
  return [
    access,
    task.allowNetwork === true ? 'network' : 'offline',
    task.allowAllDirectories === true ? 'all-directories' : 'workspace-only',
    task.allowTerminal === true ? 'terminal' : 'no-terminal',
    apiFingerprint,
  ].join('-');
}
...
function normalizeUsage(value = {}) {
  ...
  const input = Number(value.input ?? value.inputTokens ?? value.promptTokens ?? value.prompt_tokens ?? 0);
  const output = Number(value.output ?? value.outputTokens ?? value.completionTokens ?? value.completion_tokens ?? 0);
...
async function readProcessMemoryBytes(pid = 0) {
  ...
  const result = process.platform === 'win32'
    ? await runCapturedProcess('powershell.exe', [
      ...
      '-NonInteractive',
      '-Command',
      `$target = Get-Process -Id ${processId} -ErrorAction Stop; [Console]::Write($target.WorkingSet64)`,
    ], { timeoutMs: 3000, outputLimit: 200 })
    : await runCapturedProcess('ps', ['-o', 'rss=', '-p', String(processId)], { timeoutMs: 3000, outputLimit: 200 });
  if (result.code !== 0) return 0;
  const value = Number(String(result.stdout || '').trim().match(/\d+/)?.[0] || 0);
...
async function waitForOpenCodeSessionIdle(server, sessionId = '', directory = '', options = {}) {
  ...
  try {
    return await requestOpenCodeJson(
      server,
      `/api/session/${encodeURIComponent(sessionId)}/wait`,
      { method: 'POST', timeoutMs, signal },
    );
  } catch (error) {
    if (!/503|not available yet|service unavailable/i.test(String(error?.message || ''))) throw error;
  }
  const startedAt = Date.now();
  const query = `?directory=${encodeURIComponent(directory)}`;
  let observedBusy = false;
  while (Date.now() - startedAt < timeoutMs) {
    const statuses = await requestOpenCodeJson(server, `/session/status${query}`, {
    ...
    if (type === 'idle' || (!type && (observedBusy || Date.now() - startedAt >= 2000))) return true;
    await waitForAbortableDelay(300, signal);
  }
  const timeoutError = new Error('OpenCode 会话等待超时');
...
export function createAgentWorkbenchConsole(options = {}) {
  ...
  function emitTaskDelta(task, change = {}) {
    ...
    const pending = streamDeltaBuffers.get(task.id) || { delta: '', outputText: null, timer: null };
    if (change.outputText !== undefined) {
      pending.outputText = redactAgentText(change.outputText || '');
      pending.delta = '';
    } else if (pending.outputText === null && change.delta) {
  ...
  async function runTask(task) {
    ...
    const opencodeAgent = isFullAccessMode(task.mode)
      ? 'build'
      : normalizeOpenCodeAgent(task.agent, isWriteMode(task.mode) ? 'build' : 'plan');
    ...
    const refreshPendingPermissions = async () => {
      ...
      try {
        ...
        if (task.opencodeSessionId && (!liveMessagesRead || nativeEventStreamFailed)) {
          ...
          const liveOutput = getLatestOpenCodeAssistantOutput(messages, task.opencodeSessionId, previousOpenCodeMessageIds);
          const liveText = parseOpenCodeTurn(liveOutput).content || liveOutput;
          liveMessagesRead = true;
          if (liveText && liveText !== task.liveOutputText) {
            const previousText = task.liveOutputText || '';
            task.liveOutputText = liveText;
            task.updatedAt = new Date().toISOString();
            if (previousText && liveText.startsWith(previousText)) {
              emitTaskDelta(task, { delta: liveText.slice(previousText.length) });
            } else {
              emitTaskDelta(task, { outputText: liveText });
            }
          }
        }
      } catch (error) {
    ...
    const nativeEventStream = attachedServer
      ? startOpenCodeEventStream(attachedServer, task.workspacePath, event => {
        ...
        if (type === 'message.part.delta') {
          const sessionId = String(properties.sessionID || properties.sessionId || '').trim();
          const field = String(properties.field || '').trim().toLowerCase();
          const delta = typeof properties.delta === 'string' ? properties.delta : '';
          const isTextDelta = !field || field === 'text' || field === 'content' || field.endsWith('.text');
          if ((!sessionId || sessionId === task.opencodeSessionId) && isTextDelta && delta) {
            task.liveOutputText = appendLimited(task.liveOutputText, delta);
            task.updatedAt = new Date().toISOString();
            emitTaskDelta(task, { delta });
          }
          return;
        }
        ...
        try {
          ...
          while (true) {
            ...
            await waitForOpenCodeSessionIdle(
              ...
              task.workspacePath,
              { timeoutMs: task.timeoutMs, signal: requestAbort.signal },
            );
            task.followUpClosing = true;
  ...
  async function readSessionContextState(task) {
    if (!task?.opencodeSessionId) {
      return { usage: 0, limit: Math.max(0, Number(task?.contextLimit || 0)) };
    }
    const messageResult = await requestTaskOpenCode(
      task,
      `/session/${encodeURIComponent(task.opencodeSessionId)}/message?limit=20`,
  ...
  async function maybeAutoCompactSession(task, server, openCodeQuery) {
    ...
    try {
      ...
      const body = modelSelection
        ? { providerID: modelSelection.providerID, modelID: modelSelection.modelID, auto: false }
        : { auto: true };
      await requestOpenCodeJson(
        server,
        `/session/${encodeURIComponent(task.opencodeSessionId)}/summarize${openCodeQuery}`,
        { method: 'POST', body, timeoutMs: task.timeoutMs },
      );
      task.lastCompactedAt = new Date().toISOString();
      const after = await readSessionContextState(task).catch(() => ({ usage: 0, limit: state.limit }));
      task.contextUsage = after.usage;
      task.contextLimit = after.limit || state.limit;
      addTaskEvent(task, 'success', `上下文压缩完成，当前约 ${task.contextUsage} Token`);
      persistTasks();
      emitTaskUpdate(task, 'task', true);
    } catch (error) {
  ...
  async function manageSession(taskId = '', action = '', payload = {}) {
    ...
    if (action === 'summarize') {
      ...
      const body = modelSelection
        ? { providerID: modelSelection.providerID, modelID: modelSelection.modelID, auto: false }
        : { auto: true };
      await requestTaskOpenCode(task, queryPath('/summarize'), {
        method: 'POST', body, timeoutMs: task.timeoutMs,
      });
      addTaskEvent(task, 'success', modelSelection
        ? 'OpenCode 会话上下文已压缩'
        : 'OpenCode 会话上下文已压缩（使用默认模型）');
  ...
  async function listOpenCodeRuntimes() {
    ...
    const items = await Promise.all(runtimes.map(async runtime => {
      ...
      return {
        ...
        pid: Math.max(0, Number(runtime.server?.child?.pid || 0)),
        ...
        memoryBytes: await readProcessMemoryBytes(runtime.server?.child?.pid),
      };
    }));
    return {
      success: true,
