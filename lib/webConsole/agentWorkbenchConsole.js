...
// 198 实测 opencode --version 冷/热都要 25~27s（超售 VPS + 178MB 二进制），15s 必超时误报「CLI 无法启动」
const PROVIDER_PROBE_TIMEOUT_MS = 60000;
// 探测失败只短缓存 1 分钟：启动期一次超时不应把供应商拉黑 5 分钟
const PROVIDER_PROBE_FAILURE_TTL_MS = 60 * 1000;
...
export function getAgentRuntimeProfile(task = {}, mainApiConfig = {}, modelProviders = []) {
  // 权限档已下沉到按请求 agent（crystelf-read/write/full），运行时只按 API 配置区分；
  // 同一 API 下只读与完全访问复用同一个 OpenCode 进程。
  // 池键必须覆盖 OPENCODE_CONFIG_CONTENT 的全部动态输入，否则切模型/改供应商列表后
  // 任务复用旧进程，报「ProviderModelNotFoundError: Model not found: crystelf-chat/<模型>」：
  // - 主 API 的协议/地址/密钥/模型（配置面板改动）
  // - 任务的模型选择（工作台弹层切换，不改主 API 配置）
  // - 工作台自定义供应商列表（models 注册表 + custom-* 供应商声明）
  const customApi = task.customApi || {};
  const mainIdentity = customApi.enabled === true
    ? ''
    : [
        mainApiConfig.apiProtocol || '',
        mainApiConfig.baseApi || '',
        mainApiConfig.apiKey || '',
        mainApiConfig.workingModel || mainApiConfig.modelType || '',
      ].join(':');
  const modelIdentity = `${task.model || ''}@${task.modelProviderId || ''}`;
  const providersIdentity = JSON.stringify(Array.isArray(modelProviders) ? modelProviders : []);
  return crypto.createHash('sha256')
    .update(`${customApi.enabled === true}:${customApi.baseApi || ''}:${customApi.apiKey || ''}:${customApi.apiProtocol || ''}:${mainIdentity}:${modelIdentity}:${providersIdentity}`)
    .digest('hex')
    .slice(0, 12);
}

// 思考档位智能：给定模型+供应商（可选），返回下拉应显示的档位与协议语义。
// 档位来源优先级：bundled 注册 variants（任务实际可用的）→ models.dev reasoning_options（表驱动）→ 全档兜底。
// 协议语义：anthropic → thinking 预算换算；openai/grok → reasoningEffort 原值。
export function buildAgentVariantIntelligence(options = {}) {
  const model = String(options.model || '').trim();
  const providerId = String(options.providerId || '').trim();
  const isAnthropicProvider = providerId.startsWith('custom-')
    ? normalizeAiProtocol(String(options.providerProtocol || '').toLowerCase()) === 'anthropic'
    : normalizeAiProtocol(String(options.mainProtocol || '').toLowerCase()) === 'anthropic';
  const effortLabels = { minimal: '最低', low: '低', medium: '中', high: '高', xhigh: '极高', max: '最大' };
  const thinkingBudgets = { minimal: 0, low: 1024, medium: 4096, high: 12288, xhigh: 24576, max: 24576 };
  const catalogOptions = lookupAgentModelReasoningOptions(model);
  // 查表结论（模型在 models.dev 中的档位声明；查不到 = null，无法断言不支持）
  const catalogEfforts = catalogOptions?.find(item => item.type === 'effort')?.values || null;
  const catalogBudgetMin = catalogOptions?.find(item => item.type === 'budget_tokens')?.min || 0;
  const supportedEfforts = catalogEfforts && catalogEfforts.length ? catalogEfforts : null;
  // 下拉档位：查到表就收缩，查不到保持六档（自定义通道永远保留，新模型不受表更新约束）
  const variantValues = supportedEfforts || Object.keys(effortLabels);
  const variants = [{ value: '', label: '默认' }]
    .concat(variantValues.map(value => ({ value, label: effortLabels[value] || value })))
    .map(item => {
      if (isAnthropicProvider) {
        const budget = item.value ? (thinkingBudgets[item.value] || 0) : 0;
        // budget_tokens 表下限：低于下限的档位按最小合法预算兜底（Anthropic 规范最小 1024）
        const clamped = catalogBudgetMin && budget && budget < catalogBudgetMin ? catalogBudgetMin : budget;
        return { ...item, semantics: item.value ? `thinking 预算 ${clamped || '按 SDK 默认'}` : '由模型决定' };
      }
      return { ...item, semantics: item.value ? `reasoningEffort: ${item.value}` : '由模型决定' };
    });
  return {
    model,
    providerId,
    protocol: isAnthropicProvider ? 'anthropic' : 'openai',
    reasoningKnown: Boolean(catalogOptions),
    reasoningSupported: catalogOptions ? catalogOptions.length > 0 : null,
    customEnabled: true,
    budgetMin: catalogBudgetMin,
    variants,
  };
}
...
function normalizeModelVariant(value = '') {
  ...
  if (!variant) return '';
  // 自定义思考值：除档位名外允许纯数字（anthropic thinking 预算按 token 数透传），
  // 新模型的思考参数不用等 models.dev/适配层更新，用户直接填模型认识的值即可
  if (!/^[A-Za-z0-9._:/-]{1,80}$/.test(variant) && !/^\d{1,9}$/.test(variant)) {
    throw createHttpError(400, '思考等级只能包含字母、数字、点、斜杠、冒号、下划线和短横线，或直接填 thinking 预算数字', 'AGENT_MODEL_VARIANT_INVALID');
  }
  return variant;
}
...
// 工作台多供应商模型列表：{name, baseApi, apiKey, models:[]}
// 读侧 apiKey 掩码显示，保存时「留空保持原值」
function normalizeModelProviderEntry(entry = {}, previous = {}) {
  const name = normalizeSingleLine(entry?.name || previous?.name || '', 40);
  const baseApi = normalizeSingleLine(entry?.baseApi || previous?.baseApi || '', 500);
  const rawKey = String(entry?.apiKey ?? '').trim();
  const apiKey = rawKey ? rawKey.replace(/[\r\n]/g, '').slice(0, 500) : String(previous?.apiKey || '').trim();
  // 模型条目：字符串 = 仅模型名（元数据走 models.dev 查表）；对象 = {name, context, vision} 覆盖
  const models = (Array.isArray(entry?.models) ? entry.models : [])
    .map(item => {
      if (typeof item === 'string') {
        const name = normalizeModel(item);
        return name ? { name } : null;
      }
      const name = normalizeModel(item?.name);
      if (!name) return null;
      const context = Math.max(0, Math.round(Number(item?.context) || 0));
      const output = Math.max(0, Math.round(Number(item?.output) || 0));
      const vision = item?.vision === true ? true : item?.vision === false ? false : null;
      return {
        name,
        ...(context > 0 ? { context } : {}),
        ...(output > 0 ? { output } : {}),
    ...
    .filter(Boolean)
    .slice(0, 30);
  const rawProtocol = String(entry?.apiProtocol ?? previous?.apiProtocol ?? '').trim().toLowerCase();
  const apiProtocol = ['openai', 'anthropic', 'grok'].includes(rawProtocol)
    ? rawProtocol
    : (rawProtocol === 'claude' || rawProtocol === 'messages' ? 'anthropic' : rawProtocol === 'gork' || rawProtocol === 'xai' ? 'grok' : '');
  return {
    name,
    baseApi,
    apiKey,
...
function normalizeModelProviderList(value = [], previousList = []) {
  const previousByName = new Map(previousList.map(item => [item?.name, item]));
  return (Array.isArray(value) ? value : [])
    .map((entry, index) => normalizeModelProviderEntry(entry, previousByName.get(entry?.name) || previousList[index] || {}))
    .filter(item => item.name && item.baseApi && item.models.length > 0)
    .slice(0, 8);
}

// 给前端的供应商列表：不带 apiKey（掩码为「已配置/未配置」），密钥只保存在服务端
function maskModelProviderList(providers = []) {
  return (Array.isArray(providers) ? providers : []).map(item => ({
    name: item.name,
...
function normalizeCustomAgentApi(value = {}) {
  ...
  const rawProtocol = String(value?.apiProtocol || '').trim().toLowerCase();
  const apiProtocol = ['openai', 'anthropic', 'grok'].includes(rawProtocol)
    ? rawProtocol
    : (rawProtocol === 'claude' || rawProtocol === 'messages' ? 'anthropic' : rawProtocol === 'gork' || rawProtocol === 'xai' ? 'grok' : '');
  return {
...
// 模型库缓存（models.dev）→ Map<短模型名, { vision, contextLimit, reasoningOptions }>
// reasoningOptions：[{ type:'effort', values:[...] } / { type:'budget_tokens', min }]，档位自动收缩与协议语义换算用
    ...
    if (!agentModelVisionCache.map || agentModelVisionCache.mtimeMs !== mtimeMs) {
      ...
      for (const provider of Object.values(data)) {
        ...
        for (const [modelId, meta] of Object.entries(models)) {
          ...
          const reasoningOptions = Array.isArray(meta.reasoning_options)
            ? meta.reasoning_options
              .filter(item => item && typeof item === 'object')
              .map(item => ({
                type: normalizeSingleLine(item.type, 40).toLowerCase(),
                values: Array.isArray(item.values) ? item.values.map(value => normalizeSingleLine(value, 60).toLowerCase()).filter(Boolean) : [],
                min: Math.max(0, Number(item.min || 0)),
              }))
              .filter(item => item.type && (item.values.length || item.min > 0))
            : [];
          let entry = map.get(shortId);
          if (!entry) {
            entry = { vision: false, contextLimit: 0, decls: new Map() };
            map.set(shortId, entry);
          }
          // 短名跨网关重复严重（同名模型被几十个网关收录，个别乱标七档）：
          // 档位声明按出现次数取众数，乱标网关自动被多数派淹没
          entry.vision = entry.vision || vision;
          if (!entry.contextLimit && contextLimit) entry.contextLimit = contextLimit;
          if (reasoningOptions.length) {
            const key = JSON.stringify(reasoningOptions);
            entry.decls.set(key, (entry.decls.get(key) || 0) + 1);
          }
        }
      }
      for (const entry of map.values()) {
        let best = null;
        let bestCount = -1;
        for (const [key, count] of entry.decls) {
          if (count > bestCount) {
            bestCount = count;
            best = key;
          }
        }
        entry.reasoningOptions = best ? JSON.parse(best) : [];
        delete entry.decls;
      }
...
// 查表：模型在 models.dev 中的推理档位声明（无结论返回 null）
export function lookupAgentModelReasoningOptions(modelName = '') {
  const key = String(modelName || '').trim().toLowerCase();
  if (!key) return null;
  const map = getAgentModelMetaMap();
  if (!map || !map.has(key)) return null;
  return map.get(key).reasoningOptions || [];
}

// 查表：模型徽章（视觉/上下文窗口），供管理模型的模型列表展示；未知模型 known=false
export function buildAgentModelBadges(names = []) {
  const map = getAgentModelMetaMap();
  ...
  for (const name of list) {
    const meta = map?.get(name.toLowerCase()) || null;
    badges[name] = {
      known: Boolean(meta),
      vision: meta ? meta.vision === true : null,
      contextLimit: meta ? Math.max(0, Number(meta.contextLimit || 0)) : 0,
    };
  }
  return { badges };
}
...
// 用户消息入库用的附件列表：保留图片 data URL，前端会话里渲染缩略图预览
function attachmentMessageList(attachments = []) {
  return (Array.isArray(attachments) ? attachments : []).slice(0, AGENT_ATTACHMENT_LIMIT).map(item => ({
    name: normalizeSingleLine(item.name || '附件', 180),
    mime: normalizeSingleLine(item.mime || 'application/octet-stream', 120),
    size: Math.max(0, Number(item.size || 0)),
    url: /^image\//i.test(String(item.mime || '')) && /^data:image\//i.test(String(item.url || '')) ? String(item.url) : '',
  }));
}
...
export function normalizeAgentWorkbenchConfig(value = {}) {
  return {
    ...
    modelProviders: normalizeModelProviderList(value?.modelProviders, []),
...
function buildSafetyPrompt(mode, prompt, options = {}) {
  ...
  const directoryInstruction = allowAllDirectories
    ...
    : '严禁访问当前工作目录以外的路径。';
  const questionInstruction = '【提问规则】当任务存在多种可行方案、信息不足或需要用户确认方向时，必须调用 question 工具向用户提问（给出 2-4 个带简短说明的选项），用户会在控制台看到选择卡片并直接点选；不要在纯文本回复里列出"方案A/方案B"让用户打字回答。';
...
const AGENT_EMPTY_BLOB_HASH = 'e69de29bb2d1d6434b8b29ae775ad8c2e48c5391';

// 任务基线：把任务开始时所有已改动文件的内容写入 git 对象库（不触碰索引/工作树），
// 记录每文件 blob 哈希。结束时只有 blob 变化的文件才算 agent 本轮改动，
// 部署、行尾翻转、外部编辑等与任务无关的变更不会混入文件改动面板。
let emptyBlobHashCache = '';
// 空内容 blob：新建/删除文件的 diff 端点（对象库缺该对象时惰性创建）
async function ensureEmptyBlobHash(workspacePath = '') {
  if (emptyBlobHashCache) return emptyBlobHashCache;
  const devNull = process.platform === 'win32' ? 'NUL' : '/dev/null';
  const result = await runCapturedProcess('git', ['hash-object', '-w', '--', devNull], {
    cwd: workspacePath,
    timeoutMs: GIT_STATUS_TIMEOUT_MS,
    outputLimit: 1000,
  }).catch(() => null);
  emptyBlobHashCache = result && result.code === 0 ? (result.stdout || '').trim() : 'e69de29bb2d1d6434b8b29ae775ad8c2e48c5391';
  return emptyBlobHashCache;
}

export async function captureTaskGitBaseline(workspacePath = '', statusText = '') {
  const entries = parseGitChangedFiles(statusText)
    .map(item => ({ path: item.path, status: item.status }))
    .slice(0, 200);
  if (!entries.length) return { available: false, files: [] };
  const result = await runCapturedProcess('git', ['hash-object', '-w', '--', ...entries.map(item => item.path)], {
    cwd: workspacePath,
    timeoutMs: GIT_STATUS_TIMEOUT_MS,
    outputLimit: 4000,
  }).catch(() => null);
  if (!result || result.code !== 0) return { available: false, files: [] };
  const hashes = (result.stdout || '').split(/\r?\n/).map(item => item.trim()).filter(Boolean);
  if (hashes.length !== entries.length) return { available: false, files: [] };
  return { available: true, files: entries.map((item, index) => ({ path: item.path, status: item.status, blob: hashes[index] })) };
}

async function hashWorktreeBlob(workspacePath = '', filePath = '') {
  const result = await runCapturedProcess('git', ['hash-object', '-w', '--', filePath], {
    cwd: workspacePath,
    timeoutMs: GIT_STATUS_TIMEOUT_MS,
    outputLimit: 2000,
  }).catch(() => null);
...
async function headBlobOfWorktreeFile(workspacePath = '', filePath = '') {
  const result = await runCapturedProcess('git', ['rev-parse', `HEAD:${filePath}`], {
    cwd: workspacePath,
    timeoutMs: GIT_STATUS_TIMEOUT_MS,
    outputLimit: 2000,
  }).catch(() => null);
...
function buildPerTaskDiffStat(changedFiles = []) {
  const files = Array.isArray(changedFiles) ? changedFiles : [];
  let totalAdd = 0;
  let totalDel = 0;
  const lines = files.map(item => {
    const add = Math.max(0, Number(item.additions || 0));
    const del = Math.max(0, Number(item.deletions || 0));
    totalAdd += add;
    totalDel += del;
    return `${item.path} | ${Math.max(1, add + del)} ${'+'.repeat(Math.min(30, add))}${'-'.repeat(Math.min(30, del))}`;
  });
  lines.push(`${files.length} 个文件更改，${totalAdd} 插入(+)，${totalDel} 删除(-)`);
  return lines.join('\n');
}

// 结束时过滤：相对任务基线内容有变化的文件才算本轮改动。
// 基线干净的文件对照 HEAD blob，基线已有改动的文件对照其基线 blob。
export async function filterChangedFilesSinceBaseline(workspacePath = '', baseline = {}, audit = {}) {
  if (!baseline?.available) {
    return { changedFiles: audit.changedFiles, diffStat: audit.diffStat, diffText: audit.diffText, baselineFiltered: false };
  }
  ...
  for (const file of audit.changedFiles || []) {
    ...
    const currentBlob = await hashWorktreeBlob(workspacePath, file.path);
    const baseBlob = base?.blob || '';
    if (baseBlob && currentBlob && baseBlob === currentBlob) continue;
    const a = baseBlob || await headBlobOfWorktreeFile(workspacePath, file.path) || await ensureEmptyBlobHash(workspacePath);
    const b = currentBlob || await ensureEmptyBlobHash(workspacePath);
    const [numResult, patchResult] = await Promise.all([
      runCapturedProcess('git', ['diff', '--numstat', a, b], { cwd: workspacePath, timeoutMs: GIT_STATUS_TIMEOUT_MS, outputLimit: 2000 }).catch(() => null),
      runCapturedProcess('git', ['diff', '--no-ext-diff', '--unified=3', a, b], { cwd: workspacePath, timeoutMs: GIT_STATUS_TIMEOUT_MS, outputLimit: AGENT_DIFF_LIMIT }).catch(() => null),
    ]);
    const numFirst = (numResult?.stdout || '').split('\n')[0] || '';
    const numMatch = numFirst.match(/^(\d+|-)\t(\d+|-)\t/);
    let fileAdd = Math.max(0, Number(file.additions || 0));
    let fileDel = Math.max(0, Number(file.deletions || 0));
    if (numMatch) {
      fileAdd = numMatch[1] === '-' ? 0 : Number(numMatch[1]);
      fileDel = numMatch[2] === '-' ? 0 : Number(numMatch[2]);
    }
    let patchText = (patchResult?.stdout || '').trim();
    if (patchText) {
      patchText = `diff --git a/${file.path} b/${file.path}\n` + patchText.replace(/^diff --git .*\n/, '');
      patchText = patchText
        .replace(/^--- a\/[0-9a-f]{7,40}$/m, `--- a/${file.path}`)
        .replace(/^\+\+\+ b\/[0-9a-f]{7,40}$/m, `+++ b/${file.path}`);
    }
    diffChunks.push(patchText);
    changedFiles.push({ ...file, additions: fileAdd, deletions: fileDel });
  }
  const diffStat = buildPerTaskDiffStat(changedFiles);
  const untrackedNote = changedFiles.some(item => item.status === '??')
    ? `\n\n未跟踪的新文件（Git diff 不包含内容）：\n${changedFiles.filter(item => item.status === '??').map(item => `- ${item.path}`).join('\n')}`
    : '';
  const diffText = `${diffChunks.join('\n\n')}${untrackedNote}`.trim();
  return { changedFiles, diffStat, diffText, baselineFiltered: true };
}
...
function resetTaskTurnState(task) {
  ...
  task.changedFiles = [];  task.gitBaseline = null;
  task.diffStat = '';
  task.diffText = '';
}
...
function serializeAgentMessage(message = {}) {
  return {
    ...
    attachments: Array.isArray(message.attachments) ? message.attachments.slice(0, AGENT_ATTACHMENT_LIMIT).map(item => ({
      ...
      mime: normalizeSingleLine(item.mime || 'application/octet-stream', 120),
      size: Math.max(0, Number(item.size || 0)),
      // 图片附件回传 data URL 供前端缩略图预览；与工具图片同样 1.6M 字符上限控制载荷
      url: /^image\//i.test(String(item.mime || '')) && /^data:image\//i.test(String(item.url || '')) && String(item.url).length <= 1600000 ? String(item.url) : '',
    })) : [],
...
function serializeTask(task = {}, options = {}) {
  ...
  const result = {
    ...
    customApi: {
      ...
      apiProtocol: task.customApi?.apiProtocol || '',
    ...
    compacting: task.compacting === true,
...
export function createAgentWorkbenchConsole(options = {}) {
  ...
  // 事件风暴期间 persistTasks 会被高频触发：防抖 500ms 合并为一次落盘，
  // 并用「临时文件 + rename」原子写，避免写一半崩溃导致整个会话库解析失败
  let tasksStoreWriteTimer = null;
  function persistTasks() {
    if (tasksStoreWriteTimer) return;
    tasksStoreWriteTimer = setTimeout(() => {
      tasksStoreWriteTimer = null;
      writeTasksStore();
    }, 500);
  }

  function writeTasksStore() {
    try {
  ...
  async function probeProvider(providerId, force = false) {
    ...
    const cacheTtl = cached?.value?.available === false ? PROVIDER_PROBE_FAILURE_TTL_MS : PROVIDER_PROBE_TTL_MS;
    if (!force && cached && Date.now() - cached.checkedAtMs < cacheTtl) return cached.value;
  ...
  // 插件目录相对 file-browser 根（process.cwd()）的前缀：198 上为 plugins/crystelf-plugin，cwd 即插件目录时为空串
  function toPluginFilePrefix() {
    try {
      const relative = path.relative(process.cwd(), Path.root).replace(/\\/g, '/').replace(/\/+$/, '');
  ...
  async function buildPayload(options = {}) {
    ...
    const publicConfig = {
      ...config,
      // 主对话接入协议：前端思考档位智能按它换算语义（anthropic → thinking 预算）
      apiProtocol: normalizeAiProtocol(String(ConfigControl?.get?.('ai')?.apiProtocol || '')),
      customApi: {
        ...config.customApi,
        apiKey: config.customApi.apiKey ? '已配置' : '',
      },
      modelProviders: maskModelProviderList(config.modelProviders),
    ...
    return {
      ...
      data: {
        ...
        // 文件浏览器根是进程 cwd（Yunzai 根），工作台「打开」传的是插件相对路径，前端按此前缀拼接
        pluginFilePrefix: toPluginFilePrefix(),
        config: publicConfig,
  ...
  async function saveConfig(payload = {}) {
    ...
    // 供应商列表：前端不带明文密钥，按名称对齐旧条目，apiKey 留空则保留原值
    nextConfig.modelProviders = normalizeModelProviderList(payload.modelProviders, currentConfig.modelProviders);
  ...
  async function enqueueTaskFollowUp(task, payload = {}) {
    ...
    task.messages.push({
      ...
      status: 'queued',
      delivery,
      attachments: attachmentMessageList(attachments),
    });
    trimTaskMessages(task);
  ...
  async function runTask(task) {
    ...
    task.workspaceChanged = false;
    task.changedFiles = [];
    task.diffStat = '';    task.gitBaseline = null;
    task.diffText = '';
    task.pendingPermissions = [];
    task.pendingQuestions = [];
    task.status = 'running';
    ...
    task.gitCleanBefore = beforeGitStatus === '';    task.gitBaseline = await captureTaskGitBaseline(task.workspacePath, beforeGitStatus);
    ...
    // 工作台运行时配置在任务启动与池键计算两处共用（供应商列表参与池键）
    const workbenchRuntimeConfig = normalizeAgentWorkbenchConfig(ConfigControl?.get?.('coreConfig')?.tools?.agentWorkbench || {});
    if (task.providerId === 'opencode' && resolveBundledOpenCodeCommand({ rootPath: Path.root })) {
      const bundledRuntime = buildBundledOpenCodeEnvironment({
        aiConfig: ConfigControl?.get?.('ai') || {},
        model: task.model,
        requestedProviderId: task.modelProviderId,
        modelProviders: workbenchRuntimeConfig.modelProviders,
        customVariantValue: task.variant,
        ...
        allowAllDirectories: task.allowAllDirectories === true,
        allowTerminal: task.allowTerminal === true,
        nativeConfig: workbenchRuntimeConfig.nativeConfig,
      });
    ...
    if (task.providerId === 'opencode') {
      ...
      emitTaskUpdate(task);
      task.openCodeRuntimeProfile = getAgentRuntimeProfile(task, ConfigControl?.get?.('ai') || {}, workbenchRuntimeConfig.modelProviders);
    ...
    const perTaskChanges = await filterChangedFilesSinceBaseline(task.workspacePath, task.gitBaseline, changeAudit);
    task.workspaceChanged = beforeGitStatus !== null && changeAudit.available && beforeGitStatus !== changeAudit.status;
    task.changedFiles = perTaskChanges.changedFiles;
    task.diffStat = perTaskChanges.diffStat;
    task.diffText = perTaskChanges.diffText;
    if (perTaskChanges.baselineFiltered && task.gitBaseline?.files?.length) {
      addTaskEvent(task, 'info', `文件改动按任务基线过滤：本轮实际改动 ${perTaskChanges.changedFiles.length} 个文件（工作区共 ${changeAudit.changedFiles.length} 个）`);
    }
    task.finishedAt = new Date().toISOString();
    task.updatedAt = task.finishedAt;
    ...
    if (task.cancelRequested) {
      ...
      // 区分"跑满 task.timeoutMs"与"内部请求超时/会话无响应被判定为超时"
      const elapsedMs = getTaskElapsedMs(task);
      const elapsedMin = Math.max(1, Math.round(elapsedMs / 60000));
      const budgetMs = Math.max(0, Number(task.timeoutMs || 0));
      const reachedBudget = budgetMs > 0 && elapsedMs >= budgetMs - 5000;
      task.status = 'error';
      task.stage = 'timeout';
      task.error = reachedBudget
        ? `Agent 执行超时：本轮运行 ${elapsedMin} 分钟未完成，已中止`
        : `OpenCode 会话无响应（本轮运行 ${elapsedMin} 分钟），已中止；多为上游或运行时抖动，可直接重试`;
      addTaskEvent(task, 'error', task.error);
  ...
  function retryTask(taskId = '') {
    ...
    task.messages.push({
      ...
      mode: task.mode,
      status: 'sent',
      attachments: attachmentMessageList(task.currentAttachments),
    });
    trimTaskMessages(task);
  ...
  async function runSessionSummarize(task, request, { startedUsage = null, timeoutMs = 180000, modelSelection = null } = {}) {
    ...
    // 压缩是一整轮 LLM 调用：部分环境下该 POST 同步阻塞到压缩完成（实测可达 20s+），
    // 超时必须给足，否则慢上游必然失败
    ...
    try {
      ...
      try {
        await runSessionSummarize(task, (pathname, options = {}) => requestOpenCodeJson(
          ...
          options,
        ), { startedUsage: state.usage, timeoutMs: 180000, modelSelection });
      } finally {
  ...
  async function manageSession(taskId = '', action = '', payload = {}) {
    ...
    if (action === 'summarize') {
      ...
      task.compacting = true;
      addTaskEvent(task, 'info', '正在压缩上下文，请稍候...');
      emitTaskUpdate(task, 'task', true);
      try {
  ...
  return {
    ...
    serializeTask,
    probeProvider,
    buildAgentVariantIntelligence,
    buildAgentModelBadges,
  };
}
