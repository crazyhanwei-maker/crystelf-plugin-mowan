...
export function createAgentWorkbenchConsole(options = {}) {
  ...
  async function getDirectoryRuntime(workspaceId = '', directory = '') {
    ...
    if (existing) workspaceRuntimes.delete(runtimeKey);
  ...
  async function runTask(task) {
    ...
    const opencodeAgent = normalizeOpenCodeAgent(task.agent, isWriteMode(task.mode) ? 'build' : 'plan');
  ...
  async function createTask(payload = {}) {
    ...
    const agent = normalizeOpenCodeAgent(payload.agent, existingTask?.agent || (isWriteMode(mode) ? 'build' : 'plan'));
  ...
  async function buildNativePayload(workspaceId = '', taskId = '') {
    ...
    const worktreeQuery = requestWorkspaceOpenCode(workspace.id, '/experimental/worktree', { timeoutMs: 10000 })
      .catch(error => ({ __error: error.message }));
    const [providerResult, agentsResult, skillsResult, commandsResult, mcpResult, lspResult, formatterResult, ptyResult, worktreeResult, todoResult, childrenResult] = await Promise.all([
      query('/provider'), query('/agent'), query('/skill'), query('/command'), query('/mcp'), query('/lsp'), query('/formatter'), query('/pty'),
      worktreeQuery, sessionQuery('/todo'), sessionQuery('/children'),
    ]);
    ...
    return {
      ...
      data: {
        ...
        errors: [providerResult, agentsResult, skillsResult, commandsResult, mcpResult, lspResult, formatterResult, ptyResult, worktreeResult, todoResult, childrenResult]
  ...
  async function manageWorktree(workspaceId = '', action = '', payload = {}) {
    ...
    if (action === 'remove') {
      ...
      if (runtime) {
        stopAttachedOpenCodeServer(runtime.server);
