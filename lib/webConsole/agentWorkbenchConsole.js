...
function buildSafetyPrompt(mode, prompt) {
  if (isWriteMode(mode)) {
    return [
      ...
      '严禁访问当前工作目录以外的路径；严禁安装或删除依赖；严禁操作 Git、进程、服务、账户、网络和系统配置。',
      '终端命令和联网工具已被禁用。只能使用读取、检索、编辑和写入文件工具完成任务。',
      '不要修改密钥、Cookie、口令、运行日志、运行时数据和依赖目录。完成后说明修改了哪些文件以及仍需人工验证的事项。',
      '',
      '用户任务：',
      prompt,
    ].join('\n');
...
async function inspectWritableGitWorkspace(workspacePath = '') {
  const rootResult = await runCapturedProcess('git', ['rev-parse', '--show-toplevel'], {
    cwd: workspacePath,
    timeoutMs: GIT_STATUS_TIMEOUT_MS,
    outputLimit: 4000,
  }).catch(() => null);
  if (!rootResult || rootResult.code !== 0) {
    throw createHttpError(409, '实际修改模式要求工作目录本身是 Git 仓库', 'AGENT_WRITE_GIT_REQUIRED');
  }
  const gitRoot = resolveExistingDirectory(rootResult.stdout.trim());
  if (!gitRoot || !isSamePath(gitRoot, workspacePath)) {
    throw createHttpError(409, '实际修改模式只能选择 Git 仓库根目录，不能直接修改聚合目录', 'AGENT_WRITE_GIT_ROOT_REQUIRED');
  }
  const status = await readGitStatus(workspacePath);
  if (status === null) {
    throw createHttpError(409, '无法读取工作目录的 Git 状态', 'AGENT_WRITE_GIT_STATUS_FAILED');
  }
  if (status) {
    throw createHttpError(409, '工作目录存在未提交修改。请先备份或提交现有改动，再启动实际修改任务', 'AGENT_WRITE_WORKSPACE_DIRTY');
  }
  return { gitRoot, status };
}
...
export function createAgentWorkbenchConsole(options = {}) {
  ...
  async function runTask(task) {
    ...
    if (isWriteMode(task.mode) && beforeGitStatus !== '') {
      throw createHttpError(409, '任务启动前工作目录已出现未提交修改，已终止写入任务', 'AGENT_WRITE_WORKSPACE_CHANGED');
    }
  ...
  async function createTask(payload = {}) {
    ...
    if (payload.confirmed !== true) {
      throw createHttpError(428, '启动 Agent 任务前需要确认执行范围', 'AGENT_CONFIRMATION_REQUIRED');
    }
    ...
    if (isWriteMode(mode)) {
      ...
      if (payload.writeConfirmed !== true) {
      ...
      await inspectWritableGitWorkspace(workspace.path);
    }
