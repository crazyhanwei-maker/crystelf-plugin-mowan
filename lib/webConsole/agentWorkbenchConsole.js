    ...
    if (attachedServer) {
      if (!task.opencodeSessionId) {
        const session = await requestOpenCodeJson(attachedServer, `/session${openCodeQuery}`, {
          method: 'POST',
          body: { title: task.title, agent: 'build' },
          timeoutMs: 5000,
        });
      ...
      task.command = ['opencode', 'session.message', '--agent', 'build', '--dir', task.workspacePath, '[PROMPT]'];
    } else {
