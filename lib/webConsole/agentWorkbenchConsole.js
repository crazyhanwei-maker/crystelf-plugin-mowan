...
function serializeTask(task = {}, options = {}) {
  const result = {
    ...
    outputText: redactAgentText(task.liveOutputText || task.outputText || ''),
...
export function createAgentWorkbenchConsole(options = {}) {
  ...
  function hasActiveRuntimeTerminals(runtime) {
    return Array.from(terminals.values()).some(terminal => (
      terminal.workspaceId === runtime?.workspace?.id && terminal.status === 'running'
    ));
  }
  ...
  function scheduleWorkspaceRuntimeCleanup(runtime) {
    ...
    clearWorkspaceRuntimeIdleTimer(runtime);
    const remaining = Math.max(1000, AGENT_RUNTIME_IDLE_TIMEOUT_MS - (Date.now() - runtime.lastUsedAt));
    runtime.idleTimer = setTimeout(async () => {
      runtime.idleTimer = null;
      if (workspaceRuntimes.get(runtime.runtimeKey) !== runtime || runtime.closing) return;
      if (hasActiveRuntimeTerminals(runtime)) {
  ...
  function getRuntimeKey(workspaceId = '', directory = '') {
    return `${workspaceId}:${getComparablePath(directory)}`;
  }

  async function getDirectoryRuntime(workspaceId = '', directory = '') {
    ...
    const runtimeKey = getRuntimeKey(workspace.id, resolvedDirectory);
  ...
  async function requestDirectoryOpenCode(workspaceId, directory, pathname, options = {}) {
    const runtime = await getDirectoryRuntime(workspaceId, directory);
    runtime.lastUsedAt = Date.now();
    scheduleWorkspaceRuntimeCleanup(runtime);
  ...
  function importTask(payload = {}) {
    ...
    const task = {
      ...
      messages: Array.isArray(source.messages) ? source.messages.slice(-80).map(serializeAgentMessage) : [],
  ...
  async function manageWorktree(workspaceId = '', action = '', payload = {}) {
    ...
    if (action === 'remove') {
      const runtimeKey = getRuntimeKey(workspace.id, directory);
      const runtime = workspaceRuntimes.get(runtimeKey);
      if (runtime) {
