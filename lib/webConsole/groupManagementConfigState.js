...
export function createGroupManagementConfigState(options = {}) {
  ...
  function buildGroupManagementConfigSnapshot(groupId = '', allConfigs = {}) {
    ...
    const pseudoHumanGroups = isPlainObject(aiConfig.pseudoHuman?.groups) ? aiConfig.pseudoHuman.groups : {};
    const pseudoHumanRaw = isPlainObject(pseudoHumanGroups[normalizedGroupId]) ? pseudoHumanGroups[normalizedGroupId] : null;
    return {
      version: 1,
      ...
      ai: {
        ...
        pseudoHuman: pseudoHumanRaw ? cloneJsonValue(pseudoHumanRaw, {}) : null,
      },
      dailySummary: {
  ...
  function getGroupManagementConfigState(groupId = '', allConfigs = {}) {
    ...
    const pseudoHumanGroups = isPlainObject(aiConfig.pseudoHuman?.groups) ? aiConfig.pseudoHuman.groups : {};
    const pseudoHumanRaw = isPlainObject(pseudoHumanGroups[normalizedGroupId]) ? pseudoHumanGroups[normalizedGroupId] : {};
    const pseudoHumanProbability = Math.min(100, Math.max(0, Number(pseudoHumanRaw.probability ?? 10) || 0));
    const authGroups = isPlainObject(authConfig.groups) ? authConfig.groups : {};
    const newcomerGroups = isPlainObject(newcomerConfig) ? newcomerConfig : {};
    ...
    return {
      ...
      ai: {
        ...
        pseudoHuman: {
          hasCustom: Object.keys(pseudoHumanRaw).length > 0,
          enabled: pseudoHumanRaw.enabled === true,
          probability: pseudoHumanProbability,
          historyLimit: 10,
          effectiveEnabled: mainConfig.ai !== false
            && !aiBlockGroup.includes(normalizedGroupId)
            && (aiWhiteGroup.length === 0 || aiWhiteGroup.includes(normalizedGroupId))
            && pseudoHumanRaw.enabled === true
            && pseudoHumanProbability > 0,
        },
      },
      dailySummary: {
