...
export function createAgentChatBridge({ senderId, reply, config = {}, consoleApi: injectedApi } = {}) {
  ...
  async function runTaskWithReport(promptText, { onProgressReply, onResultReply, onQuestionReply, title = '', attachments = [] } = {}) {
    ...
    try {
      ...
      for (let turn = 0; turn < MAX_BRIDGE_TURNS; turn++) {
        ...
        result = await runTaskLoop(submit, text, { onProgressReply, onQuestionReply });
  ...
  async function runTaskLoop(submit, promptText, { onProgressReply, onQuestionReply }) {
