...
// 追踪任务完成并回报：轮询 listTasks（带 taskId 以获取详情）直到任务结束
...
async function isBusy() {
  ...
  async function runTaskWithReport(promptText, { onProgressReply, title = '' } = {}) {
    const task = await submitTask(promptText, { title });
    // 异步回报：每有新步骤推一条简报，完成推结论
    watchTaskCompletion({
      taskId: task.id,
      onProgress: step => {
        const line = renderStepLine(step);
        if (line && typeof onProgressReply === 'function') onProgressReply(line);
      },
      onDone: finishedTask => {
        const result = finishedTask ? formatTaskResult(finishedTask) : { ok: false, text: '任务等待超时（30 分钟），已停止跟踪；可到控制台查看任务实际状态。' };
        reply(result.text, { ok: result.ok, taskId: task.id });
      },
    }).catch(error => {
      getLogger()?.warn?.(`[agent-chat-bridge] 跟踪任务失败：${error.message}`);
    });
    return task;
  }
