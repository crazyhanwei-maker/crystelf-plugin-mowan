...
async function isBusy() {
  ...
  function lastTaskId() {
    return String(bridgeState.sessionBySender.get(senderKey) || '');
  }

  // 后续指令：交给正在执行的任务。steer = 注入当前执行（比 queue 更即时，且避免收尾窗口里被丢弃）
  async function sendFollowUp(promptText, { delivery = 'steer' } = {}) {
    const taskId = progressState.taskId || lastTaskId();
    if (!taskId) throw new Error('当前没有可跟进的任务');
    ...
    const task = await consoleApi.createTask(payload);
    if (task?.id && ['pending', 'running', 'queued'].includes(task.status)) {
  ...
  async function runTaskWithReport(promptText, { onProgressReply, title = '' } = {}) {
    ...
    const sessionId = lastTaskId();
    const submit = suffix => submitTask(promptText, { title: suffix ? `${title}${suffix}` : title, sessionId });
  ...
  async function runTaskLoop(submit, promptText, { onProgressReply }) {
    ...
    progressState.taskId = String(task?.id || '');
    if (task?.id) setBridgeSession(senderKey, task.id);
