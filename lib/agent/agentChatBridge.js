...
import { normalizeAgentWorkbenchConfig } from '../webConsole/agentWorkbenchConsole.js';
import { getOrCreateWorkbenchConsole } from '../webConsole/workbenchSingleton.js';
import { buildOpenCodeConfig } from '../webConsole/bundledOpenCodeRuntime.js';
...
// 显式“下一条开新会话”标记：resetSession 不能只删锚点——lastTaskId 的回溯会把最近桥任务
// 的会话又续回来，重置等于没生效。置哨兵值后回溯被跳过，任务跟踪时会被真实 taskId 覆盖
const SESSION_FRESH_MARKER = '__fresh__';
...
function loadBridgeState() {
  try {
    ...
    return {
      ...
      modelProviderId: String(raw.modelProviderId || ''),
      variant: String(raw.variant || ''),
      pendingQuestionBySender: raw.pendingQuestionBySender && typeof raw.pendingQuestionBySender === 'object' ? raw.pendingQuestionBySender : {},
...
function saveBridgeState(state) {
  try {
    ...
    fs.writeFileSync(BRIDGE_STORE_FILE, JSON.stringify({
      ...
      pendingQuestionBySender: state.pendingQuestionBySender || {},
...
function getConsole() {
  if (!sharedConsole) {
    ...
    sharedConsole = getOrCreateWorkbenchConsole({
      ConfigControl,
...
// 诊断/回归用：暴露桥实际使用的 console 实例（应与 server.js 的为同一份）
export function getSharedBridgeConsole() {
  return getConsole();
}
...
export function buildBridgeTaskPayload({
  ...
  workspaceId = DEFAULTS.workspaceId, model = '', modelProviderId = '', variant = '', attachments = [],
} = {}) {
  return {
    providerId: 'opencode',
    ...
    ...(model ? { model } : {}),
    // 自定义供应商（如 lyln）必须连 id 一起透传，否则模型名会套在主对话 API 上而报错
...
export function createAgentChatBridge({ senderId, reply, config = {}, consoleApi: injectedApi } = {}) {
  ...
  async function submitTask(promptText, { title = '', sessionId = '', attachments = [] } = {}) {
    const buildPayload = sid => buildBridgeTaskPayload({
      ...
      modelProviderId: bridgeState.modelProviderId,
  ...
  function lastTaskId() {
    ...
    if (stored) {
      // 刚执行过 #新建会话：显式开新会话，跳过回溯（哨兵由下一条任务跟踪时的真实 id 覆盖）
      if (stored === SESSION_FRESH_MARKER) return '';
      return stored;
    }
    try {
  ...
  // /压缩：手动触发会话上下文压缩（summarize）。任务运行中控制台会拒绝
  async function compactSession(taskId = '') {
    const id = String(taskId || progressState.taskId || lastTaskId()).trim();
    if (!id) throw new Error('当前没有可压缩的会话');
    if (progressState.running) throw new Error('任务执行中不能压缩，等本轮结束');
    const items = consoleApi.listTasks({ limit: 50 });
    const list = Array.isArray(items) ? items : items?.tasks || [];
    const target = list.find(item => String(item.id) === id);
    if (!target) throw new Error('任务不存在或已被清理');
    const result = await consoleApi.manageSession(id, 'summarize', {});
    return { id, usage: Number(result?.task?.contextUsage || 0), limit: Number(result?.task?.contextLimit || 0) };
  }

  // 恢复被标记为 interrupted 的任务（服务重启残留），桥重新跟踪直到终态
  async function resumeInterrupted({ onProgressReply, onResultReply, onEventNotice } = {}) {
    const items = consoleApi.listTasks({ limit: 50 });
    const list = Array.isArray(items) ? items : items?.tasks || [];
    const target = list.find(task => isBridgeTask(task, senderKey) && task.status === 'interrupted')
      || list.find(task => isBridgeTask(task, senderKey) && task.stage === 'timeout');
    if (!target) return null;
    await consoleApi.resumeTask(target.id);
    progressState.running = true;
    progressState.text = '';
    progressState.startedAt = Date.now();
    progressState.updatedAt = 0;
    progressState.taskId = String(target.id);
    setBridgeSession(senderKey, target.id);
    try {
      return await trackResumedTask(target, { onProgressReply, onResultReply, onEventNotice });
    } finally {
      progressState.running = false;
    }
  }

  // 恢复任务的跟踪循环：与 runTaskLoop 的轮询段一致，但不重试不重提
  async function trackResumedTask(task, { onProgressReply, onResultReply, onEventNotice }) {
    const startedAt = Date.now();
    let lastCompactedAt = '';
    while (Date.now() - startedAt < 30 * 60 * 1000) {
      ...
      if (onProgressReply) {
        ...
        if (latest) {
          const firstLine = extractReadableOutput(String(latest.content || '')).split(String.fromCharCode(10)).map(line => line.trim()).find(Boolean) || '';
          if (firstLine) onProgressReply(`⚙ 进展：${firstLine.slice(0, 120)}`);
        }
        if (onEventNotice && current.cancelRequested === true) onEventNotice({ type: 'cancel-external', taskId: task.id });
        if (onEventNotice && current.lastCompactedAt && current.lastCompactedAt !== lastCompactedAt) {
          const first = !lastCompactedAt;
          lastCompactedAt = current.lastCompactedAt;
          if (!first) onEventNotice({ type: 'compacted', taskId: task.id });
        }
      }
      if (['pending', 'running', 'queued'].includes(current.status)) continue;
      const result = formatTaskResult(current);
      if (typeof onResultReply === 'function') await onResultReply(result, { turn: 0, prompt: String(current.prompt || '') });
      return result;
    }
    return { ok: false, text: '恢复任务跟踪超时（30 分钟）；任务可能仍在后台，可到控制台查看。' };
  }

  // 本通道最近一条可恢复（interrupted/timeout）的桥任务
  function findResumable() {
    ...
    return {
      id: String(target.id),
      title: String(target.title || '').replace(`${BRIDGE_TITLE_PREFIX}${senderKey}`, '').trim() || '任务',
      status: String(target.status || ''),
      stage: String(target.stage || ''),
      promptPreview: String(target.promptPreview || '').slice(0, 80),
    };
  }

  // 归档本通道指定任务（从 /任务列表 隐藏），或清掉全部已完成/失败/中断的
  function archiveTasks(taskId = '') {
    const items = consoleApi.listTasks({ limit: 100, includeArchived: false });
    const list = Array.isArray(items) ? items : items?.tasks || [];
    let archived = 0;
    for (const task of list) {
      if (!isBridgeTask(task, senderKey)) continue;
      if (['pending', 'running', 'queued'].includes(task.status)) continue;
      if (taskId && String(task.id) !== String(taskId)) continue;
      try {
        consoleApi.archiveTask(task.id);
        archived += 1;
      } catch { }
    }
    return { archived };
  ...
  // 置哨兵而非删除，防止 lastTaskId 的回溯逻辑把旧会话又续回来
  function resetSession() {
    bridgeState.sessionBySender.set(senderKey, SESSION_FRESH_MARKER);
    saveBridgeState(bridgeState);
...
// 可用模型列表：直接从内置 OpenCode 的配置生成（与任务实际注册的模型完全一致）。
// 旧实现读「原生运行时」的 /provider 快照——那是另一套无供应商配置的进程，与控制台任务用的
// bundled 运行时互不相通，常常拿不到任何模型，导致 /模型 误报「唤醒超时：OpenCode 仍不可用」
export async function listAvailableModels() {
  try {
    const ConfigControl = ConfigControlModule?.default || ConfigControlModule;
    const aiConfig = ConfigControl?.get?.('ai') || {};
    const coreConfig = ConfigControl?.get?.('coreConfig') || {};
    const workbench = normalizeAgentWorkbenchConfig(coreConfig?.tools?.agentWorkbench || {});
    const config = buildOpenCodeConfig(aiConfig, '', { modelProviders: workbench.modelProviders || [] });
    const models = [];
    for (const [providerId, provider] of Object.entries(config.provider || {})) {
      const providerLabel = String(provider?.name || providerId).replace(/\s+/g, ' ').trim().slice(0, 40);
      for (const [modelId, modelConf] of Object.entries(provider?.models || {})) {
        if (!modelId) continue;
        const variants = Object.keys(modelConf?.variants && typeof modelConf.variants === 'object' ? modelConf.variants : {})
          .map(value => String(value).trim()).filter(Boolean);
        models.push({
          id: modelId,
          name: providerId === 'crystelf-chat' ? modelId : `${modelId} · ${providerLabel}`,
          providerId,
          reasoning: variants.length > 1,
          variants,
        });
      }
    }
    ...
    getLogger()?.warn?.(`[agent-chat-bridge] 从配置生成模型列表失败，回退原生快照：${error.message}`);
  }
  try {
...
// 模型提问待答状态：持久化到桥状态文件，bot 重启后 #回答 仍可提交
export function setBridgePendingQuestion(senderKey, entry) {
  const key = String(senderKey || '').trim();
  if (!key || !entry || typeof entry !== 'object') return;
  bridgeState.pendingQuestionBySender = bridgeState.pendingQuestionBySender || {};
  bridgeState.pendingQuestionBySender[key] = entry;
  saveBridgeState(bridgeState);
}

export function getBridgePendingQuestion(senderKey) {
  const store = bridgeState.pendingQuestionBySender || {};
  return store[String(senderKey || '')] || null;
}

export function clearBridgePendingQuestion(senderKey) {
  const key = String(senderKey || '').trim();
  if (bridgeState.pendingQuestionBySender && bridgeState.pendingQuestionBySender[key]) {
    delete bridgeState.pendingQuestionBySender[key];
    saveBridgeState(bridgeState);
  }
}

// 选择模型（空串 = 恢复默认，同时清掉供应商）；providerId 与模型一起持久化
export function setBridgeModel(model, providerId = '') {
  bridgeState.model = String(model || '').trim();
  bridgeState.modelProviderId = bridgeState.model ? String(providerId || '').trim() : '';
  saveBridgeState(bridgeState);
