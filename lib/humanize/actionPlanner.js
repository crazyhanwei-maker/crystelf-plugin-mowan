export class ActionPlanner {
  ...
  async plan(sessionId, botName, recentHistory, lastTriggerMessage, isIdleCheck = false) {
...
DO NOT include any explanation, markdown formatting, or additional text. Only output the JSON.`;
    ...
    try {
      ...
      if (!content || !content.trim()) {
        logger.warn('[ActionPlanner] Empty response from AI, using default wait');
        return { action: 'wait', reason: 'empty response', waitMs: 30000 };
      }
      ...
      if (!jsonStr) {
        ...
        return { action: 'wait', reason: 'parse failed', waitMs: 30000 };
      }
      ...
      const action = parsed.action === 'wait' ? 'wait' : parsed.action === 'complete' ? 'complete' : 'reply';

      const result = {
        action,
        reason: parsed.reason || '',
        waitMs: action === 'wait' ? Math.min(Math.max((parsed.wait_seconds || 30) * 1000, 10000), 300000) : undefined,
      };
      ...
      return { action: 'wait', reason: 'error fallback', waitMs: 30000 };
    }
  }
}
