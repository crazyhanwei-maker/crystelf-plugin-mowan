...
import { resolveBotIdentity } from './botIdentity.js';
import {
  buildBrandHero,
  buildImageThemeCss,
...
function formatFiles(files = []) {
  ...
  return files.slice(0, 8).map(file => {
    const item = typeof file === 'string' ? { displayPath: file } : (file || {});
    return `
    <div class="file-row">
      <strong>${escapeHtml(item.displayPath || item.path || '-')}</strong>
      <span>命中 ${escapeHtml(item.hitCount || 0)} · ${escapeHtml(item.mtime ? formatTime(item.mtime) : '未提供时间')}</span>
    </div>
  `;
  }).join('');
}
...
function normalizeAnalysisBlocks(text = '') {
  ...
  const normalized = content.trim() || 'AI 未返回排查结论。';
  return normalized
    .split(/\n{2,}/)
...
export function buildLogDiagnosisImageHtml(data = {}) {
  ...
  const selectedFiles = Array.isArray(data.selectedFiles) ? data.selectedFiles : [];
  const hitCount = Number(data.hitCount ?? data.summary?.errorCount ?? 0)
    + Number(data.hitCount === undefined ? data.summary?.warningCount || 0 : 0);
  const sourceText = data.source === 'auto' || !data.source ? '自动选择' : String(data.source);
  return `<!DOCTYPE html>
<html lang="zh-CN">
<head>
<meta charset="UTF-8">
<style>
  * { box-sizing: border-box; }
  ...
  .metrics { padding: 18px; border-radius: 8px; }
  .metric { padding: 14px; border: 1px solid var(--theme-line); border-radius: 7px; background: rgba(255,255,255,0.62); }
  .metric span { color: var(--theme-muted); font-size: 12px; font-weight: 800; }
  .metric strong { margin-top: 6px; color: var(--theme-ink); font-size: 25px; }
  .section { padding: 18px; }
  .section h2 { margin: 0 0 13px; color: var(--theme-ink); font-size: 19px; }
  .analysis { gap: 9px; }
  .analysis p { padding: 12px 14px; border-left: 4px solid var(--theme-blue); border-radius: 6px; background: rgba(255,255,255,0.62); color: #343b46; font-size: 15px; line-height: 1.58; }
  .file-list { gap: 8px; }
  .file-row { padding: 10px 12px; border: 1px solid var(--theme-line); border-radius: 6px; background: rgba(255,255,255,0.58); font-size: 12px; }
  .file-row span, .empty { color: var(--theme-muted); }
</style>
</head>
<body>
  <div class="page theme-sheet">
    <main class="shell">
      ${buildBrandHero({
        ...
        badge: '日志诊断',
        title: 'AI 排查报告',
        subtitle: '基于最近 Bot 与插件日志生成，敏感信息已在送检前脱敏',
        sideTitle: `${hitCount} 条告警`,
        sideText: generatedAt,
      })}
      <section class="theme-panel metrics">
        <div class="metric"><span>疑似错误/告警</span><strong>${escapeHtml(hitCount)}</strong></div>
        <div class="metric"><span>送检日志文件</span><strong>${escapeHtml(selectedFiles.length)}</strong></div>
        <div class="metric"><span>日志来源</span><strong>${escapeHtml(sourceText)}</strong></div>
      </section>
      ...
      <section class="theme-panel section">
        <h2>日志文件</h2>
        <div class="file-list">${formatFiles(selectedFiles)}</div>
      </section>
      <footer class="theme-footer"><span>Created by 魔丸插件</span><span>${escapeHtml(generatedAt)}</span></footer>
    </main>
  </div>
</body>
</html>`;
