...
export function buildStatusImageHtml(data = {}) {
  ...
  const pluginVersion = getRowValue(rows, '插件版本', 'crystelf-plugin v2.2.0');
  ...
  <main class="sheet">
    ...
    <section class="panel">
      ...
      <div class="resource-grid">
        ${renderRing(cpuPercent, 'CPU', `${Math.round(cpuPercent)}%`, '进程估算占用', 'cyan')}
        ${renderRing(memoryPercent, '物理内存', `${memoryUsedGb} / ${memoryTotalGb}`, `RSS ${rssMb}`, 'blue')}
