...
const Updater = {
  ...
  async getUpdateStatus() {
    try {
      ...
      await runGit(['fetch', PUBLIC_REPOSITORY_URL, PUBLIC_REPOSITORY_FETCH_REFSPEC]);
  ...
  async update(status = null) {
    ...
    execGit(['pull', '--ff-only', '--no-rebase', PUBLIC_REPOSITORY_URL, PUBLIC_REPOSITORY_BRANCH], {
      stdio: 'inherit',
      timeout: 3 * 60 * 1000,
    });
  ...
  async forceUpdate(options = {}) {
    ...
    await runGit(['fetch', PUBLIC_REPOSITORY_URL, PUBLIC_REPOSITORY_FETCH_REFSPEC], { timeout: 2 * 60 * 1000 });
