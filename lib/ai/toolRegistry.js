...
async function searchWeb(args = {}) {
  ...
  try {
    const usedFallbackFirst = !hasSearchEndpoint(searchConfig);
    const result = hasSearchEndpoint(searchConfig)
    ...
    logger.info(
      `[toolRegistry] search_web 成功 | query="${payload.query}" | result_count=${result.count} | total_results=${result.totalResults || 0} | elapsed_ms=${Date.now() - startedAt}${usedFallbackFirst ? ' | fallback=true' : ''}`
    );
    await appendSearchDebugLog({
      stage: 'success',
      query: payload.query,
    ...
    if (hasSearchEndpoint(searchConfig) && fallbackSearchConfig && !isSameConfiguredEndpoint(searchConfig, fallbackSearchConfig, 'apiUrl')) {
      try {
...
async function fetchWebMarkdown(args = {}) {
  ...
  try {
    const usedFallbackFirst = !hasMarkdownEndpoint(searchConfig);
    const activeConfig = hasMarkdownEndpoint(searchConfig)
      ? searchConfig
      : fallbackMarkdownConfig;
    const result = await requestWebMarkdown(activeConfig, targetUrl, clientTimeoutMs, markdownMaxLength);
    if (usedFallbackFirst) {
      return { ...result, fallback: true };
    }
    return await retryWebMarkdownFallbackIfNeeded({
      result,
    ...
    if (hasMarkdownEndpoint(searchConfig) && fallbackMarkdownConfig && !isSameConfiguredEndpoint(searchConfig, fallbackMarkdownConfig, 'markdownApiUrl')) {
