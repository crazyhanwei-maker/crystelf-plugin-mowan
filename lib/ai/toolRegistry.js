...
async function searchWeb(args = {}) {
  ...
  try {
    ...
    const maxResults = Math.min(Number(args.limit || searchConfig.maxResults || 5), 10);
...
async function fetchWebMarkdown(args = {}) {
  ...
  const targetUrl = normalizeOptionalString(args.url);
  if (!targetUrl) {
    return {
      success: false,
      error: '网页地址不能为空',
  ...
  const clientTimeoutMs = normalizeClientTimeout(args.timeout_ms || searchConfig.timeoutMs || 60000);
  ...
  try {
    ...
    while (Date.now() - startedAt < clientTimeoutMs) {
      ...
      if (data.status === 'completed') {
        const markdown = sanitizeText(data.result?.markdown || '', markdownMaxLength);
        return {
          success: true,
          url: targetUrl,
          task_id: taskId,
