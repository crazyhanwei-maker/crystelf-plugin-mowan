...
import { logAiUsage, setChatContentEnabledResolver } from './usageLogger.js';
import {
...
import { normalizeAiUserAgent } from './userAgent.js';
...
// 用量日志的提供方展示名（按该次调用实际使用的配置协议）
function providerLabelForConfig(config = {}) {
  const protocol = normalizeAiProtocol(config?.apiProtocol);
  return protocol === 'anthropic' ? 'anthropic' : protocol === 'grok' ? 'grok' : 'openai-compatible';
}
...
/**
 * 从消息数组里提取最后一条 user 消息的纯文本，用于对话日志。
 * 只保留用户实际输入，不包含 system 提示词和工具拼接内容。
 */
...
function normalizeAiErrorMessage(error, timeoutMs = 60000) {
  const name = String(error?.name || '').trim();  const code = String(error?.code || '').trim();
...
function resolveFallbackCompletionModel(config = {}, options = {}) {
  if (options?.multimodal === true) {
    return config?.multimodalModel || resolveFallbackWorkModel(config);
  }
  return resolveFallbackWorkModel(config);
}
...
function getAiResultError(result = null, fallback = '无有效回复') {
  return result?.error || result?.response || fallback;
}
...
class AiCaller {
  ...
  async init() {
    try {
      ...
      this.openaiChat.init(this.config.apiKey, this.config.baseApi, this.config.timeout, this.config.retryCount, this.config);
      this.runtimeConfigSignature = this.getConfigSignature(this.config);

      // 对话内容记录开关：每次写入时实时读取，改配置即时生效，无需重启。
      setChatContentEnabledResolver(() => ConfigControl.get()?.coreConfig?.usageControl?.chatContent !== false);
  ...
  getConfigSignature(config = {}) {
    return JSON.stringify({
      ...
      // 接入协议必须参与签名：只改协议（不改地址/密钥）也要触发热更新重初始化
      apiProtocol: String(config.apiProtocol || '').trim(),
      ...
      imageOutputFormat: String(config.imageConfig?.outputFormat || '').trim(),
      imageWatermark: config.imageConfig?.watermark === true,
      imageWebSearch: config.imageConfig?.webSearch === true,
      ...
      imageFallbackSize: String(config.imageConfig?.fallbackApi?.size || '').trim(),
      imageFallbackResponseFormat: String(config.imageConfig?.fallbackApi?.responseFormat || '').trim(),
      imageFallbackOutputFormat: String(config.imageConfig?.fallbackApi?.outputFormat || '').trim(),
      imageFallbackWatermark: config.imageConfig?.fallbackApi?.watermark === true,
      imageFallbackWebSearch: config.imageConfig?.fallbackApi?.webSearch === true,
      retryCount: Number(config.retryCount) > 0 ? Number(config.retryCount) : 0,
      userAgent: normalizeAiUserAgent(config.userAgent || ''),
    });
  }
  ...
  async getOpenaiInstanceForConfig(userId, config) {
    if (config?.__fallbackApi) {
      ...
      fallbackOpenaiChat.init(config.apiKey, config.baseApi, config.timeout, config.retryCount, config);
  ...
  async retryTextAiWithFallback({
    ...
    recordPrimaryApiFailure(config, usageContext.scene || 'chat_text', getAiResultError(result));
    const fallbackCaller = await this.getOpenaiInstanceForConfig(e?.user_id || 'system', fallbackConfig);
    const fallbackResult = await fallbackCaller.callAi({
      prompt,
    ...
    if (fallbackResult?.success) {
      recordFallbackApiSuccess(config, usageContext.scene || 'chat_text');
    } else {
      recordFallbackApiFailure(config, usageContext.scene || 'chat_text', getAiResultError(fallbackResult));
    }
  ...
  async retryMultimodalAiWithFallback({ result, messages, config, e, usageContext }) {
    ...
    recordPrimaryApiFailure(config, usageContext.scene || 'chat_multimodal', getAiResultError(result));
    const fallbackCaller = await this.getOpenaiInstanceForConfig(e?.user_id || 'system', fallbackConfig);
    const fallbackResult = await fallbackCaller.callAi({
      messages,
    ...
    if (fallbackResult?.success) {
      recordFallbackApiSuccess(config, usageContext.scene || 'chat_multimodal');
    } else {
      recordFallbackApiFailure(config, usageContext.scene || 'chat_multimodal', getAiResultError(fallbackResult));
    }
    return fallbackResult;
  }
  ...
  async createDirectCompletion(apiCaller, config, messages, options = {}, fallback = false) {
    ...
    return withRetry(
      () => (apiCaller.createChatCompletion
        ? apiCaller.createChatCompletion({
            model,
  ...
  async createToolCompletion(apiCaller, config, options = {}, fallback = false) {
    const model = fallback
      ? resolveFallbackCompletionModel(config, options)
      : resolvePreferredWorkModel(config, options.model);
    const retryCount = Number(config.retryCount) > 0 ? Number(config.retryCount) : 0;
    return withRetry(
      () => (apiCaller.createChatCompletion
  ...
  async callTextAi(prompt, chatHistory = [], memories = [], e, userConfig = null) {
    try {
      ...
      const preferFallback = shouldPreferFallbackApi(config, usageContext.scene);
      const fallbackConfig = preferFallback.preferFallback ? buildAiFallbackConfig(config) : null;
      const systemPrompt = await this.getSystemPrompt(e, memories);
      let result = null;
      let fallbackResult = null;
      if (fallbackConfig) {
        logger.warn(`[crystelf-ai] 文本AI主接口处于冷却期，优先使用备用API，恢复时间: ${preferFallback.openUntil ? new Date(preferFallback.openUntil).toLocaleString('zh-CN', { hour12: false }) : '稍后重试'}`);
        const fallbackCaller = await this.getOpenaiInstanceForConfig(e?.user_id || 'system', fallbackConfig);
        fallbackResult = await fallbackCaller.callAi({
          prompt: fullPrompt,
          chatHistory: formattedChatHistory,
          model: resolvePreferredChatModel(fallbackConfig),
          temperature: fallbackConfig.temperature,
          ...
          usageContext: {
            ...usageContext,
            scene: `${usageContext.scene}_fallback`,
          },
        });
        if (fallbackResult?.success) {
          recordFallbackApiSuccess(config, usageContext.scene);
        } else {
          recordFallbackApiFailure(config, usageContext.scene, getAiResultError(fallbackResult));
        }
      } else {
        result = await apiCaller.callAi({
          prompt: fullPrompt,
          chatHistory: formattedChatHistory,
        ...
        if (result?.success) {
          recordPrimaryApiSuccess(config, usageContext.scene);
        }
        fallbackResult = await this.retryTextAiWithFallback({
          result,
          prompt: fullPrompt,
          formattedChatHistory,
          config,
  ...
  async callMultimodalAi(originalMessages, chatHistory = [], memories = [], e, userConfig = null) {
    try {
      ...
      let fallbackResult = null;
      if (fallbackConfig) {
        logger.warn(`[crystelf-ai] 多模态AI主接口处于冷却期，优先使用备用API，恢复时间: ${preferFallback.openUntil ? new Date(preferFallback.openUntil).toLocaleString('zh-CN', { hour12: false }) : '稍后重试'}`);
        const fallbackCaller = await this.getOpenaiInstanceForConfig(e?.user_id || 'system', fallbackConfig);
        fallbackResult = await fallbackCaller.callAi({
          messages,
          model: fallbackConfig.multimodalModel || fallbackConfig.modelType,
        ...
        fallbackResult = await this.retryMultimodalAiWithFallback({
          result,
          messages,
          config,
          e,
  ...
  async getUserOpenaiInstance(userId, config) { 
    ...
    const userAgent = normalizeAiUserAgent(config?.userAgent || '');
    const globalUserAgent = normalizeAiUserAgent(this.config?.userAgent || '');

    if (config.apiKey === this.config.apiKey && config.baseApi === this.config.baseApi && timeout === globalTimeout && userAgent === globalUserAgent) {
      logger.info(`[crystelf-ai] 用户 ${userId} 使用全局OpenAI实例`);
      return this.openaiChat;
    }
    const cacheKey = `${userId}_${config.apiKey}_${config.baseApi}_${timeout}_${userAgent}`;
    ...
    userOpenaiChat.init(config.apiKey, config.baseApi, timeout, config.retryCount, config);
  ...
  async callAiDirect(prompt, chatHistory = [], memories = [], e = null, originalMessages = [], options = {}) {
    ...
    try {
      ...
      const scene = options.scene || 'direct';
      const preferFallback = shouldPreferFallbackApi(config, scene);
      const preferredFallbackConfig = preferFallback.preferFallback ? buildAiFallbackConfig(config) : null;
      let completion;
      let usedConfig = config;
      let usedFallback = false;
      if (preferredFallbackConfig) {
        logger.warn(`[crystelf-ai] callAiDirect主接口处于冷却期，优先使用备用API，恢复时间: ${preferFallback.openUntil ? new Date(preferFallback.openUntil).toLocaleString('zh-CN', { hour12: false }) : '稍后重试'}`);
        attemptedFallback = true;
        const fallbackCaller = await this.getOpenaiInstanceForConfig(userId, preferredFallbackConfig);
        completion = await this.createDirectCompletion(fallbackCaller, preferredFallbackConfig, messages, options, true);
        usedConfig = preferredFallbackConfig;
        usedFallback = true;
      } else {
        completion = await this.createDirectCompletion(apiCaller, config, messages, options);
      }

      let content = completion?.choices?.[0]?.message?.content || '';
      if (!String(content || '').trim()) {
        if (!usedFallback) {
          recordPrimaryApiFailure(config, scene, '模型没有产出可发送内容');
        }
        const fallbackConfig = buildAiFallbackConfig(config);
      ...
      if (!String(content || '').trim()) {
        if (usedFallback) {
          recordFallbackApiFailure(config, scene, '备用模型没有产出可发送内容');
        }
        throw new Error('模型没有产出可发送内容');
      }

      if (usedFallback) {
        recordFallbackApiSuccess(config, scene);
      } else {
        recordPrimaryApiSuccess(config, scene);
      }

      await logAiUsage({
        stage: 'success',
        scene: usedFallback ? `${scene}_fallback` : scene,
        provider: providerLabelForConfig(usedConfig),
      ...
      try {
        ...
        if (!attemptedFallback && fallbackConfig) {
          ...
          recordPrimaryApiFailure(config, options.scene || 'direct', error.message);
          attemptedFallback = true;
          const systemPrompt = options.systemPrompt || (e ? await this.getSystemPrompt(e, memories) : '');
          const messages = Array.isArray(options.messages) && options.messages.length > 0
            ? options.messages
          ...
          recordFallbackApiSuccess(config, options.scene || 'direct');
          await logAiUsage({
            stage: 'success',
            scene: `${options.scene || 'direct'}_fallback`,
        provider: providerLabelForConfig(fallbackConfig),
        ...
        recordFallbackApiFailure(config, options.scene || 'direct', fallbackError.message);
      }
      ...
      await logAiUsage({
        ...
        provider: providerLabelForConfig(config || this.config),
  ...
  async callAiComplete(options = {}) {
    ...
    try {
      ...
      const scene = options.scene || 'complete';
      const preferFallback = shouldPreferFallbackApi(this.config, scene);
      const preferredFallbackConfig = preferFallback.preferFallback ? buildAiFallbackConfig(this.config) : null;
      let completion;
      let usedConfig = this.config;
      let usedFallback = false;
      if (preferredFallbackConfig) {
        logger.warn(`[crystelf-ai] callAiComplete主接口处于冷却期，优先使用备用API，恢复时间: ${preferFallback.openUntil ? new Date(preferFallback.openUntil).toLocaleString('zh-CN', { hour12: false }) : '稍后重试'}`);
        attemptedFallback = true;
        const fallbackCaller = await this.getOpenaiInstanceForConfig('system', preferredFallbackConfig);
        completion = await this.createToolCompletion(fallbackCaller, preferredFallbackConfig, options, true);
        usedConfig = preferredFallbackConfig;
        usedFallback = true;
      } else {
        completion = await this.createToolCompletion(apiCaller, this.config, options);
      }
      ...
      if (!hasCompletionMessagePayload(message)) {
        if (!usedFallback) {
          recordPrimaryApiFailure(this.config, scene, '模型没有产出可用内容');
        }
      ...
      if (!hasCompletionMessagePayload(message)) {
        if (usedFallback) {
          recordFallbackApiFailure(this.config, scene, '备用模型没有产出可用内容');
        }
        throw new Error('模型没有产出可用内容');
      }

      if (usedFallback) {
        recordFallbackApiSuccess(this.config, scene);
      } else {
        recordPrimaryApiSuccess(this.config, scene);
      }
      ...
      await logAiUsage({
        ...
        scene: usedFallback ? `${scene}_fallback` : scene,
        provider: providerLabelForConfig(usedConfig),
        model: usedFallback ? resolveFallbackCompletionModel(usedConfig, options) : resolvePreferredWorkModel(usedConfig, options.model),
        ...
        promptFull: extractLastUserText(options.messages),
        responseFull: message.content || '',
        usage: completion?.usage,
      });
      ...
      try {
        ...
        if (!attemptedFallback && fallbackConfig) {
          ...
          recordPrimaryApiFailure(this.config, options.scene || 'complete', error.message);
          attemptedFallback = true;
          const startedAt = Date.now();
          const fallbackCaller = await this.getOpenaiInstanceForConfig('system', fallbackConfig);
          ...
          recordFallbackApiSuccess(this.config, options.scene || 'complete');
          const toolCalls = (message.tool_calls || []).map(tc => ({
            id: tc.id,
            name: tc.function?.name,
            arguments: tc.function?.arguments || '{}',
        ...
        provider: providerLabelForConfig(fallbackConfig),
            model: resolveFallbackCompletionModel(fallbackConfig, options),
            sessionId: options.sessionId,
            groupId: options.groupId,
            userId: options.userId,
            messageCount: Array.isArray(options.messages) ? options.messages.length : 0,
        ...
        recordFallbackApiFailure(this.config, options.scene || 'complete', fallbackError.message);
      }
      ...
      await logAiUsage({
        ...
        provider: providerLabelForConfig(this.config),
