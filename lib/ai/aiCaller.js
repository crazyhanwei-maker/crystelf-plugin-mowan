...
function resolvePreferredModel(config = {}, explicitModel = '') {
  return explicitModel || config?.workingModel || config?.modelType || '';
}
...
class AiCaller {
  ...
  async callTextAi(prompt, chatHistory = [], memories = [], e, userConfig = null) {
    try {
      ...
      const result = await apiCaller.callAi({
        ...
        model: config.workingModel || config.modelType,
  ...
  async callAiDirect(prompt, chatHistory = [], memories = [], e = null, originalMessages = [], options = {}) {
    try {
      ...
      const systemPrompt = e ? await this.getSystemPrompt(e, memories) : (options.systemPrompt || '');
      ...
      const completion = await apiCaller.openai.chat.completions.create({
        model: resolvePreferredModel(config, options.model),
        messages,
      ...
      await logAiUsage({
        stage: 'error',
        scene: options.scene || 'direct',
        model: resolvePreferredModel(this.config, options.model),
