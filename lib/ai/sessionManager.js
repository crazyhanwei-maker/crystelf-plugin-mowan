...
export class RateLimiter {
  ...
  canProcess(userId, groupId, text) {
...
export class SkillSessionManager {
  ...
  loadSkill(sessionId, skillName, tools) {
    ...
    for (const [name, session] of sessionSkills) {
      if (now < session.expiresAt) {
        for (const [toolName, tool] of session.tools) {
          allTools.set(`${name}.${toolName}`, tool);
        }
      }
    }
    return allTools;
  ...
  getActiveSkillsInfo(sessionId) {
    ...
    for (const [name, session] of sessionSkills) {
      if (now < session.expiresAt) {
        const toolNames = Array.from(session.tools.keys());
        active.push(`- ${name}: ${toolNames.join(', ')}`);
      }
    }
    return active.length > 0 ? `## Active Skills\n${active.join('\n')}` : '';
  }
