...
class ImageProcessor {
  ...
  async generateOrEditImage(prompt, sourceImageArr = [], config = this.config) {
    ...
    try {
      ...
      const scene = sourceImageArr.length > 0 ? 'image_edit' : 'image_generate';
      ...
      if (preferFallback.preferFallback && fallbackConfig) {
        ...
        result = sourceImageArr.length > 0
  ...
  async generateImageByArkAgentPlan(prompt, config) {
    try {
      ...
      const requestBody = buildArkAgentPlanImageRequest(prompt, { ...config, model });
      logger.info(`[crystelf-ai] 使用火山 Agent Plan 生成图像: ${prompt}`);
      ...
      logger.error(`[crystelf-ai] Agent Plan 返回无效图像数据: ${JSON.stringify(response.data)}`);
  ...
  async editImage(prompt, sourceImageArr, config){
    if(config.imageMode==='jimeng'){
      ...
      return {
        success: false,
        error: '火山 Agent Plan 当前仅支持文生图，不支持改图',
      };
  ...
  mergeImageConfig(userConfig) {
    ...
    if (userConfig?.imageConfig) {
      ...
      const imageRelatedKeys = [
        ...
        'size', 'responseFormat', 'modalities', 'timeout', 'fallbackReply', 'fallbackTimeoutReply', 'quality', 'background', 'style', 'outputFormat', 'watermark', 'userAgent'
      ];
  ...
  validateImageConfig(config) {
    ...
    if (imageMode !== 'jimeng' && !config.model) {
      errors.push('模型名称不能为空');
    }

    const imageSize = normalizeImageSizeValue(config.size);
    if (imageSize) {
      if (isArkAgentPlanImageMode(imageMode) && /^[124]K$/.test(imageSize)) {
