...
class ImageProcessor {
  ...
  async generateOrEditImage(prompt, sourceImageArr = [], config = this.config) {
    ...
    try {
      ...
      const result = sourceImageArr.length > 0
      ...
      await logImageUsage({
        ...
        mode: sourceImageArr.length > 0 ? `edit:${mergedConfig.imageMode || 'openai'}` : `generate:${mergedConfig.imageMode || 'openai'}`,
        model: result?.model || mergedConfig.model,
        prompt,
        elapsedMs: Date.now() - startedAt,
        sourceCount: Array.isArray(sourceImageArr) ? sourceImageArr.length : 0,
        hasImage: Boolean(result?.imageUrl),
        error: result?.success ? '' : (result?.error || result?.response || '图像接口未返回图片地址'),
      });
