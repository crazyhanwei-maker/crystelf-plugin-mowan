...
function buildCircuitKey(config = {}, scene = 'chat') {
  return [
    ...
    cleanString(config.baseApi),
    cleanString(config.modelType || config.workingModel || config.model),
    cleanString(config?.fallbackApi?.baseApi),
    cleanString(config?.fallbackApi?.modelType || config?.fallbackApi?.workingModel),
  ].join('::');
}
