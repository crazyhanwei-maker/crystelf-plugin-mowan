...
import { listTtsModels } from './ttsRegistry.js';
import {
  clearGroupVoiceModel,
  getGroupVoiceModelRecord,
  setGroupVoiceModel,
} from './ttsGroupModelStore.js';
...
function tryMarkHandled(e = {}) {
  if (e.crystelfVoiceModelHandled) {
    return false;
  }
  e.crystelfVoiceModelHandled = true;
  return true;
}
...
export async function showCurrentVoiceModel(e) {
  if (!tryMarkHandled(e)) return true;
...
export async function showVoiceModelList(e) {
  if (!tryMarkHandled(e)) return true;
  if (!isVoiceModelEnabled()) return false;
...
export async function switchVoiceModelDirectly(e) {
  if (!tryMarkHandled(e)) return true;
  if (!isVoiceModelEnabled()) return false;
  ...
  return applyVoiceModelSelection(e, groupId, parseSwitchInput(e), result.models);
}

export async function selectPendingVoiceModel(e) {
  ...
  return applyVoiceModelSelection(e, pending.groupId, e.msg, pending.models);
}

export async function resetVoiceModel(e) {
  if (!tryMarkHandled(e)) return true;
  if (!isVoiceModelEnabled()) return false;
...
export async function applyVoiceModelSelection(e, groupId, input, models) {
