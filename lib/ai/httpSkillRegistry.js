...
function validateResolvedUrl(rawUrl, resolvedUrl, allowedHosts = []) {
  let raw;
  let resolved;
  try {
...
async function executeHttpSkill(skillDefinition, toolDefinition, args = {}, globalConfig = {}) {
  ...
  const resolvedUrl = validateResolvedUrl(rawUrl, applyTemplate(rawUrl, args, templateOptions), allowedHosts);
  ...
  logger.info(
    `[http-skill] ${skillDefinition.name}.${toolDefinition.name} 开始 | method=${method} | url="${resolvedUrl}" | timeout_ms=${timeoutMs}`
  );
