...
class MemorySystem {
  ...
  async init() {
    try {
      ...
      const configuredTimeoutDays = Number(config?.memory?.timeoutDays || config?.memoryTimeoutDays || 30);
      this.defaultTimeout = Number.isFinite(configuredTimeoutDays) && configuredTimeoutDays > 0
        ? configuredTimeoutDays
        : 30;
