...
class UserConfigManager {
  ...
  async getUserImageConfig(userId) {
    try {
      ...
      const imageConfig = userConfig.imageConfig || this.globalConfig?.imageConfig || {
        enabled: true,
        imageMode: 'openai',
        model: 'gemini-3-pro-image-preview',
        baseApi: 'https://api.openai.com',
        jimengApiUrl: 'http://165.99.42.28:985',
        apiKey: '',
        maxTokens: 4000,
        temperature: 0.7,
        size: '1024x1024',
      ...
      const imageMode = String(imageConfig.imageMode || 'openai').trim();

      if (imageMode === 'jimeng') {
        return {
          ...imageConfig,
          jimengApiUrl: String(imageConfig.jimengApiUrl || '').trim(),
      ...
      return {
        ...imageConfig,
        baseApi: String(imageConfig.baseApi || userConfig?.baseApi || '').trim(),
        apiKey: String(imageConfig.apiKey || userConfig?.apiKey || '').trim(),
      };
    } catch (error) {
