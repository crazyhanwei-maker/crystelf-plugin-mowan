import { isArkAgentPlanImageMode } from './imageApi.js';
import { isSdWebUiImageMode, normalizeSdWebUiSettings } from './sdWebUiApi.js';
...
export function buildAiFallbackConfig(config = {}) {
  ...
  return {
    ...
    // 接入协议：主协议已随 ...config 继承，这里允许备用接口显式覆盖
    apiProtocol: cleanString(fallback.apiProtocol || config.apiProtocol || ''),
    __fallbackApi: true,
  };
}
...
export function buildImageFallbackConfig(mergedImageConfig = {}) {
  ...
  const arkAgentPlanMode = isArkAgentPlanImageMode(imageMode);
  const sdWebUiMode = isSdWebUiImageMode(imageMode);
  const next = {
    ...mergedImageConfig,
    imageMode,
    ...
    size: cleanString(fallback.size || (arkAgentPlanMode ? '2K' : mergedImageConfig.size)),
    responseFormat: cleanString(fallback.responseFormat || (arkAgentPlanMode ? 'url' : mergedImageConfig.responseFormat)),
    outputFormat: cleanString(fallback.outputFormat || (arkAgentPlanMode ? 'png' : mergedImageConfig.outputFormat)),
    watermark: fallback.watermark === true,
    webSearch: fallback.webSearch === true,
    sdWebUi: {
      ...(mergedImageConfig.sdWebUi || {}),
      ...(fallback.sdWebUi || {}),
    },
    __fallbackApi: true,
  };
  ...
  if (sdWebUiMode) {
    next.sdWebUi = normalizeSdWebUiSettings(next);
    next.baseApi = next.sdWebUi.baseApi;
    next.model = next.sdWebUi.model;
    next.apiKey = '';
    return next.sdWebUi.baseApi ? next : null;
  }
...
export function buildImageMonitorFallbackConfig(config = {}) {
  ...
  const next = {
    ...
    userAgent: cleanString(fallback.userAgent || config.userAgent || ''),
    __fallbackApi: true,
  };
