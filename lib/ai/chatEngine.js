...
export function shouldUseMultimodalChatModel(config = {}, { hasImages = false } = {}) {
  return config?.multimodalEnabled === true
    && (config?.smartMultimodal !== true || hasImages === true);
}

export function resolveChatModel(config = {}, { hasImages = false } = {}) {
  const textModel = config?.modelType || config?.model || config?.workingModel || '';
  if (!shouldUseMultimodalChatModel(config, { hasImages })) return textModel;
  return config?.multimodalModel || textModel;
}
...
export async function runChat(ai, toolCtx, history, targetMessage, promptCtx, humanize) {
  ...
  const builtinTools = await getBuiltinTools(toolCtx);
  ...
  const initialImageUrls = Array.isArray(toolCtx.pendingImageUrls)
    ? toolCtx.pendingImageUrls.filter(Boolean)
    : [];
  const hasInitialImages = initialImageUrls.length > 0;
  const useMultimodalModel = shouldUseMultimodalChatModel(toolCtx.config, { hasImages: hasInitialImages });
  const chatModel = resolveChatModel(toolCtx.config, { hasImages: hasInitialImages });

  logger.info(
    `[chat-engine] Session ${toolCtx.sessionId} | target: ${targetMessage.userName}(${targetMessage.userId}): "${targetMessage.content}"`
  );
  logger.info(
    `[chat-engine] Model route: ${chatModel || 'unset'} | images=${hasInitialImages} | multimodal=${toolCtx.config?.multimodalEnabled === true} | smart=${toolCtx.config?.smartMultimodal === true}`
  );
  ...
  for (let iteration = 0; maxIterations === -1 || iteration < maxIterations; iteration++) {
    ...
    const messages = buildChatCompletionMessages({
      prompt,
      targetMessage,
      imageUrls: iteration === 0 ? initialImageUrls : [],
      phase: iteration === 0 ? 'initial' : 'tool_followup',
    });

    if (iteration === 0 && hasInitialImages) {
      toolCtx.pendingImageUrls = [];
    }

    const resp = await ai.complete({
      model: chatModel,
      multimodal: useMultimodalModel,
      messages,
