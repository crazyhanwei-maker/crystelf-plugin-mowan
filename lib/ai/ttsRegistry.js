...
import { logExternalApiUsage } from './apiQualityLogger.js';
...
function buildTtsFallbackConfig(ttsConfig = {}) {
  const fallback = ttsConfig?.fallbackApi || {};
  if (fallback.enabled !== true) return null;
  const apiUrl = normalizeString(fallback.apiUrl || '', '');
  const modelsUrl = normalizeString(fallback.modelsUrl || '', '');
  if (!apiUrl && !modelsUrl) return null;
  return {
    ...ttsConfig,
    apiUrl: apiUrl || ttsConfig.apiUrl,
    dlUrl: normalizeString(fallback.dlUrl || ttsConfig.dlUrl || '', ''),
    modelsUrl: modelsUrl || ttsConfig.modelsUrl,
    __fallbackApi: true,
  };
}

function getTtsCircuitConfig(ttsConfig = {}) {
  return buildVirtualApiCircuitConfig(ttsConfig, buildTtsFallbackConfig(ttsConfig), 'tts');
}

async function requestTtsSynthesis(ttsConfig = {}, payload = {}, timeout = 120000) {
  return axios.post(ttsConfig.apiUrl, payload, {
...
async function fetchTtsModels() {
  ...
  const fallbackConfig = buildTtsFallbackConfig(ttsConfig);
  const circuitConfig = getTtsCircuitConfig(ttsConfig);
  const preferFallback = shouldPreferFallbackApi(circuitConfig, 'tts_models');
  const activeConfig = preferFallback.preferFallback && fallbackConfig?.modelsUrl ? fallbackConfig : ttsConfig;
  const apiRole = activeConfig?.__fallbackApi ? 'fallback' : 'primary';

  if (!activeConfig.modelsUrl) {
    return { success: false, error: '未配置语音模型列表地址' };
  }

  const startedAt = Date.now();
  try {
    const response = await axios.get(activeConfig.modelsUrl, {
      timeout: 20000,
    });
    ...
    await logExternalApiUsage({
      ...
      provider: 'tts-api',
      model: 'model-list',
      endpoint: activeConfig.modelsUrl,
      statusCode: response.status,
      elapsedMs: Date.now() - startedAt,
      responsePreview: `count=${Object.keys(models).length}`,
    });
    if (apiRole === 'fallback') {
      recordFallbackApiSuccess(circuitConfig, 'tts_models');
    } else {
      recordPrimaryApiSuccess(circuitConfig, 'tts_models');
    }
    ...
    if (apiRole === 'fallback') {
      recordFallbackApiFailure(circuitConfig, 'tts_models', error.response?.data?.message || error.message);
    } else {
      recordPrimaryApiFailure(circuitConfig, 'tts_models', error.response?.data?.message || error.message);
    }
    if (apiRole === 'primary' && fallbackConfig?.modelsUrl) {
      const fallbackStartedAt = Date.now();
      try {
        const response = await axios.get(fallbackConfig.modelsUrl, {
          timeout: 20000,
        });
        const data = response.data || {};
        const models = data.models || {};
        ...
        await logExternalApiUsage({
          ...
          provider: 'tts-api',
          model: 'model-list',
          endpoint: fallbackConfig.modelsUrl,
          statusCode: response.status,
        ...
        recordFallbackApiFailure(circuitConfig, 'tts_models', fallbackError.response?.data?.message || fallbackError.message);
      }
    }
    return {
      success: false,
...
async function synthesizeVoice(args = {}, toolCtx = {}) {
  ...
  const fallbackConfig = buildTtsFallbackConfig(ttsConfig);
  const circuitConfig = getTtsCircuitConfig(ttsConfig);
  const preferFallback = shouldPreferFallbackApi(circuitConfig, 'tts_synthesis');
  let activeTtsConfig = preferFallback.preferFallback && fallbackConfig?.apiUrl ? fallbackConfig : ttsConfig;
  let apiRole = activeTtsConfig?.__fallbackApi ? 'fallback' : 'primary';

  if (!activeTtsConfig.apiUrl) {
  ...
  const fallbackModel = normalizeString(
    roleMatchedModel || styleMatchedModel || toolCtx?.defaultVoiceModel || ttsConfig.defaultModel || pickFallbackModel(models),
    ''
  );
  ...
  const payload = {
    dl_url: normalizeString(args.dl_url || activeTtsConfig.dlUrl || '', ''),
    version: activeTtsConfig.version || 'v4',
    model_name: modelName,
    prompt_text_lang: language,
    emotion,
    text,
    text_lang: language,
    top_k: normalizeNumber(args.top_k ?? activeTtsConfig.topK, 10),
    top_p: normalizeNumber(args.top_p ?? activeTtsConfig.topP, 1),
    temperature: normalizeNumber(args.temperature ?? activeTtsConfig.temperature, 1),
    text_split_method: normalizeString(args.text_split_method || activeTtsConfig.textSplitMethod, '按标点符号切'),
    batch_size: normalizeNumber(args.batch_size ?? activeTtsConfig.batchSize, 10),
    batch_threshold: normalizeNumber(args.batch_threshold ?? activeTtsConfig.batchThreshold, 0.75),
    split_bucket: args.split_bucket ?? activeTtsConfig.splitBucket ?? false,
    speed_facter: normalizeNumber(args.speed_facter ?? activeTtsConfig.speed, 1),
    fragment_interval: normalizeNumber(args.fragment_interval ?? activeTtsConfig.fragmentInterval, 0.3),
    media_type: normalizeString(args.media_type || activeTtsConfig.mediaType, 'wav'),
    parallel_infer: args.parallel_infer ?? activeTtsConfig.parallelInfer ?? false,
    repetition_penalty: normalizeNumber(args.repetition_penalty ?? activeTtsConfig.repetitionPenalty, 1.35),
    seed: normalizeNumber(args.seed ?? activeTtsConfig.seed, -1),
    sample_steps: normalizeNumber(args.sample_steps ?? activeTtsConfig.sampleSteps, 16),
  ...
  try {
    const response = await requestTtsSynthesis(activeTtsConfig, payload);
    const data = response.data || {};
    if (data.msg === '合成成功' && data.audio_url) {
      ...
      await logExternalApiUsage({
        ...
        provider: 'tts-api',
        model: modelName,
        endpoint: activeTtsConfig.apiUrl,
        statusCode: response.status,
        elapsedMs: Date.now() - startedAt,
        requestPreview: sanitizeText(text, 180),
        responsePreview: normalizedAudioUrl,
      });
      if (apiRole === 'fallback') {
        recordFallbackApiSuccess(circuitConfig, 'tts_synthesis');
      } else {
        recordPrimaryApiSuccess(circuitConfig, 'tts_synthesis');
      }
    ...
    await logExternalApiUsage({
      ...
      responsePreview: JSON.stringify(data).slice(0, 220),
    });
    if (apiRole === 'fallback') {
      recordFallbackApiFailure(circuitConfig, 'tts_synthesis', data.msg || '语音合成失败');
    } else {
      recordPrimaryApiFailure(circuitConfig, 'tts_synthesis', data.msg || '语音合成失败');
    }
    if (apiRole === 'primary' && fallbackConfig?.apiUrl) {
      const fallbackPayload = { ...payload, dl_url: normalizeString(args.dl_url || fallbackConfig.dlUrl || '', '') };
      const fallbackStartedAt = Date.now();
      try {
        const fallbackResponse = await requestTtsSynthesis(fallbackConfig, fallbackPayload);
        const fallbackData = fallbackResponse.data || {};
        if (fallbackData.msg === '合成成功' && fallbackData.audio_url) {
          const normalizedAudioUrl = normalizeAudioUrl(fallbackData.audio_url, fallbackPayload.dl_url);
          await logExternalApiUsage({
            stage: 'success',
            apiType: 'tts',
            apiRole: 'fallback',
            ...
            provider: 'tts-api',
            model: modelName,
            endpoint: fallbackConfig.apiUrl,
        ...
        recordFallbackApiFailure(circuitConfig, 'tts_synthesis', fallbackData.msg || '备用语音合成失败');
      } catch (fallbackError) {
        recordFallbackApiFailure(circuitConfig, 'tts_synthesis', fallbackError.response?.data?.message || fallbackError.message);
      }
    }
    ...
    if (apiRole === 'fallback') {
      recordFallbackApiFailure(circuitConfig, 'tts_synthesis', error.response?.data?.message || error.message);
    } else {
      recordPrimaryApiFailure(circuitConfig, 'tts_synthesis', error.response?.data?.message || error.message);
    }
    if (apiRole === 'primary' && fallbackConfig?.apiUrl) {
      const fallbackPayload = { ...payload, dl_url: normalizeString(args.dl_url || fallbackConfig.dlUrl || '', '') };
      const fallbackStartedAt = Date.now();
