...
class Renderer {
  ...
  getHtmlDocument(html, options = {}) {
    const content = String(html || '');
    const marker = '<div id="render-complete" style="display:none"></div>';
    if (/<html[\s>]/i.test(content)) {
      if (/<\/body>/i.test(content)) {
        return content.replace(/<\/body>/i, `${marker}</body>`);
      }
      return `${content}${marker}`;
    }
    const background = /^#[0-9a-f]{3,8}$|^rgba?\(/i.test(String(options.background || '')) ? options.background : '#ffffff';
    const fontFamily = "'Microsoft YaHei', 'PingFang SC', 'Noto Sans SC', sans-serif";
    const width = Math.max(200, Math.min(1600, Number(options.width) || 720));
    return `
    <!DOCTYPE html>
    <html>
    <head>
  ...
  async renderHtml(html, options = {}) {
    if (!this.isInitialized) await this.init();
    try {
      logger.info('[crystelf-ai] 自定义HTML渲染开始');
      return await withPuppeteerPage(async page => {
        page.on('pageerror', (error) => {
          logger.error(`[crystelf-ai] 自定义HTML页面异常: ${error.message}`);
        });
        page.on('requestfailed', (request) => {
          logger.warn(`[crystelf-ai] 自定义HTML请求失败: ${request.url()} | ${request.failure()?.errorText || 'unknown'}`);
        });
        await page.setCacheEnabled(false);
        const width = Math.max(200, Math.min(1600, Number(options.width) || 720));
        await page.setViewport({ width, height: 900, deviceScaleFactor: 2 });
        await page.setContent(this.getHtmlDocument(html, options), { waitUntil: 'domcontentloaded', timeout: 30000 });
        await page.evaluate(async () => {
        ...
        logger.info(`[crystelf-ai] 自定义HTML渲染尺寸: ${rect.width}x${rect.height}`);
        await page.setViewport({
          width: Math.min(Math.max(rect.width, width), 1600),
          height: Math.min(Math.max(rect.height, 200), 8000),
          deviceScaleFactor: 2,
        });
        const tempDir = path.join(process.cwd(), 'temp', 'html', 'crystelf-plugin');
        if (!fs.existsSync(tempDir)) fs.mkdirSync(tempDir, { recursive: true });
        const filepath = path.join(tempDir, `html_${Date.now()}.png`);
        const imageBuffer = await page.screenshot({ fullPage: false });
        fs.writeFileSync(filepath, imageBuffer);
        logger.info(`[crystelf-ai] 自定义HTML渲染完成: ${filepath}`);
        return filepath;
      });
    } catch (error) {
      logger.error(`[crystelf-ai] 自定义HTML渲染失败: ${error.stack || error.message}`);
      return null;
    }
  }
