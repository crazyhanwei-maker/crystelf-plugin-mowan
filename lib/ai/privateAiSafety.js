...
// 内置安全关键词：误伤极低、在正常聊天中几乎不会出现的诱导性表述。
// 用户自定义关键词始终叠加在内置词之上；可通过 useBuiltinSafetyKeywords 关闭。
const BUILTIN_SAFETY_KEYWORDS = {
  highRiskKeywords: [
    '越狱', 'jailbreak', 'DAN模式', '开发者模式', '模拟越狱',
    '忽略所有规则', '无视安全限制', '取消限制', '解除限制',
  ],
  bypassKeywords: [
    '绕过安全', '绕过限制', '绕过规则', 'bypass', 'pretend',
    '扮演没有限制的', '假装没有任何规则', '你现在是自由的',
    '不要遵守', '不需要遵守', '忽略上面的指令', '忽略以上规则',
  ],
  intentKeywords: [
    '教我制作', '怎么制作炸弹', '如何制造武器', '制作毒品',
    '非法获取', '黑客攻击教程', '入侵教程', '盗号教程',
  ],
  riskKeywords: [
    '敏感内容', '违规内容', '禁止内容', 'nsfw', '18禁',
  ],
};

function mergeKeywordLists(userKeywords = [], builtinKeywords = [], useBuiltin = true) {
  if (!useBuiltin) return normalizeKeywordList(userKeywords);
  return Array.from(new Set([
    ...normalizeKeywordList(userKeywords),
    ...normalizeKeywordList(builtinKeywords),
  ]));
}
...
function normalizePrivateAiSafetyConfig(raw = {}) {
  ...
  const useBuiltin = source.useBuiltinSafetyKeywords !== false;
  return {
    ...
    warnAction: source.warnAction === 'block' ? 'block' : 'allow',
    useBuiltinSafetyKeywords: useBuiltin,
    ...
    riskKeywords: mergeKeywordLists(source.riskKeywords, BUILTIN_SAFETY_KEYWORDS.riskKeywords, useBuiltin),
    highRiskKeywords: mergeKeywordLists(source.highRiskKeywords, BUILTIN_SAFETY_KEYWORDS.highRiskKeywords, useBuiltin),
    bypassKeywords: mergeKeywordLists(source.bypassKeywords, BUILTIN_SAFETY_KEYWORDS.bypassKeywords, useBuiltin),
    intentKeywords: mergeKeywordLists(source.intentKeywords, BUILTIN_SAFETY_KEYWORDS.intentKeywords, useBuiltin),
...
export async function evaluatePrivateAiSafety(e, rawText = '', rawConfig = {}, options = {}) {
  ...
  // 「不明确」判定（review 待复审 / warn 复审不确定，均非明确 block）按 warnAction 处置：
  // allow 时放行不记警告，block 时照旧警告。
  if ((risk.risk === 'warn' || risk.risk === 'review') && config.warnAction === 'allow') {
    if (storeChanged) writeStore(store);
    return { allow: true, action: 'allow', reason: `不明确内容已放行：${risk.reason || ''}` };
  }
...
export function formatPrivateAiSafetyStatus(rawConfig = {}) {
  ...
  return [
    ...
    `- 不明确判定：${status.config.warnAction === 'allow' ? '放行（不警告）' : '警告并计数'}`,
    `- LLM 缓存：${status.config.reviewCacheEnabled ? `开启（${status.config.reviewCacheTtlHours} 小时）` : '关闭'}`,
    `- 本地规则：${hasAnyKeyword(status.config) ? '已配置' : '未配置'}${status.config.useBuiltinSafetyKeywords ? '（含内置兜底词）' : ''}`,
