...
async function probeMemeRandomUrl(apiBase, query) {
...
const Meme = {
  ...
  async getMeme(character, status, memeConfigOverride = null) {
    ...
    for (const apiBase of apiBases) {
      try {
        if (await probeMemeRandomUrl(apiBase, query)) {
  ...
  async getMemePayload(character, status, count = 1, memeConfigOverride = null) {
    ...
    for (const apiBase of apiBases) {
      try {
        const response = await fetch(buildMemeRandomUrl(apiBase, query));
        ...
        return {
          success: response.ok,
          apiBase,
          imageUrl: buildMemeRandomUrl(apiBase, query),
        };
      } catch (error) {
