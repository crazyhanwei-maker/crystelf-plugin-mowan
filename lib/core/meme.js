...
const Meme = {
  ...
  async getMeme(character, status, memeConfigOverride = null) {
    ...
    const apiBases = getMemeApiBases(memeConfig);
    ...
    for (const apiBase of apiBases) {
      try {
        if (await probeMemeRandomUrl(apiBase, query, resolveMemeApiRole(apiBase, apiBases))) {
    ...
    return buildMemeRandomUrl(apiBases[0] || resolveMemeApiBase(memeConfig), query);
  },
  ...
  async getMemePayload(character, status, count = 1, memeConfigOverride = null) {
    ...
    for (const apiBase of apiBases) {
      ...
      const apiRole = resolveMemeApiRole(apiBase, apiBases);
      try {
