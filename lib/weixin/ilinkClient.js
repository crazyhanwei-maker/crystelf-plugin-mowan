...
async function sendImageMessage({ token, toUserId, contextToken, imageBuffer }) {
  ...
  if (!uploadParam) throw new Error('ilink 上传：未返回 upload_param');

  const ciphertext = encryptAesEcb(imageBuffer, aeskey);
  const cdnUrl = `https://novac2c.cdn.weixin.qq.com/c2c/upload?${new URLSearchParams({ encrypted_query_param: uploadParam, filekey })}`;
