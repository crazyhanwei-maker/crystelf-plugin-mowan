...
// ── 收图：收到消息里的图片走 CDN 下载 + AES-128-ECB 解密，转成 data URL 供任务附件使用 ──
// 实现对齐官方实现（openclaw dist/src/cdn/pic-decrypt.js）：aes_key 兼容 base64(raw16) 与 base64(hex32) 两种形态
function decryptCdnImage(ciphertext, aesKeyBase64) {
  const decoded = Buffer.from(String(aesKeyBase64 || ''), 'base64');
  let key;
  if (decoded.length === 16) key = decoded;
  else if (decoded.length === 32 && /^[0-9a-fA-F]{32}$/.test(decoded.toString('ascii'))) key = Buffer.from(decoded.toString('ascii'), 'hex');
  else throw new Error(`aes_key 长度异常（${decoded.length} 字节）`);
...
// 下载并解密单张图片。received media 形态：{ encrypt_query_param, aes_key, encrypt_type }，部分消息带 full_url
async function downloadReceivedImage(media, baseUrl = '') {
  const encryptedQueryParam = String(media?.encrypt_query_param || '');
  const aesKey = String(media?.aes_key || '');
  const fullUrl = String(media?.full_url || media?.url || '');
  if (!encryptedQueryParam && !fullUrl) throw new Error('缺少 CDN 参数（encrypt_query_param/full_url 均为空）');
  const cdnBase = baseUrl || 'https://novac2c.cdn.weixin.qq.com/c2c';
  const url = fullUrl || `${cdnBase}/download?encrypted_query_param=${encodeURIComponent(encryptedQueryParam)}`;
  const response = await fetch(url);
  if (!response.ok) throw new Error(`CDN 下载失败：HTTP ${response.status}`);
  const buffer = decryptCdnImage(Buffer.from(await response.arrayBuffer()), aesKey);
  return buffer;
}

// 提取收到的图片（最多 maxImages 张），返回 normalizeTaskAttachments 兼容的 { name, mime, url(dataURL), size } 列表。
// 单张失败只打日志跳过，不影响其余图片与文本处理。
async function extractIncomingAttachments(message = {}, { maxImages = 3, logger = console } = {}) {
  const items = Array.isArray(message?.item_list) ? message.item_list : [];
  const mediaItems = items.filter(item => item?.type === MessageItemType.IMAGE && item?.image_item?.media).slice(0, maxImages);
  ...
  for (const item of mediaItems) {
    index += 1;
    try {
      const buffer = await downloadReceivedImage(item.image_item.media);
      if (!buffer.length || buffer.length > 5 * 1024 * 1024) throw new Error(`图片大小越界（${buffer.length} 字节）`);
      const mime = detectImageMime(buffer);
      attachments.push({
        name: `微信图片-${Date.now()}-${index}.png`,
        mime,
        url: `data:${mime};base64,${buffer.toString('base64')}`,
        size: buffer.length,
      });
    } catch (error) {
      logger?.warn?.(`[weixin-ilink] 第 ${index} 张图片提取失败：${error.message}`);
    }
  }
  return attachments;
}
