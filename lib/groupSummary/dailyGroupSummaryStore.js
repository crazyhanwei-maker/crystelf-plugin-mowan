...
export function isDailySummaryScheduleDue(date = new Date(), config = {}) {
  ...
  // 之前用 >= 会导致每个 tick（60s）只要过了计划时刻就都判定为 due，
  // 一旦 state.json 去重失效或重启丢内存就会反复触发。收紧到「当前分钟 == 计划分钟」，
  // 每天只有 1 分钟窗口可能触发，从根本上消除重复发送的窗口。
  return date.getHours() === cfg.hour && date.getMinutes() === cfg.minute;
}
