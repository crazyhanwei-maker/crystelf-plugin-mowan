...
function updateMessageWindow(groupId = '', userId = '', text = '', cfg = {}, now = nowMs()) {
  ...
  const burstWindowMs = Number(cfg.burstWindowSeconds || 20) * 1000;
...
function buildSignals(e = {}, cfg = {}, now = nowMs()) {
  ...
  if ((window.timestamps || []).length >= cfg.burstLimit) {
    signals.push(`短时刷屏 ${(window.timestamps || []).length}/${cfg.burstLimit}`);
  }
  return {
    text,
