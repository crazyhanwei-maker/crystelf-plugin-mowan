...
const moderationActionState = new Map();
...
function pruneRuntimeMaps(now = nowMs()) {
  ...
  for (const [key, value] of moderationActionState.entries()) {
    if (now - Number(value || 0) > 5 * 60 * 1000) {
      moderationActionState.delete(key);
    }
  }
}

function claimModerationAction(e = {}, now = nowMs()) {
  const groupId = normalizeId(e.group_id ?? e.groupId ?? e.gid);
  const messageId = String(e.message_id ?? e.messageId ?? '').trim();
  if (!groupId || !messageId) return true;
  const key = `${groupId}:${messageId}`;
  if (moderationActionState.has(key)) return false;
  moderationActionState.set(key, now);
  return true;
}
...
function updateMessageWindow(groupId = '', userId = '', text = '', cfg = {}, now = nowMs(), options = {}) {
  ...
  if (options.trackBurst !== false) {
    state.timestamps = [...(state.timestamps || []), now]
      .filter(item => now - Number(item || 0) <= burstWindowMs);
  }
...
export function evaluateSpamMessageWindow(groupId = '', userId = '', cfg = {}, now = nowMs()) {
  if (cfg.detectSpamMessages !== true) {
    return { triggered: false, count: 0, signal: '' };
  }
  const window = updateMessageWindow(groupId, userId, '', cfg, now, {
    trackRepeat: false,
    trackBurst: true,
  });
  const count = (window.timestamps || []).length;
  const windowSeconds = Number(cfg.burstWindowSeconds || 60);
  const windowMs = windowSeconds * 1000;
  if (count <= Number(cfg.burstLimit || 12)) {
    return { triggered: false, count, signal: '' };
  }
  if (now - Number(window.lastSpamTriggeredAt || 0) < windowMs) {
    return { triggered: false, count, signal: '' };
  }
  ...
  return {
    triggered: true,
    count,
    signal: `刷屏检测 ${count}/${cfg.burstLimit} 条/${windowSeconds} 秒`,
  };
}
...
export async function handleGroupContentModeration(e = {}) {
  ...
  if (!claimModerationAction(e)) return false;
...
export async function handleGroupSpamModeration(e = {}) {
  const groupId = normalizeId(e.group_id ?? e.groupId ?? e.gid);
  const userId = normalizeId(e.user_id ?? e.userId ?? e.uid);
  const selfId = normalizeId(e.self_id ?? e.bot?.uin ?? e.bot_id);
  if (!groupId || !userId || userId === selfId) return false;
  ...
  const state = getGroupModerationState(groupId);
  const cfg = normalizeGroupContentModerationConfig(state.content || {});
  if (!cfg.enabled || !cfg.detectSpamMessages) return false;
  if (cfg.exemptAdmins && isPrivilegedSender(e)) return false;
  if (state.whitelist?.some(item => item.userId === userId)) return false;

  const spam = evaluateSpamMessageWindow(groupId, userId, cfg);
  if (spam.count === 1 || spam.count >= Number(cfg.burstLimit || 12)) {
    logger.info(`[group-management] 刷屏计数 群[${groupId}]用户[${userId}]: ${spam.count}/${cfg.burstLimit}`);
  }
  if (!spam.triggered) return false;
  if (!claimModerationAction(e)) return false;

  const reason = spam.signal;
  const actionResult = await applyModerationAction(e, cfg, { groupId, userId, reason });
  appendGroupManagementLog({
    action: 'content_moderation_triggered',
    source: 'spamModeration',
    success: actionResult.errors.length === 0,
    group_id: groupId,
    user_id: userId,
    nickname: e.sender?.nickname || e.nickname || '',
    reason,
    error: actionResult.errors.join('；'),
    comment_preview: extractMessageText(e),
    sections: ['moderation'],
    summary: {
      requestedAction: actionResult.requestedAction,
      action: actionResult.action,
      ...
      spamWindowSeconds: Number(cfg.burstWindowSeconds || 60),
      spamLimit: Number(cfg.burstLimit || 12),
    },
  });
  logger.info(`[group-management] 群[${groupId}]用户[${userId}]触发刷屏风控: ${reason}`);
  return true;
}
