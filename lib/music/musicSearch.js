...
import { buildUserFacingErrorReply } from '../ai/userFacingError.js';
import { resolveBotIdentity } from '../system/botIdentity.js';

// 裸数字（"3"）会命中本插件规则，搜索结果若永不过期则任何人在任何时候发数字都会触发点歌。
// 结果带 TTL：过期视为无搜索状态；裸数字用短窗口，显式"发送 N"给长窗口。
const MUSIC_SEARCH_TTL_MS = 60 * 1000;
const MUSIC_SEND_TTL_MS = 5 * 60 * 1000;
...
class MusicSearch {
  ...
  async handleSearch(e, keyword) {
    try {
      ...
      const imagePath = await this.renderer.renderMusicList(
        songs,
        keyword,
        groupId,
  ...
  async handleSelection(e, selection) {
    try {
      ...
      const resolvedSong = await this.api.getSongByIndex(
        searchResult.query,
        selectedSong.sourceIndex || (index + 1),
        quality
      );

      if (!resolvedSong?.playUrl) {
        return {
          success: false,
          message: '未获取到可播放地址，请稍后重试。'
        };
      }

      const streamUrl = resolvedSong.playUrl;
      const downloadResult = await this.audioProcessor.downloadAudio(streamUrl, selectedSong, groupId);
      if (!downloadResult?.filePath) {
        return {
          success: false,
          message: '音乐文件下载失败，请稍后重试或换一首歌曲。'
        };
      }
      const processResult = await this.audioProcessor.processAudio(downloadResult.filePath, selectedSong, quality, groupId);
      if (!processResult?.filePath) {
        return {
          success: false,
          message: '音乐文件处理失败，请稍后重试。'
        };
      }
      ...
      return {
        success: false,
        message: buildUserFacingErrorReply(e, {
          groupMessage: '播放失败，请稍后重试或换一首歌曲。',
          prefix: '播放失败',
          error,
        })
      };
  ...
  async handleDirectPlay(e, songName) {
    try {
      ...
      const config = configControl.get('music');
      const quality = config?.quality || 3;
      const resolvedSong = await this.api.getSongByIndex(songName, firstSong.sourceIndex || 1, quality);
   ...
   * @param {number} ttlMs 有效期毫秒；0 表示不过期（内部读取用）
   * @returns {Object|null} 搜索结果，过期返回 null 并清理
   */
  getGroupSearchResult(groupId, ttlMs = MUSIC_SEARCH_TTL_MS) {
    const entry = this.searchHistory.get(String(groupId));
    if (!entry) return null;
    if (ttlMs > 0 && Date.now() - Number(entry.timestamp || 0) > ttlMs) {
