...
class MusicApi {
  ...
  normalizeSong(song = {}, index = 0) {
    return {
      ...
      playUrl: song.play_url || song.playUrl || song.url || song.music_url || song.musicUrl || '',
      lyric: song.lrc || '',
    };
  }

  async getSongByIndex(query, index, quality) {
    const qualityParam = this.getQualityParam(quality);
    const attempts = [
      { msg: query, n: index, br: qualityParam },
      { msg: query, n: index, bf: qualityParam },
      { msg: query, n: index },
    ];
    ...
    logger.error(`[crystelf-music] API返回格式异常: query=${query}, index=${index}, quality=${qualityParam}`);
    return null;
  }
