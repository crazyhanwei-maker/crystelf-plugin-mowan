...
export class CarbonAuth extends plugin {
  ...
  async cmdBypass(e) {
    ...
    if (!atElem || !atElem.qq) return e.reply('请 @需要绕过验证的成员。', true);
  ...
  async cmdRevalidate(e) {
    ...
    if (!atElem || !atElem.qq) return e.reply('请 @需要重新验证的成员。', true);
    const targetId = Number(atElem.qq);
    const member = await e.group.pickMember(targetId).getInfo();
    if (member.role === 'owner' || member.role === 'admin') {
      return e.reply('管理员不需要重新验证。', true);
    }
    const key = `${e.group_id}_${targetId}`;
    if (pending.get(key)) return e.reply('该成员正在验证中。', true);
...
Bot.on?.('message.group', async (e) => {
  ...
  if (type === 'math') {
    ...
    if (tries >= cfg.frequency) {
      ...
      e.reply([segment.at(e.user_id), '验证失败，你答错次数过多。'], true);
      return await Group.groupKick(e, e.user_id, e.group_id, false);
    }
    if (cfg.recall) await Message.deleteMsg(e, e.message_id);
    return e.reply(
      [segment.at(e.user_id), `回答错误，你还有 ${cfg.frequency - tries} 次机会，请再试一次。`],
      true
    );
  }
...
async function auth(e, group_id, user_id) {
  ...
  if (groupCfg.timeout > 60) {
    setTimeout(
      async () => {
        if (pending.has(key)) {
          await e.reply([segment.at(user_id), '提醒：你还有 1 分钟完成验证。']);
        }
      },
